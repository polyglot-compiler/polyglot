...
public class JL7TypeSystem_c extends JL5TypeSystem_c implements JL7TypeSystem {
    ...
    public ConstructorInstance findConstructor(
        ...
        return findConstructor(container, argTypes, typeArgs, currClass, null, fromClient);
    }
    ...
    public ConstructorInstance findConstructor(
        ...
        List<ConstructorInstance> acceptable =
                findAcceptableConstructors(
                        container, argTypes, typeArgs, currClass, expectedObjectType, fromClient);
    ...
    protected List<ConstructorInstance> findAcceptableConstructors(
            ...
            boolean fromClient)
            throws SemanticException {
        return findAcceptableConstructors(
                container, argTypes, actualTypeArgs, currClass, null, fromClient);
    }
    ...
    protected List<ConstructorInstance> findAcceptableConstructors(
        ...
        container = (ClassType) applyCaptureConversion(container, container.position());
        ...
        for (JL5ConstructorInstance ci : constructors) {
            ...
            JL5ConstructorInstance substCi =
                    (JL5ConstructorInstance)
                            callValid(ci, argTypes, actualTypeArgs, expectedObjectType);
            if (substCi != null) {
                ci = substCi;
                if (isAccessible(ci, currClass, fromClient)) {
                    if (Report.should_report(Report.types, 3))
                        Report.report(3, "->acceptable: " + ci);
                    if (varArgsRequired(ci)) phase3constructors.add(ci);
                    else if (boxingRequired(ci, argTypes)) phase2constructors.add(ci);
    ...
    public JL5ProcedureInstance callValid(
        ...
        if (ct instanceof JL5ParsedClassType && !((JL5ParsedClassType) ct).typeVariables().isEmpty()
