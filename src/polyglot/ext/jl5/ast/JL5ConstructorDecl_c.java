...
public class JL5ConstructorDecl_c extends ConstructorDecl_c implements
	...
	public JL5ConstructorDecl_c(Position pos, Flags flags, Id name,
			List formals, List throwTypes, Block body) {
		this(pos, flags, name, formals, throwTypes, body,
				new ArrayList<ParamTypeNode>());
	}

	public JL5ConstructorDecl_c(Position pos, Flags flags, Id name,
			List formals, List throwTypes, Block body, List typeParams) {
	...
	protected JL5ConstructorDecl_c reconstruct(List formals, List throwTypes,
		...
		if (!CollectionUtil.equals(formals, this.formals)
			...
			n.throwTypes = TypedList.copyAndCheck(throwTypes, TypeNode.class,
					true);
			n.body = body;
			n.typeParams = typeParams;
			return n;
	...
	public Node buildTypes(TypeBuilder tb) throws SemanticException {
		...
		ConstructorInstance ci = ts.constructorInstance(position(), ct, flags,
				formalTypes, throwTypes, typeParams);
	...
	public Node disambiguate(AmbiguityRemover ar) throws SemanticException {
		...
		JL5ConstructorInstance ci = (JL5ConstructorInstance) n
				.constructorInstance();
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
			...
			if (f.isVarArg()) {
				if (i != formals.size() - 1) {
					throw new SemanticException(
							"Only last formal can be variable in constructor declaration.",
							f.position());
				} else {
		...
		// check that the varargs flag is consistent with the type of the last
		// argument.
		if (JL5Flags.isVarArgs(cd.flags()) != JL5Flags.isVarArgs(flags)) {
			throw new InternalCompilerError(
					"VarArgs flag of AST and type disagree");
		}
		...
		if (JL5Flags.isVarArgs(flags)) {
			...
			if (ci.formalTypes().isEmpty()) {
				throw new InternalCompilerError(
						"Inconsistent var args flag with procedure type");
			}
			Type last = (Type) ci.formalTypes()
					.get(ci.formalTypes().size() - 1);
			if (!(last instanceof JL5ArrayType && ((JL5ArrayType) last)
					.isVarArg())) {
				throw new InternalCompilerError(
						"Inconsistent var args flag with procedure type");
			}
