...
public class ParamTypeNode_c extends TypeNode_c implements ParamTypeNode {
	...
	public Node disambiguate(AmbiguityRemover ar) throws SemanticException {
		...
		for (Iterator it = bounds.iterator(); it.hasNext();) {
			...
			// throw new
			// SemanticException("Cannot instantiate a type parameter with type "
			// + tn.type());
			// }
		...
		if (!ambiguous) {
			...
			// System.err.println("paramtypenode_c : dismab and setting bounds of "
			// + tv + " to " + typeList);
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
			...
			for (int j = i + 1; j < bounds.size(); j++) {
				...
				if (tc.typeSystem().equals(ti.type(), tj.type())) {
					throw new SemanticException(
							"Duplicate bound in type variable declaration",
							tj.position());
				}
			}
		}
		...
		for (int i = 0; i < bounds.size(); i++) {
			...
			if (ti.type() instanceof ArrayType) {
				throw new SemanticException(
						"Unexpected type bound in type variable declaration",
						ti.position());
			}
		}
