...
public class JL5Disamb_c extends Disamb_c {
	...
	protected Node disambiguateTypeNodePrefix(TypeNode tn)
		...
		if (t.isReference() && exprOK()) {
			try {
				FieldInstance fi = ((JL5TypeSystem) ts).findFieldOrEnum(
						t.toReference(), name.id(), c.currentClass());
		...
		if (t.isClass() && typeOK()) {
			if (t instanceof JL5SubstClassType) {
				// if t is an instantiated class, then use the base class to
				// resolve names
	...
	protected Node disambiguateNoPrefix() throws SemanticException {
		...
		TypeVariable res = ((JL5Context) c).findTypeVariableInThisScope(name
				.id());
		if (res != null) {
			// System.out.println("JL5Disamb: TypeVariable " +name.id() +
			// " has type " + res + " " + res.getClass());
				...
				if (n instanceof Type) {
					...
					if (!type.isCanonical()) {
						throw new InternalCompilerError(
								"Found an ambiguous type in the context: "
										+ type, pos);
					}
					return nf.CanonicalTypeNode(pos, type);
				}
