...
public class JL5MethodDecl_c extends MethodDecl_c implements JL5MethodDecl {
	...
	public JL5MethodDecl_c(Position pos, Flags flags, TypeNode returnType,
			Id name, List formals, List throwTypes, Block body) {
		this(pos, flags, returnType, name, formals, throwTypes, body,
	...
	public Node buildTypes(TypeBuilder tb) throws SemanticException {
		...
		MethodInstance mi = ts.methodInstance(position(), ct, f,
				ts.unknownType(position()), name.id(), formalTypes, throwTypes,
				typeParams);
		ct.addMethod(mi);
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
		...
		if (tc.context().currentClass().flags().isInterface()) {
			if (flags.isProtected() || flags.isPrivate()) {
				throw new SemanticException(
						"Interface methods must be public.", position());
			}
		}
		...
		if (flags.isStatic()
				&& !JL5Flags.isEnum(methodInstance().container().toClass()
						.flags())
		...
		if (JL5Flags.isVarArgs(flags)) {
			...
			Type last = (Type) mi.formalTypes()
					.get(mi.formalTypes().size() - 1);
