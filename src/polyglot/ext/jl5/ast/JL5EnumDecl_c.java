...
public class JL5EnumDecl_c extends JL5ClassDecl_c implements JL5EnumDecl {
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
		if (flags().isAbstract()) {
			throw new SemanticException(
					"Enum types cannot have abstract modifier", this.position());
		}
		if (flags().isPrivate() && !type().isInnerClass()) {
			throw new SemanticException(
					"Top level enum types cannot have private modifier",
					this.position());
		}
		if (flags().isFinal()) {
			throw new SemanticException(
					"Enum types cannot have final modifier", this.position());
		}
		...
		for (Iterator it = type().constructors().iterator(); it.hasNext();) {
			...
			if (!ci.flags().clear(Flags.PRIVATE).equals(Flags.NONE)) {
				throw new SemanticException(
						"Modifier " + ci.flags().clear(Flags.PRIVATE)
	...
	protected Node addDefaultConstructorIfNeeded(TypeSystem ts, NodeFactory nf)
		...
		JL5EnumDecl_c n = (JL5EnumDecl_c) super.addDefaultConstructorIfNeeded(
				ts, nf);
	...
	protected Node addDefaultConstructor(TypeSystem ts, NodeFactory nf)
		...
		args.add(nf.NullLit(Position.compilerGenerated()));// XXX the right
															// thing to do is
															// change the type
															// of java.lang.Enum
															// instead of adding
		...
		Block block = nf.Block(position().startOf(), ((JL5NodeFactory) nf)
				.ConstructorCall(position.startOf(), ConstructorCall.SUPER,
						null, args, true));

		// Default constructor of an enum is private
		ConstructorDecl cd = nf.ConstructorDecl(body().position().startOf(),
				Flags.PRIVATE, name, Collections.EMPTY_LIST,
	...
	protected Node addEnumMethods(TypeSystem ts, NodeFactory nf) {
		...
		// add values method
		JL5MethodInstance valuesMI = (JL5MethodInstance) ts.methodInstance(
				position(), this.type(), flags, ts.arrayOf(this.type()),
				"values", Collections.EMPTY_LIST, Collections.EMPTY_LIST);
		this.type.addMethod(valuesMI);

		// add valueOf method
		JL5MethodInstance valueOfMI = (JL5MethodInstance) ts.methodInstance(
				position(), this.type(), flags, this.type(), "valueOf",
				Collections.singletonList(ts.String()), Collections.EMPTY_LIST);
