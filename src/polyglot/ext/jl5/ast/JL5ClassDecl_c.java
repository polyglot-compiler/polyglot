...
public class JL5ClassDecl_c extends ClassDecl_c implements JL5ClassDecl {
	...
	public Node buildTypes(TypeBuilder tb) throws SemanticException {
		...
		if (paramTypes() != null && !paramTypes().isEmpty()) {
			List<TypeVariable> typeVars = new ArrayList(this.paramTypes()
					.size());
	...
	protected ClassDecl reconstruct(Id name, TypeNode superClass,
			List interfaces, ClassBody body, List paramTypes) {
	...
	public void prettyPrintHeader(CodeWriter w, PrettyPrinter tr) {
		...
		if (printTypeVars && !this.paramTypes().isEmpty()) {
			w.write("<");
			for (Iterator<ParamTypeNode> iter = this.paramTypes.iterator(); iter
					.hasNext();) {
