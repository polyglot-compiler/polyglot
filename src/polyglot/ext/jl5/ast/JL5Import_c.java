...
public class JL5Import_c extends Import_c implements JL5Import {
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
		...
		if (kind() == JL5Import.SINGLE_STATIC_MEMBER) {
			Type nt = (Type) tc.typeSystem().forName(
					StringUtil.getPackageComponent(name));
			String id = StringUtil.getShortNameComponent(name);
			if (!isIdStaticMember(nt.toClass(), id,
					(JL5TypeSystem) tc.typeSystem(), tc.context().package_())) {
				...
				for (TopLevelDecl tld : decls) {
					if (id.equals(tld.name())) {
						throw new SemanticException(
								"Static import conflicts with top level declaration "
										+ id, position());
					}
				}
			}
	...
	private boolean isIdStaticMember(ClassType t, String id, JL5TypeSystem ts,
		...
		try {
			...
			if (fi != null
					&& fi.flags().isStatic()
					&& ts.accessibleFromPackage(fi.flags(), t.package_(),
							package_)) {
				return true;
			}
		...
		try {
			...
			if (ct != null
					&& ct.flags().isStatic()
					&& ts.accessibleFromPackage(ct.flags(), t.package_(),
							package_)) {
				return true;
			}
		} catch (SemanticException e) {
