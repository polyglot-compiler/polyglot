...
public class JL5Import_c extends Import_c implements JL5Import{
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
        ...
        if (kind() == JL5Import.SINGLE_STATIC_MEMBER){
        	Type nt = (Type) tc.typeSystem().forName(StringUtil.getPackageComponent(name));
            String id = StringUtil.getShortNameComponent(name);
            if (!isIdStaticMember(nt.toClass(), id, (JL5TypeSystem)tc.typeSystem(), tc.context().package_())){
                throw new SemanticException("Cannot find static member " + id +" in class: "+ nt, position());
            }
            ...
            if (tc.job().ast() instanceof SourceFile) {
                ...
                for (TopLevelDecl tld : decls) {
                    if (id.equals(tld.name())) {
                        throw new SemanticException("Static import conflicts with top level declaration " + id, position());                        
                    }
                }
            }
            return this;
    ...
    private boolean isIdStaticMember(ClassType t, String id, JL5TypeSystem ts, Package package_){
	...
	public String toString(){
    	if(kind() == SINGLE_STATIC_MEMBER || kind() == STATIC_ON_DEMAND) {
    		return "import static "+ name + (kind() == STATIC_ON_DEMAND ? ".*": "");
    	}
