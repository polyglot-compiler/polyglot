...
public class JL5ConditionalExt extends JL5ExprExt {
        ...
        if (ts.unboxingConversion(t1).isNumeric() && ts.unboxingConversion(t2).isNumeric()) {
            ...
            if (t1.equals(ts.typeForName("java.lang.Byte"))
                    ...
                    && ts.numericConversionValid(
                            ts.Byte(), tc.lang().constantValue(e2, tc.lang()))) {
                return c.type(ts.Byte());
            }
            if (t2.equals(ts.typeForName("java.lang.Byte"))
                    && t1.isInt()
                    && ts.numericConversionValid(
                            ts.Byte(), tc.lang().constantValue(e1, tc.lang()))) {
                return c.type(ts.Byte());
            }

            // If one of the operands is of type Short and the other operand is a constant
            // expression of
            ...
            if (t1.equals(ts.typeForName("java.lang.Short"))
                    && t2.isInt()
                    && ts.numericConversionValid(
                            ts.Short(), tc.lang().constantValue(e2, tc.lang()))) {
                return c.type(ts.Short());
            }
            if (t2.equals(ts.typeForName("java.lang.Short"))
                    && t1.isInt()
                    && ts.numericConversionValid(
                            ts.Short(), tc.lang().constantValue(e1, tc.lang()))) {
                return c.type(ts.Short());
            }
            ...
            if (t1.equals(ts.typeForName("java.lang.Character"))
                    && t2.isInt()
                    && ts.numericConversionValid(
                            ts.Char(), tc.lang().constantValue(e2, tc.lang()))) {
                return c.type(ts.Char());
            }
            if (t2.equals(ts.typeForName("java.lang.Character"))
                    && t1.isInt()
                    && ts.numericConversionValid(
                            ts.Char(), tc.lang().constantValue(e1, tc.lang()))) {
                return c.type(ts.Char());
            }
    ...
    public Type find_lub(JL5TypeSystem ts, Type t1, Type t2) throws SemanticException {
        if (t1.isArray()
            ...
            Type base_lub = find_lub(ts, t1.toArray().base(), t2.toArray().base());
