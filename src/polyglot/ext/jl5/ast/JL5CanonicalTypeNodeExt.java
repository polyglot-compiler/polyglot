...
public class JL5CanonicalTypeNodeExt extends JL5TermExt {
    ...
    public Node typeCheck(TypeChecker tc) throws SemanticException {
        ...
        if (t instanceof JL5SubstClassType) {
            ...
            // Check for rare types: e.g., Outer<String>.Inner, where Inner has uninstantiated type
            // variables
            // See JLS 3rd ed. 4.8
                ...
                for (TypeVariable tv : st.base().typeVariables()) {
                    if (!st.subst().substitutions().keySet().contains(tv)) {
                        throw new SemanticException(
                                "\"Rare\" types are not allowed: cannot "
                                        + "use raw class "
                                        + st.name()
        ...
        if (tc.context().inStaticContext() && !((JL5Context) tc.context()).inCTORCall()) {
            for (TypeVariable tv : findInstanceTypeVariables(t)) {
                throw new SemanticException(
                        "Type variable " + tv + " cannot be used in a static context",
                        n.position());
            }
        }
    ...
    protected void checkSubtypeConstraints(JL5TypeSystem ts, Type t) throws SemanticException {
        if (t instanceof JL5SubstClassType) {
            JL5SubstClassType substClass = (JL5SubstClassType) t;
            JL5ParsedClassType pct = substClass.base();
            JL5SubstClassType capSC =
                    (JL5SubstClassType)
                            ts.applyCaptureConversion(substClass, substClass.position());
            for (JL5ParsedClassType cur = pct;
                    cur != null;
                ...
                for (TypeVariable tv : cur.typeVariables()) {
                    ...
                    Type upperBound = capSC.subst().substType(tv.upperBound());
                    ...
                    checkSubtypeConstraints(ts, substClass.subst().substType(tv));
                }
            }
