...
public class JL5NodeFactory_c extends NodeFactory_c implements JL5NodeFactory {
	...
	public JL5EnumDecl EnumDecl(Position pos, Flags flags, Id name,
		...
		JL5EnumDecl n = new JL5EnumDecl_c(pos, flags, name, superType,
				interfaces, body);
	...
	@Override
	public JL5ClassDecl ClassDecl(Position pos, Flags flags, Id name,
			TypeNode superType, List interfaces, ClassBody body, List paramTypes) {
		JL5ClassDecl n = new JL5ClassDecl_c(pos, flags, name, superType,
				interfaces, body, paramTypes);
	...
	public ClassBody ClassBody(Position pos, List members) {
		ClassBody n = new JL5ClassBody_c(pos,
	...
	public EnumConstantDecl EnumConstantDecl(Position pos, Flags flags,
			Id name, List args, ClassBody body) {
		EnumConstantDecl n = new EnumConstantDecl_c(pos, flags, name, args,
				body);
	...
	public ConstructorCall ConstructorCall(Position pos, Kind kind, Expr outer,
		...
		ConstructorCall n = new JL5ConstructorCall_c(pos, kind, outer, args,
	...
	public JL5ConstructorDecl ConstructorDecl(Position pos, Flags flags,
		...
		JL5ConstructorDecl n = ConstructorDecl(pos, flags, name, formals,
				throwTypes, body, null);
	...
	public JL5ConstructorDecl ConstructorDecl(Position pos, Flags flags,
		...
		if (typeParams == null) {
			n = new JL5ConstructorDecl_c(pos, flags, name, formals, throwTypes,
					body);
		} else {
			n = new JL5ConstructorDecl_c(pos, flags, name, formals, throwTypes,
					body, typeParams);
	...
	public JL5MethodDecl MethodDecl(Position pos, Flags flags,
			...
			Block body) {
		JL5MethodDecl n = MethodDecl(pos, flags, returnType, name, formals,
				throwTypes, body, null);
	...
	public JL5MethodDecl MethodDecl(Position pos, Flags flags,
			TypeNode returnType, Id name, List formals, List throwTypes,
			Block body, List typeParams) {
		JL5MethodDecl n;
		if (typeParams == null) {
			n = new JL5MethodDecl_c(pos, flags, returnType, name, formals,
					throwTypes, body);
		} else {
			n = new JL5MethodDecl_c(pos, flags, returnType, name, formals,
					throwTypes, body, typeParams);
		}
		n = (JL5MethodDecl) n.ext(extFactory().extMethodDecl());
		n = (JL5MethodDecl) n.del(delFactory().delMethodDecl());
		return n;
	...
	public ParamTypeNode ParamTypeNode(Position pos, List bounds, Id id) {
		...
		n = (polyglot.ext.jl5.ast.ParamTypeNode) n.ext(extFactory()
				.extParamTypeNode());
		n = (polyglot.ext.jl5.ast.ParamTypeNode) n.del(delFactory()
				.delParamTypeNode());
		return n;
	}
	...
	public AmbTypeInstantiation AmbTypeInstantiation(Position pos,
			TypeNode base, List typeArguments) {
		AmbTypeInstantiation n = new AmbTypeInstantiation(pos, base,
				typeArguments);
		return n;
	}
	...
	@Override
	public Call Call(Position pos, Receiver target, List typeArgs, Id name,
			List args) {
		Call n = new JL5Call_c(pos, target,
	...
	public New New(Position pos, Expr outer, List<TypeNode> typeArgs,
			TypeNode objectType, List args, ClassBody body) {
		New n = new JL5New_c(pos, outer, CollectionUtil.nonNullList(typeArgs),
