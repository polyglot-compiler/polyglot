...
public class JL5AssignDel extends JL5Del {
    ...
    public Node typeCheck(TypeChecker tc) throws SemanticException {
        ...
        if (a.operator() == Assign.ADD_ASSIGN) {
            ...
            throw new SemanticException("The " + a.operator() + " operator must have "
                                        + "numeric or String operands.",
                                        a.position());
        }

        if (a.operator() == Assign.SUB_ASSIGN || a.operator() == Assign.MUL_ASSIGN ||
                a.operator() == Assign.DIV_ASSIGN || a.operator() == Assign.MOD_ASSIGN) {
            if (isNumeric(t) && isNumeric(s)) {
                return a.type(ts.promote(numericType(t), numericType(s)));
            }
