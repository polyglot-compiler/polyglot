...
public interface JL5TypeSystem extends TypeSystem, ParamTypeSystem {
	...
	/**
	 * 
	 * @return the list of types that makes the implicit cast valid. Returns
	 *         empty list if isImplicitCastValid(t1, t2) == false. If the return
	 *         value is non empty, then t1 is the first element, and t2 is the
	 *         last.
	 */
	LinkedList<Type> isImplicitCastValidChain(Type t1, Type t2);

	/**
	 * Do methods mi and mj have the same signature? See JLS 3rd ed. 8.4.2 They
	 * have the same signature if they have the same name and argument types.
	 * Two method or constructor declarations M and N have the same argument
	 * types if all of the following conditions hold: 1. They have the same
	 * number of formal parameters (possibly zero) 2. They have the same number
	 * of type parameters (possibly zero) 3. Let <A1,...,An> be the formal type
	 * parameters of M and let <B1,...,Bn> be the formal type parameters of N.
	 * After renaming each occurrence of a Bi in N's type to Ai the bounds of
	 * corresponding type variables and the argument types of M and N are the
	 * same.
	 */
	boolean hasSameSignature(JL5MethodInstance mi, JL5MethodInstance mj);

	/**
	 * Is method mi a sub-signature of method mj? The signature ofmethod mi is a
	 * subsignature of the signature of a method mj if either mj hasthe same
	 * signature as mi, or the signature of mi is the same as the erasure of the
	 * signature of mj. See JLS 3rd ed. 8.4.2
	 */
	boolean isSubSignature(JL5MethodInstance mi, JL5MethodInstance mj);

	/**
	 * Are methods mi and mj override equivalent? See JLS 3rd ed. 8.4.2 They are
	 * override equivalent if either mi is a subsignature of mj, or mj is a
	 * subsignature of mi.
	 */
	boolean areOverrideEquivalent(JL5MethodInstance mi, JL5MethodInstance mj);

	/**
	 * Are types ri and rj return type substitutable? See JLS 3rd ed. 8.4.5 They
	 * are return type substitutable if: - if ri is a primitive type, and rj is
	 * identical to ri. - if ri is a reference type, and ri is either a subtype
	 * of rj or ri can be converted to a subtype of rj by unchecked conversion
	 * (5.1.9), or ri is the erasure of rj. - if ri is void and rj is void.
	 */
	boolean areReturnTypeSubstitutable(Type ri, Type rj);

	/**
	 * Is there an unchecked conversion from "from" to "to"? See JLS 3rd ed.
	 * 5.1.9
	 * 
	 * True if: "from" is a generic type G with n formal type parameters, and
	 * "to" is any parameterized type of the form G<T1 ... Tn>.
	 */
	boolean isUncheckedConversion(Type from, Type to);

	/**
	 * Returns the erased type of t. See JLS 3rd ed. 4.6
	 * 
	 */
	 ...
	 * then type inference should be performed. (See JLS 3rd ed. 15.12.2.7) Will
	 * return null if mi cannot be successfully called. Will return an
	 * appropriately instantiated method instance if the call is valid (i.e.,
	 * the substitution after type inference).
	 */
	JL5MethodInstance methodCallValid(JL5MethodInstance mi, String name,
			List<Type> argTypes, List<Type> actualTypeArgs);

	/**
	 * Check whether <code>ci</code> can be called with arguments of type
	 * <code>actualTypes</code>, with type parameters instantiated with
	 * actualTypeArgs. If actualTypeArgs is null or empty, then type inference
	 * should be performed. (See JLS 3rd ed. 15.12.2.7) Will return null if ci
	 * cannot be successfully called. Will return an appropriately instantiated
	 * instance if the call is valid (i.e., the substitution after type
	 * inference).
	 */
	...
	/**
	 * Returns the PrimitiveType corresponding to the wrapper type t. For
	 * example primitiveOf([java.lang.Integer]) = [int]
	 */
	...
	ConstructorInstance findConstructor(ClassType container, List argTypes,
			List<Type> typeArgs, ClassType currClass) throws SemanticException;

	/**
	 * Base is a generic supertype (e.g., a class C with uninstantiated
	 * parameters). Try to find a supertype of sub that is an instantiation of
	 * base.
	 */
