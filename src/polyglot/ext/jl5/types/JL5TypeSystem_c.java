...
public class JL5TypeSystem_c extends ParamTypeSystem_c implements JL5TypeSystem {
	...
	protected final Flags TOP_LEVEL_CLASS_FLAGS = JL5Flags
			.setEnum(super.TOP_LEVEL_CLASS_FLAGS);
	...
	public MethodInstance methodInstance(Position pos, ReferenceType container,
		...
		return methodInstance(pos, container, flags, returnType, name,
				argTypes, excTypes, Collections.EMPTY_LIST);
	}
	...
	protected List findAcceptableMethods(ReferenceType container, String name,
		...
		return findAcceptableMethods(container, name, argTypes,
				Collections.EMPTY_LIST, currClass);
	}

	// @Override
	protected List findAcceptableMethods(ReferenceType container, String name,
			List argTypes, List actualTypeArgs, ClassType currClass)
			throws SemanticException {
	...
	protected List findAcceptableMethods(ReferenceType container, String name,
		...
		while (!typeQueue.isEmpty()) {
			...
			for (JL5MethodInstance mi : (List<JL5MethodInstance>) type
					.toReference().methods()) {
				...
				// System.err.println("methodCallValidImpl for " + mi + " " +
				// name + " " + argTypes + " : " + substMi);
				if (substMi != null) {
					mi = substMi;
	...
	@Override
	public boolean methodCallValid(MethodInstance mi, String name, List argTypes) {
	...
	public boolean callValid(ProcedureInstance mi, List argTypes) {
		return this.callValid((JL5ProcedureInstance) mi, (List<Type>) argTypes,
				null) != null;
	}
	...
	public JL5ProcedureInstance callValid(JL5ProcedureInstance mi,
		...
		if (!mi.typeParams().isEmpty() && subst != null) {
			...
			for (TypeVariable tv : (Set<TypeVariable>) subst.substitutions()
					.keySet()) {
	 ...
	 * @return
	 */
	private JL5Subst inferTypeArgs(JL5ProcedureInstance mi, List argTypes,
			Type expectedReturnType) {
	...
	public JL5ProcedureInstance instantiate(Position pos,
			JL5ProcedureInstance mi, List<Type> actuals) {
	...
	public boolean areReturnTypeSubstitutable(Type ri, Type rj) {
		if (ri.isPrimitive()) {
			...
			return ri.isSubtype(rj) || this.isUncheckedConversion(ri, rj)
					|| ri.typeEquals(this.erasureType(rj));
		} else if (ri.isVoid()) {
			return rj.isVoid();
		} else {
	 ...
	 */
	protected List findAcceptableConstructors(ClassType container,
			List argTypes, List<Type> actualTypeArgs, ClassType currClass)
			throws SemanticException {
		assert_(container);
		assert_(argTypes);
		...
		for (JL5ConstructorInstance ci : (List<JL5ConstructorInstance>) container
				.constructors()) {
			if (Report.should_report(Report.types, 3))
	...
	public JL5SubstClassType findGenericSupertype(JL5ParsedClassType base,
		...
		for (ReferenceType a : ancestors) {
			...
			JL5ParsedClassType instBase = (JL5ParsedClassType) instantiatedType
					.base();
	...
	protected UnknownType unknownReferenceType = new UnknownReferenceType_c(
			this);
