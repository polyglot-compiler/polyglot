...
public class JL5Subst_c extends Subst_c implements JL5Subst {
	...
	public JL5Subst_c(ParamTypeSystem ts, Map subst, Map cache,
		...
		for (Iterator i = entries(); i.hasNext();) {
			...
			if (e.getKey() instanceof TypeVariable
					&& e.getValue() instanceof Type)
				continue;
	...
	protected Type uncachedSubstType(Type t) {
		if (t instanceof JL5SubstClassType && isRawClass) {
			// don't perform substitution on subst types, just return a raw
			// class
	...
	public Type substTypeVariable(TypeVariable t) {
		...
		t = (TypeVariable) t.upperBound((ReferenceType) substType(t
				.upperBound()));
		return t;
	}
	...
	public ConstructorInstance substConstructor(ConstructorInstance ci) {
		JL5ConstructorInstance cj = (JL5ConstructorInstance) super
				.substConstructor(ci);
