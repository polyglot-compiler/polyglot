...
public class InferenceSolver_c implements InferenceSolver {
	...
	public InferenceSolver_c(JL5ProcedureInstance pi, List<Type> actuals,
			JL5TypeSystem ts) {
		this.pi = pi;
	...
	// public InferenceSolver_c(List<TypeVariable> typeVars, List<Type> formals,
	// List<Type> actuals,
	// JL5TypeSystem ts) {
	// this.ts = ts;
	// this.actualArgumentTypes = actuals;
		...
		for (int i = 0; i < solution.length; i++) {
			if (solution[i] == null) {
				...
				if (u.size() == 1) {
					...
					solution[i] = ts.intersectionType(
							Position.compilerGenerated(), u);
				}
			}
		}
	...
	private List<Constraint> getInitialConstraints() {
		...
		for (int i = 0; i < numFormals - 1; i++) {
			constraints.add(new SubConversionConstraint(actualArgumentTypes
					.get(i), formalTypes.get(i), this));
		}
		if (numFormals > 0) {
			if (pi != null && JL5Flags.isVarArgs(pi.flags())) {
				JL5ArrayType lastFormal = (JL5ArrayType) pi.formalTypes().get(
						numFormals - 1);
				for (int i = numFormals - 1; i < actualArgumentTypes.size() - 1; i++) {
					constraints
							.add(new SubConversionConstraint(
									actualArgumentTypes.get(i), lastFormal
											.base(), this));
				}
			} else if (numFormals == actualArgumentTypes.size()) {
				constraints.add(new SubConversionConstraint(actualArgumentTypes
						.get(numFormals - 1), formalTypes.get(numFormals - 1),
						this));
			}
		}
		return constraints;
