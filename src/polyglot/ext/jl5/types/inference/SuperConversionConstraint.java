	...
	public List<Constraint> simplify() {
		...
		if (solver().isTargetTypeVariable(formal)) {
			...
			if (formal.isArray()) {
				if (actual.isArray() && actual.toArray().base().isReference()) {
					r.add(new SuperConversionConstraint(
							actual.toArray().base(), formal.toArray().base(),
							solver));
			...
			if (!actual_pt.base().equals(formal_pt.base())) {
				f = solver.typeSystem().findGenericSupertype(actual_pt.base(),
						formal_pt);
				if (f != null) {
					// SC: this appears to be dead code
					// solver.typeSystem().applySubstitution(f,
					// formal_pt.base().typeVariables(),
					// formal_pt.typeArguments());
			...
			if (f != null) {
				for (TypeVariable tv : ((JL5ParsedClassType) formal_pt.base())
						.typeVariables()) {
					ReferenceType formal_targ = (ReferenceType) f.subst()
							.substType(tv);
					ReferenceType actual_targ = (ReferenceType) actual_pt
							.subst().substType(tv);
