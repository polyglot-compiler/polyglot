...
/**
 * Represents a constraint A << F See JLS 3rd ed. 15.12.2.7
 * 
 */
	...
	public List<Constraint> simplify() {
		...
		if (actual instanceof JL5PrimitiveType) {
			...
			r.add(new SubConversionConstraint(solver().typeSystem()
					.wrapperClassOfPrimitive(prim_actual), formal, solver));
			...
			JL5SubstClassType s = solver.typeSystem().findGenericSupertype(
					(JL5ParsedClassType) formal_pt.base(),
					(ReferenceType) actual);
			if (s != null) {
				for (TypeVariable tv : ((JL5ParsedClassType) formal_pt.base())
						.typeVariables()) {
					ReferenceType formal_targ = (ReferenceType) formal_pt
							.subst().substType(tv);
					ReferenceType actual_targ = (ReferenceType) s.subst()
							.substType(tv);
					if (!(formal_targ instanceof WildCardType)) {
						...
						if (!(actual_targ instanceof WildCardType)) {
							r.add(new SuperConversionConstraint(actual_targ,
									formal_targ_wc.lowerBound(), solver));
						} else if (actual_targ instanceof WildCardType
								&& ((WildCardType) actual_targ)
										.isSuperConstraint()) {
							WildCardType actual_targ_wc = (WildCardType) actual_targ;
							r.add(new SuperConversionConstraint(actual_targ_wc
									.lowerBound(), formal_targ_wc.lowerBound(),
									solver));
						}
					}
				}
