...
public class JL5Signature extends Attribute {
	...
	/**
	 * Grammar: class_sig = formal_type_params_opt super_class_sig
	 * super_inter_sig_list_opt formal_type_params_opt = * empty * | LEFT_ANGLE
	 * formal_type_param_list RIGHT_ANGLE formal_type_param_list =
	 * formal_type_param | formal_type_param_list formal_type_param
	 * formal_type_param = ID class_bound inter_bound_list_opt class_bound =
	 * COLON field_type_sig_opt inter_bound_list_opt = * empty * |
	 * inter_bound_list inter_bound_list = inter_bound inter_bound_list
	 * inter_bound inter_bound = COLON field_type_sig super_class_sig =
	 * class_type_sig super_inter_sig_list_opt = * empty * |
	 * super_inter_sig_list super_inter_sig_list = super_inter_sig |
	 * super_inter_sig_list | super_inter_sig super_inter_sig = class_type_sig
	 * field_type_sig = class_type_sig | array_type_sig | type_var_sig
	 * class_type_sig = L pack_spec_list_opt simple_class_type_sig
	 * class_type_sig_suffix_list_opt SEMI_COLON pack_spec_list_opt = * empty *
	 * | pack_spec_list pack_spec_list = pack_spec | pack_spec_list pack_spec
	 * pack_spec = ID SLASH simple_class_type_sig = ID type_args_opt
	 * class_type_sig_suffix_list_opt = * empty * | class_type_sig_suffix_list
	 * class_type_sig_suffix_list = class_type_sig_suffix |
	 * class_type_sig_suffix_list class_type_sig_suffix class_type_sig_suffix =
	 * DOT simple_class_type_sig type_var_sig = T ID SEMI_COLON type_args =
	 * LEFT_ANGLE type_arg_list RIGHT_ANGLE type_arg_list = type_arg |
	 * type_arg_list type_arg type_arg = wild_card_ind_opt field_type_sig | STAR
	 * wild_card_ind_opt = * empty * | wild_card_ind wild_card_ind = PLUS |
	 * MINUS array_type_sig = LEFT_SQUARE type_sig type_sig = field_type_sig |
	 * base_type
	 * 
	 * method_type_sig = formal_type_params_opt LEFT_BRACE type_sig_list_opt
	 * RIGHT_BRACE return_type throws_sig_list_opt return_type = type_sig | V
	 * throws_sig_list_opt = * empty * | throws_sig_list throws_sig_list =
	 * throws_sig | throws_sig_list throws_sig throws_sig = HAT class_type_sig |
	 * HAT type_var_sig
	 * 
	...
	class MethodSig {
		public MethodSig(List typeVars, List formalTypes, Type returnType,
				List throwTypes) {
			this.typeVars = typeVars;
	...
	public Result classSig(String value, int pos) {
		...
		return new Result(new ClassSig(fres == null ? new ArrayList()
				: (List) fres.result(), (Type) sres.result(), superInterfaces),
				pos);
	}
	...
	public Result formalTypeParamList(String value, int pos) {
		// System.err.println("### Parsing formal type param list " +
		// value.substring(pos));
		typeVars = null;
		int oldpos = pos;
		createTypeVars = true;
	...
	public Result formalTypeParam(String value, int pos) {
		// System.err.println("### Parsing formalTypeParam " +
		// value.substring(pos));
		String id = "";
		char token = value.charAt(pos);
		while (token != COLON) {
		...
		if (createTypeVars) {
			...
			return new Result(ts.typeVariable(position, id,
					ts.intersectionType(position, bounds)), pos);
		} else {
	...
	public Result fieldTypeSig(String value, int pos) {
		// System.err.println("### Parsing field type sig " +
		// value.substring(pos));
		Result res = null;
		char token = value.charAt(pos);
		switch (token) {
		case L: {
			res = classTypeSig(value, pos);
			break;
		}
		case LEFT_SQUARE: {
			res = arrayTypeSig(value, pos);
			break;
		}
		case T: {
			res = typeVarSig(value, pos);
			break;
		}
		case COLON: {
			res = new Result(ts.Object(), pos);
			break;
		}
		}
		return res;
	...
	public Result classTypeSig(String value, int pos) {
		...
		while (token != SEMI_COLON) {
			...
			case LEFT_ANGLE: { // id is a className
								// System.err.println("Parsing type arg list " +
								// value.substring(pos));
		...
		if (classArgsMap.containsKey(ct.name())) {
			...
			if (!createTypeVars) {
				try {
					ct = ts.instantiate(position, pct.pclass(),
							(List<Type>) classArgsMap.get(ct.name()));
				} catch (SemanticException e) {
		...
		while (outer != null) {
			if (classArgsMap.containsKey(outer.name())) {
				...
				// This is dealing with instantiating for nested classes, but
				// it's unclear what it should do.
				// System.err.println("Current is " + current);
				// System.err.println("Outer is " + outer.name() +
				// " and classArgsMap is " + classArgsMap + " outer is " +
				// outer.getClass());
				...
				try {
					ClassType pt = ts.instantiate(position, pct.pclass(),
							(List) classArgsMap.get(outer.name()));
	...
	public Result typeVarSig(String value, int pos) {
		// System.err.println("### Parsing type var sig " +
		// value.substring(pos));
		Result res = null;
		char token = value.charAt(pos);
		switch (token) {
	...
	public Result typeArg(String value, int pos) {
		...
		switch (token) {
		case PLUS: {
			Result fres = fieldTypeSig(value, ++pos);
			res = new Result(ts.wildCardType(position,
					(ReferenceType) fres.result(), null), fres.pos());
			break;
		}
		case MINUS: {
			Result fres = fieldTypeSig(value, ++pos);
			res = new Result(ts.wildCardType(position, null,
	...
	public Result typeSigList(String value, int pos) {
		// System.err.println("### Parsing type sig list " +
		// value.substring(pos));
	...
	public Result methodTypeSig(String value, int pos) {
		// System.err.println("### Parsing method type sig " +
		// value.substring(pos));
		char token = value.charAt(pos);
		Result fres = null;
		if (token == LEFT_ANGLE) {
		...
		return new Result(new MethodSig(fres == null ? new ArrayList()
				: (List) fres.result(), ares == null ? new ArrayList()
				: (List) ares.result(), (Type) rres.result(),
				tres == null ? new ArrayList() : (List) tres.result()), pos);
	}
	...
	public Result returnType(String value, int pos) {
		...
		case S:
		case Z: {
			res = typeSig(value, pos);
			break;
		}
		case V: {
			pos++;
			res = new Result(ts.Void(), pos);
			break;
		}
		}
		return res;
	...
	public Result baseType(String value, int pos) {
		...
		switch (token) {
		case B: {
			res = new Result(ts.Byte(), ++pos);
			break;
		}
		case C: {
			res = new Result(ts.Char(), ++pos);
			break;
		}
		case D: {
			res = new Result(ts.Double(), ++pos);
			break;
		}
		case F: {
			res = new Result(ts.Float(), ++pos);
			break;
		}
		case I: {
			res = new Result(ts.Int(), ++pos);
			break;
		}
		case J: {
			res = new Result(ts.Long(), ++pos);
			break;
		}
		case S: {
			res = new Result(ts.Short(), ++pos);
			break;
		}
		case Z: {
			res = new Result(ts.Boolean(), ++pos);
			break;
		}
	...
	public List<TypeVariable> parseClassTypeVariables(TypeSystem ts,
			Position pos) throws IOException, SemanticException {
		this.ts = (JL5TypeSystem) ts;
		this.position = pos;
		String sigValue = (String) cls.getConstants()[index].value();
		...
		if (token == LEFT_ANGLE) {
			results = (List<TypeVariable>) this.createFormalTypeParamList(
					sigValue, 1).result();
		}
	...
	private TypeVariable findTypeVar(String next) {
		...
		if (curClass != null && curClass instanceof JL5ParsedClassType) {
			...
			if (((JL5ParsedClassType) curClass).typeVariables() != null) {
				...
				for (Iterator it = ((JL5ParsedClassType) curClass)
						.typeVariables().iterator(); it.hasNext();) {
					TypeVariable iType = (TypeVariable) it.next();
					// System.err.println("### Checking against " +
					// iType.name());
					if (iType.name().equals(next))
						return iType;
				}
	...
	/*
	 * public List typeVariables(){ return typeVars; }
	 */
