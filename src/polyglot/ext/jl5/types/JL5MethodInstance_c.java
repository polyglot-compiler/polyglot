...
public class JL5MethodInstance_c extends MethodInstance_c implements
	...
	public List overridesImpl() {
		...
		while (rt != null) {
			...
			// rt
			for (MethodInstance mj : (List<MethodInstance>) rt
					.methodsNamed(name)) {
	...
	public boolean canOverrideImpl(MethodInstance mj, boolean quiet)
		...
		if (!ts.throwsSubset(mi, mj)) {
			if (Report.should_report(Report.types, 3))
				Report.report(3,
						mi.throwTypes() + " not subset of " + mj.throwTypes());
			if (quiet)
				return false;
			throw new SemanticException(mi.signature() + " in "
					+ mi.container() + " cannot override " + mj.signature()
					+ " in " + mj.container() + "; the throw set "
					+ mi.throwTypes() + " is not a subset of the "
		...
		if (mi.flags().moreRestrictiveThan(mj.flags())) {
			if (Report.should_report(Report.types, 3))
				Report.report(3,
						mi.flags() + " more restrictive than " + mj.flags());
			if (quiet)
				return false;
			throw new SemanticException(mi.signature() + " in "
					+ mi.container() + " cannot override " + mj.signature()
	...
	public boolean callValidImpl(List argTypes) {
		...
		if (this.container() instanceof JL5ParsedClassType) {
			...
			// with their bounds.
			erasureSubst = ((JL5ParsedClassType) this.container())
					.erasureSubst();
		}
		...
		while (actualTypes.hasNext()) {
			...
			if (erasureSubst != null
					&& ts.isImplicitCastValid(actual,
							erasureSubst.substType(formal))) {
				return true;
			}
