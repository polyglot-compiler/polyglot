...
public class AutoBoxer extends AscriptionVisitor {
    ...
    public Expr ascribe(Expr e, Type toType) throws SemanticException {
        ...
        if(toType.isPrimitive() && !toType.isVoid() && !fromType.isPrimitive() && !fromType.isSubtype(ts.toRawType(ts.Enum()))) {
        ...
        else if (!toType.isPrimitive() && fromType.isPrimitive() && !fromType.isVoid() && !ts.String().equals(toType)) {
            ...
            ClassType wrapperType = ((JL5TypeSystem)ts).wrapperClassOfPrimitive(fromType.toPrimitive());
