...
public class RemoveExtendedFors extends ContextVisitor {
    ...
    private Node translateExtendedFor(ExtendedFor n, List<String> labels) throws SemanticException {
        ...
        // "{ Iterator iter = e.iterator(); L1,...,Ln: while (iter.hasNext();)  { C x =
        // (C)iter.next(); b }"

        // Create the iter declaration "Iterator iter = e.iterator()"
        String iterName = freshName("iter");
        LocalDecl iterDecl;
        LocalInstance iterLI = ts.localInstance(pos, Flags.NONE, iterType, iterName);
        {
        ...
        List<Stmt> loopBody = new ArrayList<>();
        {
            ...
            Cast cast =
                    nodeFactory()
                            .Cast(pos, nodeFactory().CanonicalTypeNode(pos, iteratedType), call);
    ...
    protected Node translateExtForArray(ExtendedFor n, List<String> labels)
        ...
        Type iteratedType = decl.type().type();
        // translate "L1,...,Ln: for (C x: e) b" to
        // "{ C[] arr = e; int iter = 0;  L1,...,Ln: while (iter < arr.length)  { C x = arr[iter];
        // iter = iter + 1; b; }"
        List<Stmt> stmts = new ArrayList<>();

        // add the declaration of arr: "C[] arr = e"
        String arrID = freshName("arr");
        LocalInstance arrLI = ts.localInstance(pos, Flags.NONE, expr.type(), arrID);
        {
            LocalDecl ld =
                    nodeFactory()
                            .LocalDecl(
                                    pos,
                                    Flags.NONE,
                                    nodeFactory().CanonicalTypeNode(pos, arrLI.type()),
        ...
        LocalInstance iterLI = ts.localInstance(pos, Flags.NONE, ts.Int(), iterID);
        {
            LocalDecl ld =
                    nodeFactory()
                            .LocalDecl(
        ...
        Expr cond;
        {
            ...
            Field field =
                    (Field) nodeFactory().Field(pos, makeLocal(pos, arrLI), id).type(ts.Int());
            field =
                    field.fieldInstance(
                            ts.findField(
                                    arrLI.type().toReference(),
                                    "length",
            ...
            cond =
                    nodeFactory()
                            .Binary(pos, makeLocal(pos, iterLI), Binary.LT, field)
                            .type(ts.Boolean());
        }
        ...
        Expr init;
        {
            init = nodeFactory().ArrayAccess(pos, makeLocal(pos, arrLI), makeLocal(pos, iterLI));
        ...
        Stmt inc;
        {
            ...
            Assign incStore =
                    (Assign)
                            nodeFactory()
                                    .Assign(pos, makeLocal(pos, iterLI), Assign.ASSIGN, incExpr)
                                    .type(ts.Int());
        ...
        // build the while loop
        {
            ...
            Block loopBody = nodeFactory().Block(pos, decl.init(init), inc, n.body());
    ...
    protected Expr makeLocal(Position pos, LocalInstance li) {
        Local l =
                (Local)
                        nodeFactory()
                                .Local(pos, nodeFactory().Id(pos, li.name()))
                                .localInstance(li)
                                .type(li.type());
