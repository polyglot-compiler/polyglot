...
public class RemoveExtendedFors extends ContextVisitor {
	...
	private Node translateExtendedFor(ExtendedFor n, List<String> labels)
		...
		LocalDecl iterDecl;
		LocalInstance iterLI = ts.localInstance(pos, Flags.NONE, iterType,
				iterName.id());
		{
			Call iterator = nodeFactory().Call(pos, n.expr(), "iterator");
			iterator = (Call) iterator.type(iterType);
			iterator = iterator.methodInstance(ts.findMethod(n.expr().type()
					.toClass(), "iterator", Collections.emptyList(), this
					.context().currentClass()));

			iterDecl = nodeFactory().LocalDecl(pos, Flags.NONE,
					nodeFactory().CanonicalTypeNode(pos, iterType), iterName,
					iterator);
		...
		While loop;
		{
			Call cond = nodeFactory()
					.Call(pos,
							((Local) nodeFactory().Local(pos, iterName).type(
									iterType)).localInstance(iterDecl
									.localInstance()), "hasNext");
			cond = (Call) cond.type(ts.Boolean());
			cond = cond.methodInstance(ts.findMethod(iterType.toClass(),
					"hasNext", Collections.EMPTY_LIST, this.context()
							.currentClass()));

			loop = nodeFactory().While(pos, cond,
	...
	private Node translateExtForArray(ExtendedFor n, List<String> labels)
		...
		LocalInstance arrLI = ts.localInstance(pos, Flags.NONE,
				n.expr().type(), arrID.id());
		{
			LocalDecl ld = nodeFactory().LocalDecl(pos, Flags.NONE,
					nodeFactory().CanonicalTypeNode(pos, arrLI.type()), arrID);
		...
		LocalInstance iterLI = ts.localInstance(pos, Flags.NONE, ts.Int(),
				iterID.id());
		{
			LocalDecl ld = nodeFactory()
					.LocalDecl(
							pos,
							Flags.NONE,
							nodeFactory().CanonicalTypeNode(pos, iterLI.type()),
							iterID);
		...
		Expr cond;
		{
			Local iterLocal = (Local) nodeFactory().Local(pos, iterID)
					.localInstance(iterLI).type(ts.Int());
			Local arrLocal = (Local) nodeFactory().Local(pos, arrID)
					.localInstance(arrLI).type(arrLI.type());
			Field field = (Field) nodeFactory().Field(pos, arrLocal, "length")
					.type(ts.Int());
			field = field.fieldInstance(ts.findField(
					arrLI.type().toReference(), "length"));

			cond = nodeFactory().Binary(pos, iterLocal, Binary.LT, field).type(
					ts.Boolean());
		}
		...
		Stmt inc;
		{
			Local iterLocal = (Local) nodeFactory().Local(pos, iterID)
					.localInstance(iterLI).type(ts.Int());
			Expr incExpr = nodeFactory().Binary(pos, iterLocal.type(ts.Int()),
					Binary.ADD,
					nodeFactory().IntLit(pos, IntLit.INT, 1).type(ts.Int()))
					.type(ts.Int());
			Assign incStore = (Assign) nodeFactory().Assign(pos, iterLocal,
					Assign.ASSIGN, incExpr).type(ts.Int());
			inc = nodeFactory().Eval(pos, incStore);
		}

		// build the while loop
		{
			// Create a new loop body from the old body followed by the
			// increment
			Block loopBody = nodeFactory().Block(pos, n.decl().init(init),
					n.body(), inc);
	...
	/**
	 * Label stmt s with labels in the list. If the list contains L1, .., Ln,
	 * then the stmt returned will be "L1:L2:...Ln: s"
	 * 
	 * @param s
	 * @param labels
	 * @return
	...
	private Stmt labelStmt(Stmt s, List<String> labels) {
		for (int i = labels.size() - 1; i >= 0; i--) {
			s = nodeFactory().Labeled(Position.compilerGenerated(),
					labels.get(i), s);
		}
		return s;
	}
