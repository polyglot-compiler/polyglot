...
public class RemoveVarargVisitor extends ErrorHandlingVisitor {
    ...
    private Node rewriteProcedureDecl(ProcedureDecl n) {
        ...
        if (formals.size() > 0) {
            ...
            JL5FormalExt varArgFormalExt = (JL5FormalExt) JL5Ext.ext(varArgFormal);

            if (varArgFormalExt.isVarArg()) {
                Formal newFormal =
                        nf.Formal(
                                varArgFormal.position(),
                                varArgFormal.flags(),
                                varArgFormal.type(),
                                varArgFormal.id());
                newFormal =
                        newFormal
                                .type(varArgFormal.type())
                ...
                if (n instanceof MethodDecl) {
                    ...
                    throw new InternalCompilerError(
                            "Unexepected ProcedureDecl " + n + " of type " + n.getClass());
                }
            }
        }
        return n;
    ...
    private Node rewriteEnumConstantDecl(EnumConstantDecl n) {
        JL5ProcedureInstance pi = (JL5ProcedureInstance) n.constructorInstance();
    ...
    private List<Expr> rewriteProcedureArgs(
            JL5ProcedureInstance pi, List<Expr> args, Position pos) {
        if (pi.isVariableArity()) {
            ...
            List<Expr> standardArgs = new ArrayList<>(args.subList(0, numStandardFormals));

            ArrayInit initValues = nf.ArrayInit(pos, args.subList(numStandardFormals, numArgs));
            initValues = (ArrayInit) initValues.type(varArgArrayType);
            NewArray varArgArray =
                    nf.NewArray(
                            pos,
                            nf.CanonicalTypeNode(
                                    Position.compilerGenerated(), varArgArrayType.base()),
                            1,
                            initValues);
