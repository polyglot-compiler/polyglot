...
public class RemoveEnums extends ContextVisitor {
	...
	/**
	 * If the visitor is in an enum declaration, then this is the type of the
	 * declaration.
	 */
	...
	/**
	 * ClassMembers to add at the closest surrounding class body. An element is
	 * pushed when entering a ClassBody, and popped when exiting a ClassBody.
	 */
		...
		for (ClassMember m : membersToAdd) {
			...
			if (m instanceof MethodDecl) {
				...
				if (md.methodInstance().container() == null) {
					md.methodInstance().setContainer(
							this.context().currentClass());
				}
			}
			n = n.addMember(m);
	...
	/**
	 * Translate an enum declaration to java 1.4 features. Creates a new
	 * ClassDecl.
	 */
	private Node translateEnumDecl(JL5EnumDecl enumDecl)
		...
		ClassDecl classDecl = nf.ClassDecl(
				enumDecl.position(),
	...
	/**
	 * Translate a constructor declared in an enum constructor into a suitable
	 * constructor. Addes two new arguments for the name and ordinal of the enum
	 * constant, and hands those to the super class.
	 */
	private Node tranlsateEnumConstructor(ConstructorDecl n) {
		...
		LocalInstance enumNameLI = ts.localInstance(pos, Flags.NONE,
				ts.String(), enumName.id());
		LocalInstance enumOrdLI = ts.localInstance(pos, Flags.NONE, ts.Int(),
				enumOrdinal.id());
		newFormals.add(nodeFactory().Formal(pos, Flags.NONE,
				nodeFactory().CanonicalTypeNode(pos, ts.String()), enumName)
				.localInstance(enumNameLI));
		newFormals.add(nodeFactory().Formal(pos, Flags.NONE,
				nodeFactory().CanonicalTypeNode(pos, ts.Int()), enumOrdinal)
		...
		newStmts.add(nodeFactory().ConstructorCall(
				pos,
				ConstructorCall.SUPER,
				CollectionUtil.list(
						nodeFactory().Local(pos, enumName).localInstance(
								enumNameLI),
						nodeFactory().Local(pos, enumOrdinal).localInstance(
								enumOrdLI))));
	...
	/**
	 * Adds utility members: a valueOf method, a values method, and a values
	 * field that contains all the enum constants.
	 */
	...
	private ClassBody addValuesField(ClassBody body) {
		...
		for (MemberInstance mi : (Collection<MemberInstance>) enumDeclType
				.toClass().members()) {
			if (mi instanceof EnumInstance) {
				EnumInstance ei = (EnumInstance) mi;
				Field f = nf.Field(pos,
		...
		FieldDecl fd = nf.FieldDecl(pos, Flags.STATIC.Final(),
				nf.CanonicalTypeNode(pos, ts.arrayOf(enumDeclType)),
				nf.Id(pos, "values"), ai);
		FieldInstance fi = ts.fieldInstance(pos, enumDeclType, Flags.NONE,
				ts.arrayOf(enumDeclType), "values");
	...
	private ClassBody addValuesMethod(ClassBody body) {
		...
		Position pos = Position.compilerGenerated();
		Field f = nf.Field(pos, nf.CanonicalTypeNode(pos, enumDeclType),
				nf.Id(pos, "values"));
		Call cl = nf.Call(pos, f, "clone");
		Cast cst = nf.Cast(pos,
				nf.CanonicalTypeNode(pos, ts.arrayOf(enumDeclType)), cl);
		Return ret = nf.Return(pos, cst);
		MethodDecl md = nf.MethodDecl(pos, Flags.PUBLIC.Static(),
				nf.CanonicalTypeNode(pos, ts.arrayOf(enumDeclType)), "values",
				Collections.emptyList(), Collections.emptyList(),
				nf.Block(pos, ret));

		MethodInstance mi = ts.methodInstance(pos, enumDeclType, Flags.NONE,
				ts.arrayOf(enumDeclType), "values", Collections.emptyList(),
	...
	private ClassBody addValueOfMethod(ClassBody body) throws SemanticException {
		// public static T valueOf(String s) { return (T)Enum.valueOf(T.class,
		// s); }
		Position pos = Position.compilerGenerated();
		Local arg = nf.Local(pos, nf.Id(pos, "s"));
		Field clazz = nf.Field(pos, nf.CanonicalTypeNode(pos, enumDeclType),
				nf.Id(pos, "class"));

		LocalInstance argLI = ts.localInstance(pos, Flags.NONE, ts.String(),
				arg.name());
		arg = arg.localInstance(argLI);

		Call cl = nf.Call(pos,
				nf.CanonicalTypeNode(pos, ts.typeForName(this.enumImplClass)),
				nf.Id(pos, "valueOf"), clazz, arg);
		Cast cst = nf.Cast(pos, nf.CanonicalTypeNode(pos, enumDeclType), cl);
		Return ret = nf.Return(pos, cst);

		Formal formal = nf.Formal(pos, Flags.NONE,
				nf.CanonicalTypeNode(pos, ts.String()), nf.Id(pos, "s"))
				.localInstance(argLI);
		MethodDecl md = nf.MethodDecl(pos, Flags.PUBLIC.Static(),
				nf.CanonicalTypeNode(pos, enumDeclType), nf.Id(pos, "valueOf"),
				Collections.singletonList(formal), Collections.emptyList(),
				nf.Block(pos, ret));
		MethodInstance mi = ts
				.methodInstance(pos, enumDeclType, Flags.NONE, enumDeclType,
						"valueOf", Collections.singletonList(ts.String()),
						Collections.emptyList());
		md = md.methodInstance(mi);
	...
	private Node translateEnumConstantDecl(EnumConstantDecl ecd) {
		...
		args.add(nf.IntLit(Position.compilerGenerated(), IntLit.INT,
				ecd.ordinal()));
		args.addAll(ecd.args());
		Expr init = nf.New(Position.compilerGenerated(), nf.CanonicalTypeNode(
				Position.compilerGenerated(), enumDeclType), args, ecd.body());
		init = init.type(enumDeclType);

		FieldDecl fd = nf.FieldDecl(ecd.position(), Flags.FINAL.Public()
				.Static(), nf.CanonicalTypeNode(Position.compilerGenerated(),
				enumDeclType), ecd.name(), init);
	...
	/**
	 * Take a ClassDecl that is the translation of an enum declaration (as
	 * produced by this class) and pretty prints it like a Java 1.5 enum. This
	 * method lets us get around a "feature" of Java 1.5 compilers that refuse
	 * to let classes directly extend java.lang.Enum. (OK, this is actually part
	 * of the Java Language Spec.) So in order to produce code that is
	 * compilable by a Java 1.5 compiler, we need to output the translation of
	 * an enum declaration as an enum declaration.
	 */
	public static void prettyPrintClassDeclAsEnum(ClassDecl decl, CodeWriter w,
		...
		for (ClassMember cm : (List<ClassMember>) decl.body().members()) {
		...
		for (Iterator i = otherMembers.iterator(); i.hasNext();) {
			...
			if (member instanceof ConstructorDecl) {
				prettyPrintConstructorDeclAsEnumConstructorDecl(
						(ConstructorDecl) member, w, tr);
			} else {
	...
	private static void prettyPrintEnumConstFieldDecl(FieldDecl fd,
		...
		if (init instanceof New) {
			...
			List newArgs = new LinkedList(ne.arguments());
			newArgs.remove(0);
			newArgs.remove(0);
			if (!newArgs.isEmpty()) {
				w.write("(");
			...
			throw new InternalCompilerError(
					"Uh oh, don't know how to translate " + fd);
		}
	}
	...
	private static void prettyPrintConstructorDeclAsEnumConstructorDecl(
		...
		List newStmts = new LinkedList(cd.body().statements());
	...
	private Node translateSwitch(Switch n) {
		...
		// The basic idea is given
		// "switch (e) { case GREEN: ...; case RED: ...; }"
		// we translate it to
		// "switch(enum$SwitchMap(e)) { case 0: ...; case 1: ...; }"
		// where enum$SwitchMap is a new method that maps from enum constant
		// fields to some integers (say, their ordinals)
		Position pos = Position.compilerGenerated();
		ClassType enumType = n.expr().type().toClass();
		Id methodName = nodeFactory().Id(pos,
				"enum$SwitchMap$" + enumType.fullName().replace('.', '_'));
		Call methodCall = nodeFactory().Call(pos, methodName, n.expr());
		// XXX missing type information on the methodcall
		n = n.expr(methodCall);

		// the body of the method is something like "private static int
		// enum$SwitchMap(Object e) { if (e == Coin.GREEN) return
		// Coin.GREEN.ordinal(); ... return -1; }
		Id arg = nodeFactory().Id(pos, "e");
		List<Stmt> stmts = new ArrayList();

		LocalInstance argLI = ts.localInstance(pos, Flags.NONE, enumType,
				arg.id());
		...
		for (FieldInstance field : enumConstantFieldInstances(enumType)) {
			...
			Stmt s = qq.parseStmt(
					...
					nodeFactory().Local(pos, arg).localInstance(argLI)
		...
		formals.add(nodeFactory().Formal(pos, Flags.NONE,
				nodeFactory().CanonicalTypeNode(pos, ts.Object()), arg)
				.localInstance(argLI));
		MethodDecl switchMethod = nodeFactory().MethodDecl(pos,
				Flags.PRIVATE.Static(),
				nodeFactory().CanonicalTypeNode(pos, ts.Int()), methodName,
				formals, Collections.emptyList(),
				nodeFactory().Block(pos, stmts));

		ClassType container = this.context().currentClass();
		MethodInstance mi = ts
				.methodInstance(pos, container, Flags.NONE, ts.Int(),
						methodName.id(),
	...
	private int findEnumConstIndex(ClassType enumType, FieldInstance field) {
		List l = enumConstantFieldInstances(enumType);
	...
	private List<FieldInstance> enumConstantFieldInstances(ClassType enumType) {
		...
		for (FieldInstance f : (List<FieldInstance>) enumType.fields()) {
	...
	private Node translateCase(Case n) {
		...
		n = n.expr(nodeFactory().IntLit(Position.compilerGenerated(),
				IntLit.INT, n.value()));
		return n;
	}
}
