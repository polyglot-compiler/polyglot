...
public class SimplifyExpressionsForBoxing extends HaltingVisitor {
        ...
        if (Unary.PRE_DEC.equals(u.operator())
            ...
            if (!isTargetPure(u.expr())) {
                throw new InternalCompilerError(
                        "Don't support effectful LHS " + u + " " + u.expr().getClass());
            }
            Expr left = u.expr();
            ...
            Assign ass =
                    nf.Assign(
                            ...
                            left,
                            newOp,
                            nf.IntLit(u.position(), IntLit.INT, 1).type(ts.Int()));
            ass = (Assign) ass.type(left.type());
            return simplifyAssignment(ass);
        } else if (Unary.POST_DEC.equals(u.operator()) || Unary.POST_INC.equals(u.operator())) {
            if (!isTargetPure(u.expr())) {
                throw new InternalCompilerError(
                        "Don't support effectful LHS " + u + " " + u.expr().getClass());
            }
            ...
            Assign.Operator newOp =
                    Unary.POST_INC.equals(u.operator()) ? Assign.ADD_ASSIGN : Assign.SUB_ASSIGN;
            Assign ass =
                    nf.Assign(
                            u.position(),
                            left,
            ...
            Binary b =
                    nf.Binary(
                            u.position(),
                            sa,
                            Unary.POST_INC.equals(u.operator()) ? Binary.SUB : Binary.ADD,
                            nf.IntLit(u.position(), IntLit.INT, 1).type(ts.Int()));

            // JLS 3rd ed: 15.14.3:  The type of the postfix decrement expression is the type of the
            // variable.
    ...
    protected Expr simplifyAssignment(Assign ass) {
        ...
        Binary b =
                nf.Binary(ass.position(), (Expr) ass.left().visit(new DeepCopy(lang())), op, right);
    ...
    private boolean isTargetPure(Expr target) {
        ...
        if (target instanceof Field) return isTargetPure(((Field) target).target());
        if (target instanceof Cast) return isTargetPure(((Cast) target).expr());
