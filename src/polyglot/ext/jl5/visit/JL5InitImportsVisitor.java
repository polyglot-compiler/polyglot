...
public class JL5InitImportsVisitor extends InitImportsVisitor
{
    ...
    private void checkForConflicts(String id, List<String> imports, Position position) throws SemanticException {
        for (String other : imports) {
            String name = StringUtil.getShortNameComponent(other);
            if (id.equals(name)){
                throw new SemanticException("The import statement " + this + " collides with import statement " + other + " .", position);
            }
        }
