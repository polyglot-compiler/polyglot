        ...
        if (ra != null) {
            for (Type at : ra.annotationTypes()) {
                if (at.equals(typeSystem().TargetAnnotation())) {
                    ...
                    checkTargetMetaAnnotation(
                            (AnnotationElementValueArray) ra.singleElement(at), annotation, decl);
                }
            }
        }
    }
    ...
    protected void checkOverrideAnnotation(Declaration decl) throws SemanticException {
        if (!(decl instanceof MethodInstance)) {
            throw new SemanticException(
                    "An override annotation can apply only to methods.", decl.position());
        }
        ...
        if (overrides.isEmpty()) {
            throw new SemanticException(
                    "Method " + mi.signature() + " does not override a method.", decl.position());
        }
    }

    protected void checkTargetMetaAnnotation(
            AnnotationElementValueArray targetKinds, AnnotationElem n, Declaration decl)
            throws SemanticException {
        AnnotationElem annotation = n;
        Collection<EnumInstance> eis = annotationElementTypesForDeclaration(decl);
        // the array targs must contain at least one of the eis.
        boolean foundAppropriateTarget = false;
        requiredCheck:
        for (EnumInstance required : eis) {
        ...
        if (!foundAppropriateTarget) {
            throw new SemanticException(
                    "Annotation " + annotation + " not applicable to this kind of declaration.",
                    n.position());
        }
    }
