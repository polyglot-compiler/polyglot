...
public class JL5Options extends Options {
	...
	protected int parseCommand(String args[], int index, Set source)
		...
		if (args[index].equals("-enumImplClass")
				|| args[index].equals("--enumImplClass")) {
			index++;
			this.enumImplClass = args[index++];
			return index;
		}

		if (args[index].equals("-removeJava5isms")
				|| args[index].equals("--removeJava5isms")) {
			index++;
	...
	public void usage(PrintStream out) {
		super.usage(out);
		usageForFlag(out, "-removeJava5isms",
				"Translate Java 5 language features to Java 1.4 features");
		usageForFlag(
				out,
				"-enumImplClass",
				"Runtime class to implement Enums (if not specified, translate to java.lang.Enum)");
	}
}
