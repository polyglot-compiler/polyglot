...
/**
 * A TypedListIterator is an ListIterator which will not allow members not
 * belonging to a given type to be added to a collection. Optionally, it may
 * also present an immutable view.
 * 
 * If an attempt is made to change an immutable listiterator, or if an attempt
 * is made to insert an improperly-typed element, an
 * UnsupportedOperationException is thrown.
 * 
 * This class is given so that we can present a ListIterator for a given given
 * class without worrying about outsiders breaking the rep.
 * 
 * This is a poor substitute for PolyJ.
 **/
public class TypedListIterator implements ListIterator {
	/**
	 * Requires: <iter> not null Creates a new TypedIterator around <iter> which
	...
	public boolean hasPrevious() {
		return backing_iterator.hasPrevious();
	}
	...
	public int nextIndex() {
		return backing_iterator.nextIndex();
	}

	public Object previous() {
		return backing_iterator.previous();
	}

	public int previousIndex() {
		return backing_iterator.previousIndex();
	}
