...
public class StdErrorQueue extends AbstractErrorQueue {
	...
	public void displayError(ErrorInfo e) {
		String message = e.getErrorKind() != ErrorInfo.DEBUG ? e.getMessage()
		 ...
		 * 
		 * w.begin(0); w.write(prefix + ":"); w.write(" ");
		 * 
		 * StringTokenizer st = new StringTokenizer(message, " ");
		 * 
		 * while (st.hasMoreTokens()) { String s = st.nextToken(); w.write(s);
		 * if (st.hasMoreTokens()) { w.allowBreak(0, " "); } }
		 * 
		 * w.end(); w.newline(0);
		 * 
	...
	protected void showErrorIndicator(Position pos, int lineNum, String s) {
		...
		int startIndAt = i; // column position to start showing the indicator
							// marks
		int stopIndAt = s.length() - 1; // column position to stop showing the
										// indicator marks
		if (pos.line() == lineNum) {
