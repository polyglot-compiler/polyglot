 ...
 * breaks of equal or lower level in all containing blocks must also be broken,
 * and breaks in the same block must also be broken if they are of strictly
 * lower level or if they are of the same level but marked as "unified".
 */
public class OptimalCodeWriter extends CodeWriter {
	/**
	 * Create a OptimalCodeWriter object with output stream <code>o</code> and
	 * width <code>width_</code>.
	 * 
	 * @param o
	 *            the writer to write to. Must be non-null.
	 * @param width_
	 *            the formatting width. Must be positive.
	 */
	...
	/**
	 * Like <code>flush</code>, but passing <code>format=false</code> causes
	 * output to be generated in the fastest way possible, with all breaks
	 * broken.
	 * 
	 * @param format
	...
	public boolean flush(boolean format) throws IOException {
		...
		if (format) {
			try {
				top = input;
				Item.format(input, 0, 0, width, width, new MaxLevels(
 ...
 * margin was exceeded by at least <code>amount</code> chars. If sameLine, the
 * overrun occurred on the first line of the requested formatting; otherwise, it
 * occurred on a subsequent line.
 */
...
/**
 * An <code>Item</code> is a piece of input handed to the formatter. It contains
 * a reference to a possibly empty list of items that follow it.
 */
abstract class Item {
	 ...
	 *         both), and set any contained breaks accordingly. </p>
	 * 
	 * @param lmargin
	 ...
	 * 
	 *            <p>
	 *            Breaks may be broken up to level maxLevel, which is set
	 *            whenever a break is not broken. Not breaking an ordinary break
	 *            means that equal or higher-level breaks in all contained
	 *            blocks must not be broken either, and breaks of strictly
	 *            higher level in the same block must not be broken. Not
	 *            breaking a unified break means that breaks of the same level
	 *            in the same block must not also be broken. The parameter
	 *            maxLevelInner controls the maxLevel in nested blocks; it is
	 *            equal to either maxLevel or maxLevel-1.
	 * 
	 *            <p>
	 *            <dl>
	 *            <dt>Example 1:
	 *            <dd>Suppose we have a current maxLevel of 4, and an ordinary,
	 *            non-unified break of level 2 is not broken. Then for that
	 *            block, maxLevel is set to 2 and maxLevelInner is set to 1.
	 *            This permits further breaks of level 1 or 2 in the same block,
	 *            but only level-1 breaks in inner blocks.
	 * 
	 *            <dt>Example 2:</dt>
	 *            <dd>Suppose we have a current maxLevel of 4, and a unified
	 *            break of level 2 is not broken. Then for that block, maxLevel
	 *            and maxLevelInner are set to 1. This permits no breaks in this
	 *            block or in any nested blocks.</dd>
	 *            </dl>
	 * 
	 *            <p>
	 *            When a break is broken in a nested block, it means that all
	 *            equal or higher-level breaks in containing blocks must be
	 *            broken. However, these breaks may be encountered after the
	 *            nested block. The parameter <code>minLevel</code> is used to
	 *            communicate the level of breaks broken in nested blocks (and
	 *            earlier in the current block). Any break of level <= minLevel
	 *            <em>must</em> be broken. The parameter <code>
	 * minLevelUnified</code> is the minimum level at which unified breaks must
	 *            be broken. minLevelUnified is equal to either minLevel or
	 *            minLevel+1.
	 *            </p>
	 * 
	 *            <dl>
	 *            <dt>Example 3:
	 *            <dd>Suppose we have a current maxLevel of 4, and a break of
	 *            level 2 is broken. Then for its block, minLevel is at least 1,
	 *            and minLevelUnified is at least 2. For containing blocks,
	 *            minLevel is at least 2.</dd>
	 *            </dl>
	 * 
	 *            <b>Note: </b> It is important that formatN not necessarily
	 *            convert overruns in its final position into exceptions. This
	 *            allows the calling routine to distinguish between 'internal'
	 *            overruns and ones that it can tack on a conservative estimate
	 *            of how much formatting the rest of the list will make the
	 *            overrun go up by. Also, it simplifies the coding of formatN.
	 *            </p>
	 * 
	 *            Requires: rmargin &lt; lmargin, pos &lt;= rmargin, lmargin
	 ...
	 * signifying an empty list. Requires: lmargin &lt; rmargin, pos &le;
	...
	static FormatResult format(Item it, int lmargin, int pos, int rmargin,
		...
		if (OptimalCodeWriter.debug) {
			if (it != null && it != OptimalCodeWriter.top) {
				...
				PrintWriter w = new PrintWriter(new OutputStreamWriter(
						System.err));
				try {
					OptimalCodeWriter.top
							.sendOutput(w, 0, 0, rmargin, true, it);
				} catch (IOException e) {
				}
				w.write("<END>\n");
				w.flush();
		...
		if (it == null) { // no items to format. Check against final position.
			if (pos > fin) {
				if (OptimalCodeWriter.debug)
					System.err
							.println("Final position overrun: " + (pos - fin));
		...
		int amount = pos + getMinPosWidth(it, m) - rmargin; // overrun on first
															// line
		if (amount > 0) {
			if (OptimalCodeWriter.debug)
		...
		int amount3 = lmargin + getMinIndent(it, m) - fin; // overrun on last
															// line
		if (amount3 > 0) {
			if (OptimalCodeWriter.debug)
	 ...
	 * possible with an item and its following items, if all breaks are broken.
	 * The purpose is to more aggressively tighten bounds when an overrun
	 * occurs. Formatting is measured relative to both "lmargin" and to "pos". T
	 * 
	 * lmargin pos | | | xxxxx xxxxxxxx xxxxxx <------> min_width (at least
	 * min_pos_width): distance from lmargin to rightmost char <--->
	 * min_pos_width: distance from initial pos to end of first line <---->
	 * min_indent (at most min_width): distance from lmargin to final position
	 * on last line
	 */
	...
	/**
	 * Minimum lmargin-rhs width on second and following lines. A map from max
	 * levels to Integer(width).
	 */
		...
		if (canLeaveUnbroken(minLevel, minLevelUnified)) {
			...
			try {
				...
				return format(
						...
						rmargin,
						fin,
						new MaxLevels(Math.min(unified ? level - 1 : level,
								m.maxLevel), Math.min(level - 1,
								m.maxLevelInner)), minLevel, minLevelUnified);
			...
			// width overrun: might help by breaking (e.g., if breaking permits
			// nested breaks)
			// fin overrun: similar
			catch (Overrun o) {
				if (OptimalCodeWriter.debug) {
...
/**
 * A Newline is simply a level-1 break that cannot be left unbroken.
 */
...
class BlockItem extends Item {
	...
	FormatResult formatN(int lmargin, int pos, int rmargin, int fin,
		...
		while (true) {
			FormatResult fr = format(first, pos + indent, pos, rmargin,
					childfin, new MaxLevels(m.maxLevelInner, m.maxLevelInner),
					0, 0);
			int minLevel2 = Math.max(minLevel, fr.minLevel);
			int minLevelU2 = Math.max(minLevelUnified, fr.minLevel);
			try {
				return format(next, lmargin, fr.pos, rmargin, fin, m,
						minLevel2, minLevelU2);
			} catch (Overrun o) {
	...
	boolean selfContainsBreaks(MaxLevels m) {
		...
		boolean result = containsBreaks(first, new MaxLevels(m.maxLevelInner,
				m.maxLevelInner));
