 ...
 * type to be added to a collection. Optionally, it may also present an
 * immutable view.
 * 
 * If an attempt is made to change an immutable list, or if an attempt is made
 * to insert an improperly-typed element, an UnsupportedOperationException is
 * thrown.
 * 
 * This class is given so that we can present a List for a given class without
 * worrying about outsiders breaking the rep.
 * 
...
public class TypedList implements List, java.io.Serializable, Cloneable {
	...
	/**
	 * Requires: <list> not null, and every element of <list> may be cast to
	 * class <c>. Creates a new TypedList, containing all the elements of
	 * <list>, which restricts all members to belong to class <c>. If <c> is
	...
	/**
	 * Creates a new TypedList, containing all the elements of <list>, which
	 * restricts all members to belong to class <c>. If <c> is null, no typing
	 * restriction is made. If <immutable> is true, no modifications are
	 * allowed.
	 * 
	 * Throws an UnsupportedOperationException if any member of <list> may not
	 * be cast to class <c>.
	 **/
	...
	/**
	 * Throws an UnsupportedOperationException if any member of <list> may not
	 * be cast to class <c>. Otherwise does nothing.
	 **/
	public static void check(List list, Class c) {
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (o != null && !c.isAssignableFrom(o.getClass())) {
				throw new UnsupportedOperationException("Tried to add a "
						+ o.getClass().getName() + " to a list of type "
						+ c.getName());
			}
		}
	}
	...
	/**
	 * Requires: <list> not null, and every element of <list> may be cast to
	 * class <c>. Effects: Creates a new TypedList around <list> which restricts
	 * all members to belong to class <c>. If <c> is null, no typing restriction
	 * is made. If <immutable> is true, no modifications are allowed.
	 **/
	...
	public List subList(int from, int to) {
		return new TypedList(backing_list.subList(from, to), allowed_type,
				immutable);
	}
