...
public class CodeWriter
{
	...
	boolean success = true;
        try {
            input.format1(0, 0, width, width, true, true);
        } catch (Overrun e) {
	    success = false;
        }
...
abstract class Item 
{
    ...
    /** 
     * Try to format this item. The current cursor position is
     * <code>pos</code>, left and right margins are as
     * specified. Returns the final position, which must be
     * <code>&lt; fin</code>. If breaks may be broken,
     * <code>can_break</code> is set. Return the new cursor position
     * and set any contained breaks appropriately if formatting was
     * successful. If <code>nofail</code>, formatting is forced to 
     * "succeed" (best effort) even if it doesn't; <code>nofail</code>
     * requires <code>can_break</code>.
     *
     * Requires: rmargin &lt; lmargin, pos &lt;= rmargin.
     */
    abstract int format1(int lmargin, int pos, int rmargin, int fin,
    ...
    static int formatList(Item it, int lmargin, int pos, int rmargin, int fin,
	  	          boolean can_break, boolean nofail) throws Overrun {
	if (CodeWriter.debug)
	    System.err.println("Formatting list " + it +
		"\n  lmargin = "  + lmargin + " pos = " + pos +
		" fin = " + fin +
		(can_break ? " can break" : " no breaks") +
                (nofail ? " [no fail]" : ""));
	int this_fin = rmargin;
	boolean thisnofail = false;
	if (it.next == null) this_fin = fin;
	    // this_fin is final position bound for current item. We crank
	    // it in from the right margin as subsequent items overrun.
	while (true) {
	    int nextpos;
	    try {
		nextpos = it.format1(lmargin, pos, rmargin, this_fin,
			             can_break, thisnofail);
	    } catch (Overrun o) {
		if (!nofail) throw o;
		if (CodeWriter.debug && thisnofail) {
		    System.err.println("Failed with nofail!");
		    throw o;
		}
		thisnofail = true;
		continue;
	    ...
	    if (it.next == null) return nextpos;
	    try {
		return formatList(it.next, lmargin, nextpos, rmargin, fin,
				  can_break, nofail);
	    } catch (Overrun o) {
		...
		if (nextpos > this_fin) {
		    nextpos = this_fin;
		    if (!nofail) {
			System.err.println("Formatting failed silently!");
		    }
		    thisnofail = true;
		}
		this_fin = nextpos - o.amount;
		if (this_fin < lmargin) throw o;
		if (CodeWriter.debug)
		    System.err.println("  Trying list " +
			it.next + " again with fin = " + this_fin);
		// try again!
	    }
	}
    }
	...
	if (next == null) return selfToString();
	return selfToString() + "," + next.toString();
    }
...
class Block extends Item {
    ...
    int format1(int lmargin, int pos, int rmargin, int fin, boolean can_break,
		boolean nofail)
        throws Overrun
    {
	if (CodeWriter.debug)
        ...
        try {
	    return formatList(first, pos + indent, pos, rmargin, fin, false, false);
        } catch (Overrun overrun) {
            if (!can_break) throw overrun;
	    try {
		return formatList(first, pos + indent, pos, rmargin, fin, true,
				  false);
	    } catch (Overrun o) {
		if (!nofail) throw o;
		return formatList(first, pos + indent, pos, rmargin, fin, true,
...
class StringItem extends Item {
    ...
    int format1(int lmargin, int pos, int rmargin, int fin, boolean can_break,
        ...
        throws Overrun {
        pos += s.length();
        if (pos > fin && !nofail) throw new Overrun(pos - fin);
        return pos;
    }
