 ...
 * possible. Newlines occur in the output only at places where a line break is
 * permitted by the use of <code>allowBreak</code> and <code>unifiedBreak</code>
 * .
 * 
...
public abstract class CodeWriter {
	...
	/**
	 * Print the string <code>s</code> on the output stream. Pretend that it has
	 * width <code>length</code> even if it has a different number of
	 ...
	 * <code>allowBreak</code>, but unified breaks should also break if any
	 * break of the same level in the same block is broken, whereas ordinary
	 * breaks do not necessarily break in this case. That is, unified breaks act
	 * as if they were slightly lower level than other breaks of the same level
	 ...
	 * @param altlen
	 *            the length in characters that 'alt' will be treated as taking
	 *            up.
	 ...
	 * used sparingly; a call to allowBreak() gives the pretty-printer much more
	 * flexibility to do a good job.
	 */
	public void newline() {
		newline(0, 1);
	}
	...
	/**
	 * Flush all formatted text to the underlying writer. Returns true if
	 * formatting was completely successful (the margins were obeyed).
	 */
	public abstract boolean flush() throws IOException;

	/**
	 * Flush all formatted text, reset formatter state, and close the underlying
	 * writer.
	 */
