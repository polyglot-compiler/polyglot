...
/**
 * A NestedMap is a map which, when it cannot find an element in itself, defers
 * to another map. Modifications, however, are not passed on to the supermap.
 * 
 * A NestedMap and its backing collections and iterators support all operations
 * except 'remove' and 'clear', since operations to a NestedMap must not affect
 * the backing map. Instead, use the 'release' method.
 * 
...
public class NestedMap extends AbstractMap implements Map {
	/**
	 * Creates a new nested map, which defers to <containing>. If containing is
	 * null, it defaults to a NilMap.
	 **/
