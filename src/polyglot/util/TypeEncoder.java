 ...
 * <p>
 * It uses a form of serialization to encode the <code>Type</code> into a byte
 * stream and then converts the byte stream to a standard Java string.
 * <p>
 * The difference between the encoder and a normal serialization process is that
 * in order to encode this type, we need to sever any links to other types in
 * the current environment. So any <code>ClassType</code> other than the the
 * type being encoded is replaced in the stream with a <code>PlaceHolder</code>
 * that contains the name of the class. To aid in the decoding process,
 * placeholders for member classes user their "mangled" name; non-member classes
...
public class TypeEncoder {
	...
	public String encode(TypeObject t) throws IOException {
		...
		// System.out.println(t + " " + t.isCanonical() + " " + t.position() +
		// " " + t.typeSystem() + " " + t.getClass().getName());
		// Thread.dumpStack();
		oos.writeObject(t);
		oos.flush();
		oos.close();
		...
		try {
			if (zip && !base64) {
				...
				ois = new TypeInputStream(new GZIPInputStream(
						new ByteArrayInputStream(b)), ts, placeHolderCache);
			} else {
