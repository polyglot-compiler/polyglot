 ...
 * initialize the members of a class loaded from a class file. Passes may be
 * rerun if a goal is not reached. Goals are processed via a worklist. A goal
 * may have <i>prerequisite</i> dependencies and <i>corequisite</i>
 * dependencies. All prerequisites must be reached before the goal is attempted.
 * A corequisite may be reached while satisfying the goal itself, or vice versa.
 * 
 * Recursive passes are not allowed. If a goal cannot be reached a
 * SchedulerException (or more usually the subclass MissingDependencyException)
 * is thrown. The scheduler catches the exception and adds the goal back onto
 * the worklist and adds the missing dependency, if any, to the dependency
 * graph. Optionally, a pass may catch the exception, but it must mark the goal
 * as unreachable on this run so that it will be added back to the worklist; the
 * pass must also add any missing dependencies.
 * 
 * @author nystrom
 */
public abstract class Scheduler {
	...
	/**
	 * Add a new <code>subgoal</code> of <code>goal</code>. <code>subgoal</code>
	 * must be completed before <code>goal</code> is attempted.
	 * 
	...
	public void addPrerequisiteDependencyChain(List deps)
			throws CyclicDependencyException {
		Goal prev = null;
	 ...
	 * when checking if a goal was reached. Pass schedulerPass(Goal g) { return
	 * new EmptyPass(g); }
	 */
	...
	/**
	 * Run a pass until the <code>goal</code> is attempted. Callers should check
	 * goal.completed() and should be able to handle the goal not being reached.
	 * 
	...
	protected boolean attemptGoal(Goal goal, Set above) {
		...
		while (progress && !reached(goal)) {
			...
			for (Iterator j = goal.prerequisiteGoals(this).iterator(); j
					.hasNext();) {
				Goal subgoal = (Goal) j.next();
				if (!reached(subgoal)) {
					runPass = false;
	...
	protected boolean runPass(Pass pass) {
		...
		if (count >= MAX_RUN_COUNT) {
			...
			String message = "Possible infinite loop detected trying to run a pass for "
		...
		if (job == null || job.status()) {
			...
			try {
				...
				if (!result) {
					...
					extInfo.getStats().accumPassTimes("total goal failures", 1,
							1);
					...
					if (goal.state() == Goal.RUNNING) {
						extInfo.getStats().accumPassTimes(key + " reached", 1,
								1);
						...
						extInfo.getStats().accumPassTimes(key + " unreached",
								1, 1);
						extInfo.getStats().accumPassTimes(
								"total goal unreached", 1, 1);
		...
		if (Report.should_report(Report.time, 2)) {
			Report.report(2,
					"Finished " + pass + " status=" + statusString(result)
							+ " inclusive_time=" + pass.inclusiveTime()
							+ " exclusive_time=" + pass.exclusiveTime());
	...
	protected void dumpDependenceGraph() {
		...
		for (Iterator i = new ArrayList(goals.keySet()).iterator(); i.hasNext();) {
			...
			for (Iterator j = new ArrayList(g.prerequisiteGoals(this))
					.iterator(); j.hasNext();) {
				Goal g2 = (Goal) j.next();
				g2 = internGoal(g2);
			...
			for (Iterator j = new ArrayList(g.corequisiteGoals(this))
					.iterator(); j.hasNext();) {
				Goal g2 = (Goal) j.next();
				g2 = internGoal(g2);
				int h2 = System.identityHashCode(g2);
	...
	protected void dumpDependenceGraph(Goal g) {
		...
		// dump out the successors.
		for (Iterator j = new ArrayList(g.prerequisiteGoals(this)).iterator(); j
				.hasNext();) {
			Goal g2 = (Goal) j.next();
			g2 = internGoal(g2);
			int h2 = System.identityHashCode(g2);
			if (!seen.contains(new Integer(h2))) {
				...
				Report.report(2,
						h2 + " [ label = \"" + StringUtil.escape(g2.toString())
								+ "\" ];");
			}
			Report.report(2, h2 + " -> " + h + " [style=bold]");
		}

		for (Iterator j = new ArrayList(g.corequisiteGoals(this)).iterator(); j
				.hasNext();) {
			Goal g2 = (Goal) j.next();
			g2 = internGoal(g2);
			int h2 = System.identityHashCode(g2);
