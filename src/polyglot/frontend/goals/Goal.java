 ...
 * <code>Pass</code> is run. It keeps trying until it is successful or it fails
 * and the compiler quits with an error.
 * 
 * @author nystrom
 */
public interface Goal {
	...
	/**
	 * Add a new corequisite subgoal <code>g</code>. <code>g</code> is a goal on
	 * which this goal mutually depends. The caller must be careful to ensure
	 * that all corequisite goals can be eventually reached. <code>g</code>
	 * should be interned.
	 */
	public void addCorequisiteGoal(Goal g, Scheduler scheduler);

	/**
	 * Add a new subgoal <code>g</code>. <code>g</code> must be completed before
	 * this goal is attempted. <code>g</code> should be interned.
	 * 
