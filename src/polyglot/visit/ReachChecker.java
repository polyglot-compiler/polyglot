...
public class ReachChecker extends DataFlow {
	public ReachChecker(Job job, TypeSystem ts, NodeFactory nf) {
		super(job, ts, nf, true /* forward analysis */, true /*
															 * perform dataflow
															 * on entry to
															 * CodeDecls
															 */);
	...
	protected static class DataFlowItem extends Item {
		...
		public static final DataFlowItem REACHABLE_EX_ONLY = new DataFlowItem(
				true, false);

		// terms that are not reachable
		public static final DataFlowItem NOT_REACHABLE = new DataFlowItem(
				false, false);
	...
	public Map flow(Item in, FlowGraph graph, Term n, boolean entry,
			Set succEdgeKeys) {
	...
	public Item confluence(List inItems, List itemKeys, Term node,
		...
		if (g != null) {
			...
			if (peers != null && !peers.isEmpty()) {
				...
				for (Iterator iter = peers.iterator(); iter.hasNext();) {
					...
					// is reachable. This would cover all cases, except that
					// some
					// peers may have no successors (e.g. peers that throw an
					// an exception that is not caught by the method). So we
					// need
					...
					if (p.outItems != null) {
						for (Iterator k = p.outItems.values().iterator(); k
								.hasNext();) {
				...
				if (!isNormalReachable && n instanceof Initializer) {
					throw new SemanticException(
							"Initializers must be able to complete normally.",
							n.position());
				}
