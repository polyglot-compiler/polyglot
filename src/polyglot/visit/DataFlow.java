...
public abstract class DataFlow extends ErrorHandlingVisitor {
	...
	/**
	 * Indicates whether the dataflow should detect back edges. Detecting back
	 * edges allows clients to use a widening operator just on confluences from
	 * back edges, permitting analyses to use data flow lattices of infinite
	 * height while guaranteeing termination.
	 */
	protected final boolean detectBackEdges;

	/**
	 * Indicates whether the dataflow should be performed on entering a
	 * <code>CodeNode</code>, or on leaving a <code>CodeNode</code>. If dataflow
	 * is performed on entry, then the control flow graph will be available when
	 * visiting children of the <code>CodeNode</code>, via the
	 * <code>currentFlowGraph</code> method. If dataflow is performed on
	 * leaving, then the control flow graph will not be available, but nested
	 ...
	 * children of the CodeNode, if subclasses want to use this information to
	 * update AST nodes. The stack is maintained only if
	 ...
	 */
	public DataFlow(Job job, TypeSystem ts, NodeFactory nf, boolean forward,
			boolean dataflowOnEntry, boolean detectBackEdges) {
		super(job, ts, nf);
	...
	/**
	 * Produce new <code>Item</code>s as appropriate for the <code>Term n</code>
	 * and the input <code>Item in</code>.
	 * 
	 * @param in
	 *            the Item flowing into the node. Note that if the Term n has
	 *            many flows going into it, the Item in may be the result of a
	 *            call to confluence(List, List, Term).
	 * @param graph
	 *            the FlowGraph which the dataflow is operating on
	 * @param n
	 *            the Term which this method must calculate the flow for.
	 * @param entry
	 *            indicates whether we are looking at the entry or exit of n.
	 * @param edgeKeys
	 *            a set of FlowGraph.EdgeKeys, being all the EdgeKeys of the
	 *            edges leaving this node. The returned Map must have mappings
	 *            for all objects in this set.
	 * @return a Map from FlowGraph.EdgeKeys to Items. The map must have entries
	 *         for all EdgeKeys in edgeKeys.
	 */
	...
	/**
	 * Produce new <code>Item</code>s as appropriate for the <code>Peer</code>
	 * and the input <code>Item</code>s.
	 * 
	 * @param inItems
	 *            all the Items flowing into the node.
	 * @param inItemKeys
	 *            the FlowGraph.EdgeKeys for the items in the list inItems
	 * @param graph
	 *            the FlowGraph which the dataflow is operating on
	 * @param peer
	 *            the Peer which this method must calculate the flow for.
	 * @return a Map from FlowGraph.EdgeKeys to Items. The map must have entries
	 *         for all EdgeKeys in edgeKeys.
	 */
	protected Map flow(List inItems, List inItemKeys, FlowGraph graph, Peer peer) {
		return this.flow(inItems, inItemKeys, graph, peer.node, peer.isEntry(),
				peer.succEdgeKeys());
	}

	/**
	 * Produce new <code>Item</code>s as appropriate for the <code>Peer</code>
	 * and the input <code>Item</code>s. Subclasses should override if flow
	 * behavior needs to distinguish between source peers.
	 * 
	 * @param inItems
	 *            all the Items flowing into the node.
	 * @param inItemKeys
	 *            the FlowGraph.EdgeKeys for the items in the list inItems
	 * @param inItemPeers
	 *            the Peers from which the Items flowed into the node
	 * @param graph
	 *            the FlowGraph which the dataflow is operating on
	 * @param peer
	 *            the Peer which this method must calculate the flow for.
	...
	protected Map flow(List inItems, List inItemKeys, List inItemPeers,
		...
		if (this.detectBackEdges) {
			// determine for each inItemPeer whether it represents a back edge
			// or not,
			// and record the results in a list.
			// Since there is an edge from inItemPeer to the current peer p, the
			// edge
			// is a back edge if the post-order number of inItemPeer is less
			// than
			...
			while (inPeers.hasNext()) {
				...
				int inPeerOrder = ((Integer) postordering.get(inPeer))
						.intValue();
				isBackEdges
						.add(Boolean.valueOf(inPeerOrder < currentPeerOrder));
			}
	 ...
	 *            the Peers from which the Items flowed into the node
	 * @param isBackEdge
	 *            list of Booleans indicating whether the edge represented by
	 *            the corresponding element in inItems is a back edge or not.
	 * @param graph
	 *            the FlowGraph which the dataflow is operating on
	 * @param peer
	 *            the Peer which this method must calculate the flow for.
	 ...
	 */
	protected Map flow(List inItems, List inItemKeys, List inItemPeers,
			List isBackEdges, FlowGraph graph, Peer p) {
		throw new InternalCompilerError(
				"Unimplemented. Should be implemented if a subclass wants to detect back edges."
	...
	/**
	 * Produce new <code>Item</code>s as appropriate for the <code>Term n</code>
	 * and the input <code>Item</code>s. The default implementation of this
	 * method is simply to call <code>confluence</code> for the list of inItems,
	 * and pass the result to flow(Item, FlowGraph, Term, Set). Subclasses may
	 * want to override this method if a finer-grained dataflow is required.
	 * Some subclasses may wish to override this method to call
	 * <code>flowToBooleanFlow</code>.
	 * 
	 * @param inItems
	 *            all the Items flowing into the node.
	 * @param inItemKeys
	...
	/**
	 * A utility method that simply collects together all the TRUE items, FALSE
	 * items, and all other items (including ExceptionEdgeKey items), calls
	 * <code>confluence</code> on each of these three collections as necessary,
	 * and passes the results to flow(Item, Item, Item, FlowGraph, Term, Set).
	 * It is expected that this method will typically be called by subclasses
	 * overriding the flow(List, List, FlowGraph, Term, Set) method, due to the
	 * need for a finer-grained dataflow analysis.
	 * 
	 * @param inItems
	 *            all the Items flowing into the node.
	 * @param inItemKeys
	...
	protected final Map flowToBooleanFlow(List inItems, List inItemKeys,
		...
		Item trueItem = trueItems.isEmpty() ? null : this.safeConfluence(
				trueItems, trueItemKeys, n, entry, graph);
		Item falseItem = falseItems.isEmpty() ? null : this.safeConfluence(
				falseItems, falseItemKeys, n, entry, graph);
		Item otherItem = otherItems.isEmpty() ? null : this.safeConfluence(
				otherItems, otherItemKeys, n, entry, graph);

		return this.flow(trueItem, falseItem, otherItem, graph, n, entry,
				edgeKeys);
	}
	 ...
	 * 
	 * @param trueItem
	 *            The item for flows coming into n for true conditions. Cannot
	 *            be null.
	 * @param falseItem
	 *            The item for flows coming into n for false conditions. Cannot
	 *            be null.
	 * @param otherItem
	 *            The item for all other flows coming into n
	 * @param n
	 ...
	 */
	protected Map flowBooleanConditions(Item trueItem, Item falseItem,
			Item otherItem, FlowGraph graph, Expr n, Set edgeKeys) {
		if (!n.type().isBoolean()
		...
		if (n instanceof Unary) {
			...
			if (b.operator() == Binary.COND_AND) {
				...
				// from the second operand. However, this operator could be
				// false
				// if either the first or the second argument returned false.
				Item bitANDFalse = this.safeConfluence(trueItem,
						FlowGraph.EDGE_KEY_TRUE, falseItem,
						FlowGraph.EDGE_KEY_FALSE, n, false, graph);
				...
				Item bitORTrue = this.safeConfluence(trueItem,
						FlowGraph.EDGE_KEY_TRUE, falseItem,
						FlowGraph.EDGE_KEY_FALSE, n, false, graph);
	 ...
	 * 
	 * @param items
	 *            List of <code>Item</code>s that flow into <code>node</code>.
	 *            this method will only be called if the list has at least 2
	 *            elements.
	 * @param node
	 *            <code>Term</code> for which the <code>items</code> are flowing
	 *            into.
	 * @param entry
	 *            indicates whether we are looking at the entry or exit of node.
	 ...
	 *            elements.
	 * @param itemKeys
	 *            List of <code>FlowGraph.ExceptionEdgeKey</code>s for the edges
	 *            that the corresponding <code>Item</code>s in
	 ...
	 *            <code>items</code> flowed from.
	 * @param peer
	 *            <code>Peer</code> for which the <code>items</code> are flowing
	 *            into.
	 */
	protected Item safeConfluence(List items, List itemKeys, Peer peer,
			FlowGraph graph) {
		return this.safeConfluence(items, itemKeys, peer.node(),
				peer.isEntry(), graph);
	}
	...
	protected Item safeConfluence(Item item1, FlowGraph.EdgeKey key1,
			Item item2, FlowGraph.EdgeKey key2, Term node, boolean entry,
			FlowGraph graph) {
		return safeConfluence(item1, key1, item2, key2, null, null, node,
				entry, graph);
	}

	protected Item safeConfluence(Item item1, FlowGraph.EdgeKey key1,
			Item item2, FlowGraph.EdgeKey key2, Item item3,
			FlowGraph.EdgeKey key3, Term node, boolean entry, FlowGraph graph) {
	...
	/**
	 * Check that the term n satisfies whatever properties this dataflow is
	 * checking for. Provided the method check(FlowGraph, Peer) is not
	 * overridden, this method is called for each term in a code declaration
	 * block after the dataflow for that block of code has been performed.
	 * 
	 * @throws SemanticException
	 *             if the properties this dataflow analysis is checking for is
	 *             not satisfied.
	 */
	...
	/**
	 * Check that the term n satisfies whatever properties this dataflow is
	 * checking for. This method is called for each term in a code declaration
	 * block after the dataflow for that block of code has been performed.
	 * 
	 * @throws SemanticException
	 *             if the properties this dataflow analysis is checking for is
	 ...
	 * <code>dataflow(FlowGraph)</code> has been called, and for pushing the
	 * <code>FlowGraph</code> onto the stack of <code>FlowGraph</code>s if
	...
	static protected class Frame {
		...
		Frame(Peer p, boolean forward) {
			peer = p;
			if (forward)
				edges = p.succs().iterator();
			else
				edges = p.preds().iterator();
		}
	}

	/**
	 * Returns the linked list [by_scc, scc_head] where by_scc is an array in
	 * which SCCs occur in topologically sorted order. scc_head[n] where n is
	 * the first peer in an SCC is set to -1. scc_head[n] where n is the last
	 * peer in a (non-singleton) SCC is set to the index of the first peer.
	 * Otherwise it is -2.
	 */
		...
		if (Report.should_report(Report.dataflow, 2)) {
			for (int j = 0; j < n; j++) {
				...
				case -2:
					...
					break;
				default:
					Report.report(2, j + " ->" + scc_head[j] + " : "
							+ by_scc[j]);
				}
				for (Iterator i = by_scc[j].succs().iterator(); i.hasNext();) {
					Report.report(3,
							"     successor: " + ((Edge) i.next()).getTarget());
				}
			}
		}
	...
	/**
	 * Map from <code>Peer</code>s to <code>Integer</code>s that contains a
	 * post-ordering of <code>Peer</code>s if <code>this.detectBackEdges</code>
	 * is true.
	 */
	 ...
	 * reachable from p (that are reachable without going through any peer in
	 * the set <code>visited</code>). The postorder will start from
	 * <code>count</code>.
	 * 
	 * @param p
	 * @param count
	...
	protected void dataflow(FlowGraph graph) {
		...
		/*
		 * by_scc contains the peers grouped by SCC. scc_head marks where the
		 * SCCs are. The SCC begins with a -1 and ends with the index of the
		 * beginning of the SCC.
		 */
	 ...
	 * analysis.
	 * 
	 * @return null if no dataflow analysis should be performed for this code
	 *         declaration; otherwise, an appropriately initialized
	 *         <code>FlowGraph.</code>
	 */
	 ...
	 *            The type system
	 * @param g
	 *            The flow graph to that the CFGBuilder will construct.
	...
	protected Node leaveCall(Node old, Node n, NodeVisitor v)
		...
		if (n instanceof CodeNode) {
			if (!dataflowOnEntry) {
				...
				FlowGraphSource fgs = (FlowGraphSource) flowgraphStack
						.getFirst();
	...
	/**
	 * Check all of the Peers in the graph, after the dataflow analysis has been
	 * performed.
	 */
	 ...
	 * should not be called if dataflow is not being performed on entry to the
	 * <code>CodeNode</code>s, as the stack is not maintained in that case. If
	 * this method is called by a subclass from the <code>enterCall</code> or
	 * <code>leaveCall</code> methods, for an AST node that is a child of a
	 * <code>CodeNode</code>, then the <code>FlowGraph</code> returned should be
	 * the <code>FlowGraph</code> for the dataflow for innermost
	 * <code>CodeNode</code>.
	 */
	...
	/**
	 * This utility methods is for subclasses to convert a single Item into a
	 * <code>Map</code>, to return from the <code>flow</code> methods. This
	 ...
	 *            <code>Map</code> as the value for every <code>EdgeKey</code>
	 *            in <code>edgeKeys.</code>
	 * @param edgeKeys
	 *            the <code>Set</code> of <code>EdgeKey</code>s to be used as
	 *            keys in the returned <code>Map</code>.
	...
	/**
	 * This utility method is for subclasses to convert Items into a
	 * <code>Map</code>, to return from the <code>flow</code> methods.
	 * 
	 * @param trueItem
	 *            the <code>Item</code> to be placed in the returned
	 *            <code>Map</code> as the value for the
	 *            <code>FlowGraph.EDGE_KEY_TRUE</code>, if that key is present
	 *            in <code>edgeKeys.</code>
	 * @param falseItem
	 *            the <code>Item</code> to be placed in the returned
	 *            <code>Map</code> as the value for the
	 *            <code>FlowGraph.EDGE_KEY_FALSE</code>, if that key is present
	 *            in <code>edgeKeys.</code>
	 * @param remainingItem
	 *            the <code>Item</code> to be placed in the returned
	 ...
	 *         <code>Item i</code>.
	 */
	protected static final Map itemsToMap(Item trueItem, Item falseItem,
			Item remainingItem, Set edgeKeys) {
		Map m = new HashMap();
	 ...
	 * that are not associated with error flows, that is, only <code>Item</code>
	 * s whose associated <code>EdgeKey</code>s are not
	 ...
	 * @param itemKeys
	 *            List of <code>EdgeKey</code>s corresponding to the edge keys
	 *            for the <code>Item</code>s in <code>items</code>.
	 * @return a filtered list of items, containing only those whose edge keys
	 *         are not <code>FlowGraph.ExceptionEdgeKey</code>s with whose
	 *         exception types are <code>Error</code>s.
	 */
	protected final List filterItemsNonError(List items, List itemKeys) {
		...
		while (i.hasNext() && j.hasNext()) {
			...
			if (!(key instanceof ExceptionEdgeKey && ((ExceptionEdgeKey) key)
					.type().isSubtype(ts.Error()))) {
	 ...
	 * of <code>excType</code>. That is, only <code>Item</code>s whose
	 * associated <code>EdgeKey</code>s are
	 * <code>FlowGraph.ExceptionEdgeKey</code>s, with the type a subclass of
	 * <code>excType</code>.
	 * 
	 * @param items
	 *            List of Items to filter
	 ...
	 *            for the <code>Item</code>s in <code>items</code>.
	 * @param excType
	 *            an Exception <code>Type</code>.
	 * @return a filtered list of items, containing only those whose edge keys
	 *         are not <code>FlowGraph.ExceptionEdgeKey</code>s.
	 */
	protected final List filterItemsExceptionSubclass(List items,
			List itemKeys, Type excType) {
		List filtered = new ArrayList(items.size());
		Iterator i = items.iterator();
		Iterator j = itemKeys.iterator();
		while (i.hasNext() && j.hasNext()) {
	 ...
	 *            for the <code>Item</code>s in <code>items</code>.
	 * @param filterEdgeKey
	 *            the <code>EdgeKey</code> to use as a filter.
	 * @return a filtered list of items, containing only those whose edge keys
	 *         are the same as <code>filterEdgeKey</code>s.
	 */
	protected final List filterItems(List items, List itemKeys,
	 ...
	 * 
	 * @param edgeKeys
	 *            the <code>Set</code> of <code>EdgeKey</code>s of the successor
	 *            edges of a given node.
	 * @return true if the <code>edgeKeys</code> contains both
	 *         <code>Edge_KEY_TRUE</code> and <code>Edge_KEY_FALSE</code>
	 */
	 ...
	 * 
	 * @param booleanCond
	 *            the boolean condition that is used to branch on. The type of
	 *            the expression must be boolean.
	 * @param startingItem
	 *            the <code>Item</code> at the start of the flow for the
	 *            expression <code>booleanCond</code>.
	 * @param succEdgeKeys
	 *            the set of <code>EdgeKeys</code> of the successor nodes of the
	 *            current node. Must contain both
	 *            <code>FlowGraph.EDGE_KEY_TRUE</code> and
	 *            <code>FlowGraph.EDGE_KEY_FALSE</code>.
	 * @param navigator
	 *            an instance of <code>ConditionNavigator</code> to be used to
	 *            generate appropriate <code>Item</code>s from the boolean
	 *            condition.
	 * @return a <code>Map</code> containing mappings for all entries in
	 *         <code>succEdgeKeys</code>. <code>FlowGraph.EDGE_KEY_TRUE</code>
	 *         and <code>FlowGraph.EDGE_KEY_FALSE</code> map to
	 *         <code>Item</code>s calculated for them using navigator, and all
	 *         other objects in <code>succEdgeKeys</code> are mapped to
	 *         <code>startingItem</code>.
	 * @deprecated
	 */
	protected static Map constructItemsFromCondition(Expr booleanCond,
			Item startingItem, Set succEdgeKeys, ConditionNavigator navigator) {
		// check the arguments to make sure this method is used correctly
		if (!booleanCond.type().isBoolean()) {
			throw new IllegalArgumentException(
					"booleanCond must be a boolean expression");
		}
	 ...
	 * <code>Item</code> that is used when an expression is true, the other
	 * being the one that is used when an expression is false. It is used by the
	 * <code>ConditionNavigator</code>.
	 * 
	...
	/**
	 * A <code>ConditionNavigator</code> is used to traverse boolean expressions
	 * that are used as conditions, such as in if statements, while statements,
	 * left branches of && and ||. The <code>ConditionNavigator</code> is used
	 * to generate a finer-grained analysis, so that the branching flows from a
	 ...
	 * <code>cond</code> evaluated to true, and similarly, <code>s2</code> can
	 * be analyzed using the knowledge that <code>cond</code> evaluated to
	 * false.
	 * 
	 * @deprecated
	 */
	protected abstract static class ConditionNavigator {
		 ...
		 * <code>startingItem</code>.
		 * 
		 * A <code>BoolItem</code> is returned, containing the <code>Item</code>
		 * s that are appropriate when <code>expr</code> evaluates to true and
		 * false.
		 */
		...
		/**
		 * Combine the results of analyzing the left and right arms of an AND
		 * boolean operator (either &amp;&amp; or &amp;).
		 */
		public BoolItem andResults(BoolItem left, BoolItem right,
				Item startingItem) {
		...
		/**
		 * Combine the results of analyzing the left and right arms of an OR
		 * boolean operator (either || or |).
		 */
		public BoolItem orResults(BoolItem left, BoolItem right,
				Item startingItem) {
		...
		/**
		 * Modify the results of analyzing the child of a NEGATION boolean
		 * operator (a !).
		 */
		 ...
		 * combining them corresponds to unioning the sets. Note that this could
		 * be a different operation to the confluence operation.
		 */
		public abstract Item combine(Item item1, Item item2);

		/**
		 * Produce a <code>BoolItem</code> for an expression that is not a
		 * boolean operator, such as &&, &, ||, | or !.
		 */
	...
	protected void dumpFlowGraph(FlowGraph graph, Term root) {
		String name = StringUtil.getShortNameComponent(this.getClass()
				.getName());
		name += flowCounter++;
		...
		for (Iterator iter = graph.peers().iterator(); iter.hasNext();) {
			...
			Report.report(
					...
					p.hashCode()
							...
							+ "\\n("
							+ StringUtil.escape(StringUtil
									.getShortNameComponent(p.node.getClass()
											.getName())) + ")\" ];");
			...
			for (Iterator iter2 = p.succs.iterator(); iter2.hasNext();) {
				...
				Report.report(
						...
						q.getTarget().hashCode()
								+ " [ label = \""
								+ StringUtil.escape(q.getTarget().node
										.toString())
								+ " ("
								+ StringUtil.escape(StringUtil
										.getShortNameComponent(q.getTarget().node
												.getClass().getName()))
								+ ")\" ];");
				...
				Report.report(2, p.hashCode() + " -> "
						+ q.getTarget().hashCode() + " [label=\"" + label
						+ "\"];");
			}
