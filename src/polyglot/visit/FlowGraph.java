...
public class FlowGraph {
	 ...
	 * <code>peerMap</code> maps <code>IdentityKey(Node)</code>s to path maps. A
	 * path map is a map from paths (<code>ListKey(List of Terms)</code>) to
	 * <code>Peer</code>s. In particular, if <code>n</code> is an AST node in a
	 * finally block, then there will be a <code>Peer</code> of <code>n</code>
	 * for each possible path to the finally block, and the path map records
	 * which <code>Peer</code> corresponds to which path. If <code>n</code> does
	 * not occur in a finally block, then the path map should have only a single
	 * entry, from an empty list to the unique <code>Peer</code> for
	 * <code>n</code>.
	 * 
	 * <p>
	...
	/**
	 * Retrieve the entry or exit <code>Peer</code> for the <code>Term n</code>,
	 * where <code>n</code> does not appear in a finally block. If no such Peer
	 ...
	 * evaluates to true, and one will have the EdgeKey
	 * FlowGraph.EDGE_KEY_FALSE, and is the flow that is taken when the
	 * condition evaluates to false.
	 * 
	 * The differentiation of the flow graph edges allows for a finer grain data
	 * flow analysis, as the dataflow equations can incorporate the knowledge
	...
	/**
	 * This class extends EdgeKey and is the key for edges that are taken when
	 * an exception of type t is thrown. Thus, the flow from line 2 in the
	 * example below to the catch block (line 4) would have an ExceptionEdgeKey
	 * constructed with the Type representing NullPointerExceptions.
	 * 
	 * <pre>
	 * ...
	 * try {                                      // line 1
	...
	public static class ExceptionEdgeKey extends EdgeKey {
		...
		public String toString() {
			return (type().isClass() ? type().toClass().name() : type()
					.toString());
		}
	}

	/**
	 * This EdgeKey is the EdgeKey for edges where the expression evaluates to
	 * true.
	 */
	...
	/**
	 * This EdgeKey is the EdgeKey for edges where the flow is not suitable for
	 * EDGE_KEY_TRUE, EDGE_KEY_FALSE or an ExceptionEdgeKey, such as the edges
	 * from a switch statement to its cases and the flow from a sink node in the
	 * control flow graph.
	 */
	...
	/**
	 * A <code>Peer</code> is an occurrence of an AST node in a flow graph. For
	 * most AST nodes, there will be only one Peer for each AST node. However,
	 * if the AST node occurs in a finally block, then there will be multiple
	 * <code>Peer</code>s for that AST node, one for each possible path to the
	 * finally block. This is because flow graphs for finally blocks are copied,
	 * one copy for each possible path to the finally block.
	 */
	public static class Peer {
		protected DataFlow.Item inItem; // Input Item for dataflow analysis
		protected Map outItems; // Output Items for dataflow analysis, a map
								// from EdgeKeys to DataFlowlItems
		...
		protected List path_to_finally; // the path to the finally block that
										...
										// from the other Peers for the AST
										// node.
		 ...
		 * succs. This Set is lazily constructed, as needed, by the method
		 * succEdgeKeys()
		 */
		...
		/**
		 * The input data flow item. Should only be called after data flow
		...
		/**
		 * The output item for a particular EdgeKey. Should only be called after
	 ...
	 * equality (as defined in {@link polyglot.util.CollectionUtil
	 * CollectionUtil}).
	 */
	...
	public String toString() {
		...
		while (!queue.isEmpty()) {
			...
			for (Iterator i = p.succs.iterator(); i.hasNext();) {
				...
				// sb.append("  " +
				// StringUtil.getShortNameComponent(q.node.getClass().getName())
				// + " ["+q.node+"]" + "\n");
