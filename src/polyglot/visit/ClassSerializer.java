...
public class ClassSerializer extends NodeVisitor {
	...
	public List createSerializationMembers(ClassType ct) {
		try {
			...
			/* Add the compiler version number. */
			String version = ver.major() + "." + ver.minor() + "."
					+ ver.patch_level();

			Position pos = Position.compilerGenerated();

			fi = ts.fieldInstance(pos, ct, flags, ts.String(),
					"jlc$CompilerVersion$" + suffix);
			fi.setConstantValue(version);
			ii = ts.initializerInstance(pos, ct, Flags.STATIC);
			f = nf.FieldDecl(fi.position(), fi.flags(), nf.CanonicalTypeNode(
					fi.position(), fi.type()), nf.Id(fi.position(), fi.name()),
			...
			fi = ts.fieldInstance(pos, ct, flags, ts.Long(),
			...
			do {
				...
				String additionalFieldSuffix = numberETIFields == 0 ? ""
						: ("$" + numberETIFields);
