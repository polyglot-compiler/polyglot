...
public class LocalClassRemover extends ContextVisitor {
	protected final class ConstructorCallRewriter extends NodeVisitor {
		...
		public Node leave(Node old, Node n, NodeVisitor v) {
			if (n instanceof New) {
				...
				ConstructorInstance nci = (ConstructorInstance) ci
						.declaration();
				if (nci.container().toClass().declaration() == theLocalClass
						.declaration()) {
					neu = (New) neu.arguments(addArgs(neu, nci, newFields,
							curr, theLocalClass));
					if (!theLocalClass.flags().isStatic()) {
						Expr q;
						if (theLocalClass.outer() == context.currentClass())
							q = nf.This(neu.position()).type(
									theLocalClass.outer());
						else
							q = nf.This(
									neu.position(),
									nf.CanonicalTypeNode(neu.position(),
											theLocalClass.outer())).type(
									theLocalClass.outer());
						neu = neu.qualifier(q);
					}
				}
			...
			if (n instanceof ConstructorCall) {
				...
				if (nci.container().toClass().declaration() == theLocalClass
						.declaration()) {
					neu = (ConstructorCall) neu.arguments(addArgs(neu, nci,
							newFields, curr, theLocalClass));
					// This is wrong: we cannot refer to this in a super() call
					// qualifier.
					// For now, let this pass and assume InnerClassRemover will
					// fix it.
					if (!theLocalClass.flags().isStatic()) {
						Expr q;
						if (theLocalClass.outer() == context.currentClass())
	...
	public Node override(Node parent, Node n) {
		...
		// Add a field initializer to each constructor for each introduced
		// field.
		...
		if (n instanceof Block) {
			...
			for (int i = 0; i < ss.size(); i++) {
				...
				if (s instanceof LocalClassDecl) {
					...
					Flags flags = context.inStaticContext() ? Flags.PRIVATE
							.Static() : Flags.PRIVATE;
					cd = cd.flags(flags);
					cd.type().flags(flags);
					cd.type().kind(ClassType.MEMBER);

					cd = rewriteLocalClass(
							cd,
							(List) hashGet(newFields, cd.type(),
									Collections.EMPTY_LIST));

					if (cd != lcd.decl()) {
						ss.set(i, lcd.decl(cd));

						// Rewrite the constructor calls in the remaining
						// statements, including the class declaration statement
						// itself.
						for (int j = i; j < ss.size(); j++) {
							Stmt sj = (Stmt) ss.get(j);
							sj = (Stmt) rewriteConstructorCalls(
									sj,
									cd.type(),
									(List) hashGet(newFields, cd.type(),
											Collections.EMPTY_LIST));
	...
	protected Node leaveCall(Node old, Node n, NodeVisitor v)
		...
		if (n instanceof Local && !inConstructorCall) {
			...
			if (!isLocal(context, l.name())) {
				...
				if (fi != null) {
					Field f = nf.Field(pos, makeMissingFieldTarget(fi, pos),
							nf.Id(pos, fi.name()));
		...
		if (n instanceof New) {
			...
			ClassDecl cd = nf.ClassDecl(pos, Flags.PRIVATE, name, superClass,
					interfaces, body);
			...
			Flags flags = context.inStaticContext() ? Flags.PRIVATE.Static()
					: Flags.PRIVATE;
			type.flags(flags);
			...
			cd = rewriteLocalClass(
					cd,
					(List) hashGet(newFields, cd.type(), Collections.EMPTY_LIST));
			hashAdd(orphans, context.currentClassScope(), cd);
			neu = neu.objectType(nf.CanonicalTypeNode(pos, type)).body(null);
			neu = (New) rewriteConstructorCalls(
					neu,
					cd.type(),
					(List) hashGet(newFields, cd.type(), Collections.EMPTY_LIST));
			return neu;
	...
	ConstructorDecl addConstructor(ClassDecl cd, New neu) {
		// Build the list of formal parameters and list of arguments for the
		// super call.
		...
		for (Iterator j = neu.arguments().iterator(); j.hasNext();) {
			...
			i++;
			Formal f = nf.Formal(pos, Flags.FINAL,
					nf.CanonicalTypeNode(pos, e.type()), name);
			Local l = nf.Local(pos, name);

			LocalInstance li = ts.localInstance(pos, f.flags(), f.declType(),
					name.id());
			li.setNotConstant();
		...
		// Build the list of throw types, copied from the new expression's
		// constructor (now the superclass constructor).
		List throwTypeNodes = new ArrayList();
		List throwTypes = new ArrayList();
		for (Iterator j = neu.constructorInstance().throwTypes().iterator(); j
				.hasNext();) {
			Type t = (Type) j.next();
			throwTypes.add(t);
		...
		ConstructorDecl td = nf.ConstructorDecl(pos, Flags.PRIVATE, cd.id(),
				formals, throwTypeNodes, nf.Block(pos, statements));
		ConstructorInstance ci = ts.constructorInstance(pos, cd.type(),
				Flags.PRIVATE, argTypes, throwTypes);
	...
	// Add local variables to the argument list until it matches the
	// declaration.
	List addArgs(ProcedureCall n, ConstructorInstance nci, List fields,
			ClassType curr, ClassType theLocalClass) {
		...
		for (Iterator i = fields.iterator(); i.hasNext();) {
			...
			if (curr != null
				...
				// If in the local class being rewritten, use the boxed local
				// (i.e., field) instead of the local.
				// This could generate a bad constructor call since the field
				// will refer to 'this' before the superclass
	...
	private FieldInstance boxLocal(LocalInstance li) {
		...
		fi = ts.fieldInstance(pos, curr, li.flags().Private(), li.type(),
				li.name());
		fi.setNotConstant();
