...
/**
 * Visitor which checks that all (terminating) paths through a method must
 * return.
 */
public class ExitChecker extends DataFlow {
	...
	protected static class DataFlowItem extends Item {
		public final boolean exits; // whether all paths leaving this node lead
									// to an exit
	...
	public Map flow(Item in, FlowGraph graph, Term n, boolean entry,
		...
		// we're okay. So make the exit bit false at exit and true at every
		// return;
	...
	public void check(FlowGraph graph, Term n, boolean entry, Item inItem,
		...
		if (n == graph.root() && entry) {
			if (outItems != null && !outItems.isEmpty()) {
				// due to the flow equations, all DataFlowItems in the outItems
				// map
				// are the same, so just take the first one.
				DataFlowItem outItem = (DataFlowItem) outItems.values()
						.iterator().next();
