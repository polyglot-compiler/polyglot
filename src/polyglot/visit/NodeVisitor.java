 ...
 * visitor that traversal of a particular subtree has begun. <code>leave</code>
 * informs the visitor that traversal is finishing a particular subtree.
 * 
...
public abstract class NodeVisitor implements Copy {
	 ...
	 * were made to the subtree, then the visitor should return a <i>copy</i> of
	 * <code>n</code> with appropriate changes. Finally, if the visitor does not
	 * wish to override traversal of the subtree rooted at <code>n</code>, then
	 * it should return <code>null</code>.
	 * <p>
	 * The default implementation of this method is to call
	 * {@link #override(Node) override(n)}, as most subclasses do not need to
	 * know the parent of the node <code>n</code>.
	 * 
	 * @param parent
	 *            The parent of <code>n</code>, <code>null</code> if
	 *            <code>n</code> has no parent.
	 * @param n
	 ...
	 * {@link #override(Node, Node) override(parent, n)}. If a subclass
	 * overrides the method {@link #override(Node, Node) override(parent, n)}
	 ...
	 * <p>
	 * The default implementation of this method is to call {@link #enter(Node)
	 * enter(n)}, as most subclasses do not need to know the parent of the node
	 * <code>n</code>.
	 * 
	 * @param parent
	 *            The parent of <code>n</code>, <code>null</code> if
	 ...
	 * visitor which will be used to visit the children of <code>n</code>.
	 * <p>
	 * This method is typically called by the method {@link #enter(Node, Node)
	 * enter(parent, n)}. If a subclass overrides the method
	 * {@link #enter(Node, Node) enter(parent, n)} then this method may not be
	 * called.
	 * 
	 * @param n
	 *            The root of the subtree to be traversed.
	 ...
	 * <p>
	 * The default implementation of this method is to call
	 * {@link #leave(Node, Node, NodeVisitor) leave(old, n, v)}, as most
	 * subclasses do not need to know the parent of the node <code>n</code>.
	 * 
	 * @param parent
	 *            The parent of <code>old</code>, <code>null</code> if
	 *            <code>old</code> has no parent.
	 * @param old
	 *            The original state of root of the current subtree.
	 * @param n
	 *            The current state of the root of the current subtree.
	 * @param v
	 *            The <code>NodeVisitor</code> object used to visit the
	 *            children.
	 ...
	 * <p>
	 * This method is typically called by the method
	 * {@link #leave(Node, Node, Node, NodeVisitor) leave(parent, old, n v)}. If
	 * a subclass overrides the method
	...
	/**
	 * The begin method is called before the entire tree is visited. This method
	 * allows the visitor to perform any initialization that cannot be done when
	 * the visitor is created. If <code>null</code> is returned, the ast is not
	 * traversed.
	 * 
	...
	/**
	 * The finish method is called after the entire tree has been visited. This
	 * method allows the visitor to perform any last minute cleanup, including
	 * flushing buffers and I/O connections.
	 */
	public void finish() {
	}
	 ...
	 * node <code>child</code>. This method recursively visits the subtree
	 * rooted at <code>child</code>.
	 * 
	 * @param parent
	 *            the parent node of <code>child</code>, <code>null</code> if
	 *            <code>child</code> was visited by calling
	 *            {@link polyglot.ast.Node#visit(NodeVisitor)
	 *            Node.visit(NodeVisitor)} instead of
	 *            {@link polyglot.ast.Node#visitChild(Node, NodeVisitor)
	 *            polyglot.ast.Node.visitChild(Node, NodeVisitor)}.
	 * @param child
	 *            the child node of <code>parent</code> to be visited.
	 * @return the (possibly new) version of <code>child</code> after the
	 *         subtree rooted at <code>child</code> has been recursively
	 *         visited.
	 */
	...
	/**
	 * Visit the edge between the parent node <code>parent</code>, and child
	 * node <code>child</code>, without invoking <code>override</code> for the
	 * child. This method recursively visits the subtree rooted at
	 * <code>child</code>.
	 * 
	 * @param parent
	 * @param child
	...
	public Node visitEdgeNoOverride(Node parent, Node child) {
		...
		if (n == null) {
			throw new InternalCompilerError(
					"Node.visitChildren() returned null.");
		}
