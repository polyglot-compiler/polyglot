 ...
 * @author nystrom
 * 
 *         This class translates local classes and anonymous classes to member
 *         classes. It adds fields to the classes for each local variable in the
 *         enclosing method that is used in the class body.
 */
public class LocalClassRemoverOld extends ContextVisitor {
	...
	static class EnvCollector extends ContextVisitor {
		...
		/*
		 * public Node override(Node parent, Node n) { if (n instanceof
		 * LocalClassDecl) { return n; } if (parent instanceof New && n
		 * instanceof ClassBody) { return n; } return super.override(n); }
		 */
		...
		public Node leaveCall(Node old, Node n, NodeVisitor v)
			...
			if (n instanceof Local) {
				...
				// The variable should be in the environment if it's not in the
				// local
				...
				if (!context.isLocal(local.name())) {
					try {
						...
						if (outerContext.isLocal(local.name())) {
							...
							// System.out.println("  defined in enclosing method: "
							// + n);
							env.add(local.localInstance().orig());
						}
					} catch (SemanticException e) {
						// System.out.println("  not in scope of enclosing method: "
						// + n);
						// The local was defined somewhere within the class
						// body.
					}
				} else {
	...
	FieldInstance localToField(ParsedClassType ct, LocalInstance li) {
		FieldInstance fi = ts.fieldInstance(li.position(), ct, li.flags()
				.Protected(), li.type(), namePrefix() + li.name());
		return fi;
	}
	...
	class ClassBodyTranslator extends ContextVisitor {
		...
		ClassBodyTranslator(Job job, TypeSystem ts, NodeFactory nf,
				Context context, ParsedClassType ct, Map fieldMap) {
			super(job, ts, nf);
			this.ct = ct;
		...
		public Node leaveCall(Node old, Node n, NodeVisitor v)
			...
			if (n instanceof Local) {
				Local l = (Local) n;
				FieldInstance fi = (FieldInstance) fieldMap
						.get(new IdentityKey(l.localInstance().orig()));
				if (fi != null) {
					Special this_;
					if (ct.equals(context.currentClass())) {
						...
						this_ = nf.Special(l.position(), Special.THIS,
								nf.CanonicalTypeNode(l.position(), ct));
					}
			...
			if (n instanceof ConstructorDecl) {
				...
				ClassType ct2 = (ClassType) ctd.constructorInstance()
						.container();
				if (ct2.equals(ct)) {
	...
	ConstructorInstance createEmptyCI(ParsedClassType ct) {
		ConstructorInstance ci = ts.constructorInstance(ct.position(), ct,
				Flags.PRIVATE, Collections.EMPTY_LIST, Collections.EMPTY_LIST);
		ct.addConstructor(ci);
		return ci;
	}

	ConstructorDecl createEmptyConstructorDecl(ParsedClassType ct,
			ConstructorInstance ci) {
		List stmts = new ArrayList();

		try {
			ConstructorInstance superCI = ct.typeSystem().findConstructor(
					(ClassType) ct.superType(), Collections.EMPTY_LIST, ct);
			ConstructorCall superCall;

			if (!ct.flags().isStatic()) {
				Special this_ = nf.Special(ci.position(), Special.THIS,
						nf.CanonicalTypeNode(ci.position(), ct.container()));
		...
		ConstructorDecl cd = nf.ConstructorDecl(ci.position(), ci.flags(),
				ct.name(), Collections.EMPTY_LIST, Collections.EMPTY_LIST, b);
	...
	ConstructorDecl translateConstructorDecl(ParsedClassType ct,
		...
		if (cc == null || cc.kind() == ConstructorCall.SUPER) {
			for (Iterator i = env.iterator(); i.hasNext();) {
				...
				FieldAssign assign = nf.FieldAssign(cd.position(), target,
		...
		/*
		 * void m() { final T x; class C { C(y) { super(y); ... x ... } C() {
		 * this(0); ... x ... } } new C(e); }
		 * 
		 * ->
		 * 
		 * class C { T x; C(y) { super(y); this.x = x; ... this.x ... } C() {
		 * this(0); ... this.x ... } } void m() { final T x; new C(e, x); }
		 */
	}

	ClassDecl createMemberClass(ParsedClassType ct, ClassBody body) {
		TypeNode superClass = nf.CanonicalTypeNode(ct.position(),
				ct.superType());
		...
		ClassDecl cd = nf.ClassDecl(ct.position(), ct.flags(), ct.name(),
	...
	List envAsFormals(List env) {
		...
		for (Iterator i = env.iterator(); i.hasNext();) {
			...
			Formal f = nf.Formal(Position.compilerGenerated(), li.flags(),
					nf.CanonicalTypeNode(li.position(), li.type()), li.name());
	...
	protected Node leaveCall(Node old, Node n, NodeVisitor v)
		...
		// If this class extends a local class, we need to change its
		// constructor
		...
		if (n instanceof ConstructorDecl) {
			...
			if (isLocal(ct) && !ct.isLocal()) {
				n = translateConstructorDecl(ct, (ConstructorDecl) n,
		...
		if (n instanceof New) {
			...
			if (newExp.body() != null) {
				...
				Context c = newExp.del()
						.enterChildScope(newExp.body(), context);
				...
				newExp = newExp.objectType(nf.CanonicalTypeNode(
						newExp.position(), pct));
	...
	protected void translateAnonClassBody(ParsedClassType ct, List arguments,
		...
		Special this_ = nf.Special(
				Position.compilerGenerated(),
				Special.THIS,
				nf.CanonicalTypeNode(Position.compilerGenerated(),
						ct.container()));
		this_ = (Special) this_.type(ct.container());
		ConstructorCall superCall = nf.SuperCall(Position.compilerGenerated(),
				this_, actuals);
		superCall = superCall.constructorInstance(superCI);
		stmts.add(superCall);
		...
		ConstructorInstance ci = ts.constructorInstance(ct.position(), ct,
				Flags.PRIVATE, superCI.formalTypes(), superCI.throwTypes());
		ConstructorDecl decl = nf.ConstructorDecl(Position.compilerGenerated(),
				ci.flags(), nf.Id(Position.compilerGenerated(), ct.name()),
				formals, ci.throwTypes(),
	...
	protected void translateLocalClassBody(ParsedClassType ct, ClassBody body,
			Context context) {
		...
		ClassBodyTranslator v = new ClassBodyTranslator(job, ts, nf, context,
				ct, fieldMap);
