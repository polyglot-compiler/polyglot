...
public class CFGBuilder implements Copy {
	...
	/**
	 * The innermost loop or try-block in lexical scope. We maintain a stack of
	 * loops and try-blocks in order to add edges for break and continue
	 ...
	 * that caused entry into this and lexically enclosing finally blocks. We
	 * construct a unique subgraph for each such path. The list is empty if this
	 * CFGBuilder is not constructing the CFG for a finally block.
	 */
	 ...
	 * graph. By default, we do not, but subclasses can change this behavior if
	 * needed.
	 */
	...
	/**
	 * Construct a new CFGBuilder with the a new innermost loop or try-block
	 * <code>n</code>.
	 */
	...
	/**
	 * Construct a new CFGBuilder with the a new innermost loop or try-block
	 * <code>n</code>, optionally skipping innermost catch blocks.
	 */
	...
	/**
	 * Visit edges from a branch. Simulate breaking/continuing out of the loop,
	 * visiting any finally blocks encountered.
	 */
		...
		for (CFGBuilder v = this; v != null; v = v.outer) {
			...
			if (c instanceof Try) {
				...
				if (tr.finallyBlock() != null) {
					last_visitor = tryFinally(v, last, last_visitor,
							tr.finallyBlock());
			...
			if (b.label() != null) {
				if (c instanceof Labeled) {
					...
					if (l.label().equals(b.label())) {
						if (b.kind() == Branch.BREAK) {
							edge(last_visitor, last, l, Term.EXIT,
							...
							if (s instanceof Loop) {
								...
								edge(last_visitor, last, loop.continueTarget(),
										Term.ENTRY, FlowGraph.EDGE_KEY_OTHER);
							} else {
								throw new CFGBuildError(
										"Target of continue statement must "
				...
				if (c instanceof Loop) {
					...
					edge(last_visitor, last, c, Term.EXIT,
							FlowGraph.EDGE_KEY_OTHER);
					return;
				}
			}
	...
	public void visitCFG(Term a, FlowGraph.EdgeKey edgeKey1, Term succ1,
		...
		visitCFG(a, CollectionUtil.list(new EdgeKeyTermPair(edgeKey1, succ1,
				entry1), new EdgeKeyTermPair(edgeKey2, succ2, entry2)));
	}
	 ...
	 * and each corresponding element determines whether a successor is an entry
	 * or exit node (using Term.ENTRY or Term.EXIT).
	 */
	...
	/**
	 * Create edges for a node <code>a</code> with successors <code>succs</code>
	 * .
	 * 
	 * @param a
	 *            the source node for the edges.
	 * @param succs
	 *            a list of <code>EdgeKeyTermPair</code>s
	 */
	protected void visitCFG(Term a, List succs) {
		...
		if (child == null) {
			...
			edge(this, a, Term.ENTRY, child, Term.ENTRY,
					FlowGraph.EDGE_KEY_OTHER);
		}
	...
	public void visitThrow(Term t, int entry, Type type) {
			...
			if (c instanceof Try) {
				...
				if (!v.skipInnermostCatches) {
					...
					for (Iterator i = tr.catchBlocks().iterator(); i.hasNext();) {
						...
						int e = (last == t && entry == Term.ENTRY) ? Term.ENTRY
								: Term.EXIT;
						...
						// possible catch
							...
							edge(last_visitor,
									...
									cb,
									Term.ENTRY,
									new FlowGraph.ExceptionEdgeKey(cb
											.catchType()));
						}
					}
					if (definiteCatch) {
		...
		if (errorEdgesToExitNode || !type.isSubtype(ts.Error())) {
			FlowGraph.Peer pp = graph.peer(last, last_visitor.path_to_finally,
					e);
	 ...
	 * 
	 * @param v
	 *            v.innermostTarget is the Try term that the finallyBlock is
	 *            assoicated with. @@@XXX
	 * @param last
	 *            the last term visited before the finally block is entered.
	 * @param last_visitor
	 *            @@@XXX
	 * @param finallyBlock
	 *            the finally block associated with a try finally block.
	 */
	protected static CFGBuilder tryFinally(CFGBuilder v, Term last,
		...
		// @@@XXX
		v_.edge(last_visitor, last, finallyBlock, Term.ENTRY,
				FlowGraph.EDGE_KEY_OTHER);
	...
	/**
	 * Enter a finally block. This method returns a new CFGBuilder with the
	 * path_to_finally field pointing to a list that has the Term
	 * <code>from</code> appended.
	 */
	 ...
	 * entry or exit of <code>q</code>.
	 */
	public void edge(CFGBuilder p_visitor, Term p, Term q, int qEntry,
	 ...
	 */
	public void edge(CFGBuilder p_visitor, Term p, Peer pq,
			FlowGraph.EdgeKey edgeKey) {
	...
	/**
	 * @param p_visitor
	 *            The visitor used to create p ("this" is the visitor that
	 *            created q)
	 * @param p
	 *            The predecessor node in the forward graph
	 * @param pEntry
	 ...
	 *            Term.ENTRY or Term.EXIT.
	 */
	public void edge(CFGBuilder p_visitor, Term p, int pEntry, Term q,
	...
	protected void edge(FlowGraph.Peer pp, FlowGraph.Peer pq,
			FlowGraph.EdgeKey edgeKey) {
		if (Report.should_report(Report.cfg, 2))
