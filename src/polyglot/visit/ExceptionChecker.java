...
public class ExceptionChecker extends ErrorHandlingVisitor {
	...
	/**
	 * Set of exceptions that can be caught. Combined with the outer field,
	 * these sets form a stack of exceptions, representing all and only the
	 * exceptions that may be thrown at this point in the code.
	 * 
	 * Note: Consider the following code, where A,B,C,D are Exception
	 * subclasses. void m() throws A, B { try { ... } catch (C ex) { ... } catch
	 * (D ex) { ... } }
	 * 
	 * Inside the try-block, the stack of catchable sets is: { C } { D } { A, B
	 * }
	 */
		...
		if (!t.isUncheckedException()) {
			...
			while (!exceptionCaught && ec != null) {
				if (ec.catchable != null) {
					for (Iterator iter = ec.catchable.iterator(); iter
							.hasNext();) {
	...
	public static class UncaughtReporter {
		/**
		 * This method must throw a SemanticException, reporting that the
		 * Exception type t must be caught.
		 * 
		 * @throws SemanticException
		 */
