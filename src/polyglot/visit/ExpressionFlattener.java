 ...
 * </ul>
 * 
 * To summarize, each statement will make at most one write, and contain at most
 * one dereference. The only write operation is the simple assignment (=).
 */
public class ExpressionFlattener extends NodeVisitor {
	 ...
	 * themselves, and not their subexpressions (unless they are also in the set
	 * explicitly).
	 */
	...
	/**
	 * Create a local declaration that can take a value of the same type as e,
	 * and initialize it to the expression val. Return the new local that was
	 * declared.
	 */
	...
	/**
	 * Returns true if e should never be flattened or if e has been added to the
	 * don't flatten list. Note that is then removed from the don't flatten
	 * list.
	 */
	...
	/**
	 * Create a declaration for a local variable with the same type as the
	 * expression e, with initializing expression init. Return the LocalDecl
	 * produced. Do NOT add the local decl to the statements
	 */
	protected LocalDecl createDecl(Expr e, Expr init) {
		String name = newId();
		Position pos = e.position();
		LocalInstance li = ts.localInstance(pos, Flags.NONE, typeOf(e), name);
		LocalDecl d = nf.LocalDecl(pos, Flags.NONE,
				(TypeNode) postCreate(nf.CanonicalTypeNode(pos, typeOf(e))),
	...
	protected Local createLocal(LocalDecl d) {
		...
		Local l = (Local) nf.Local(pos, (Id) postCreate(nf.Id(pos, d.name())))
				.type(typeOf(li));
	...
	protected Eval createAssign(Expr l, Expr r) {
		...
		l = (Expr) deepCopy(l);
		Eval a = nf.Eval(pos, ((Assign) postCreate(nf.Assign(pos, l,
				Assign.ASSIGN, r))).type(typeOf(l)));
		a = (Eval) postCreate(a);
		return a;
	}
	...
	protected Assign createSimpleAssign(Assign a) {
		...
		a = (Assign) nf.Assign(
				...
				a.left(),
				Assign.ASSIGN,
				((Binary) postCreate(nf.Binary(pos, (Expr) deepCopy(a.left()),
						op, a.right()))).type(typeOf(a))).type(typeOf(a));
		a = (Assign) postCreate(a);
		return a;
	}

	/**
	 * Convert an increment or decrement to an assignment, e.g. "i++" to
	 * "i = i + 1"
	 */
	...
	/**
	 * Create an if statement that assigns to l the value of "cond && e",
	 * evaluating e only if "cond" is true. i.e.,
	 * "if (cond) l = e; else l = false;"
	 */
	...
	/**
	 * Create an if statement that assigns to l the value of "cond || e",
	 * evaluating e only if "cond" is false. i.e.,
	 * "if (cond) l = true; else l = e;"
	 */
	...
	/**
	 * Create an if statement that assigns to l the value of "cond ? e1 : e2"
	...
	protected BooleanLit createBool(boolean val) {
		return (BooleanLit) ((BooleanLit) postCreate(nf.BooleanLit(
				Position.compilerGenerated(), val))).type(ts.Boolean());
	}
	...
	protected IntLit createInt(int val) {
		return (IntLit) ((IntLit) postCreate(nf.IntLit(
				Position.compilerGenerated(), IntLit.INT, val))).type(ts.Int());
	}
