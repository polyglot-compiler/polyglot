...
public class CopyPropagator extends DataFlow {
	...
	protected static class DataFlowItem extends Item {
		...
		protected static class CopyInfo {
			final public LocalInstance li; // Local instance this node pertains
											// to.
			...
			protected void setRoot(CopyInfo root) {
				...
				while (worklist.size() > 0) {
					CopyInfo ci = (CopyInfo) worklist
							.remove(worklist.size() - 1);
		...
		private void consistencyCheck() {
			for (Iterator it = map.entrySet().iterator(); it.hasNext();) {
				...
				if (li != ci.li)
					die();
				if (!map.containsKey(ci.root.li))
					die();
				if (map.get(ci.root.li) != ci.root)
					die();
				...
				if (ci.from == null) {
					...
					if (!map.containsKey(ci.from.li))
						die();
					if (map.get(ci.from.li) != ci.from)
						die();
				...
				for (Iterator i = ci.to.iterator(); i.hasNext();) {
					CopyInfo toCI = (CopyInfo) i.next();
					if (!map.containsKey(toCI.li))
						die();
					if (map.get(toCI.li) != toCI)
						die();
	...
	public Map flow(Item in, FlowGraph graph, Term t, boolean entry,
			Set succEdgeKeys) {
