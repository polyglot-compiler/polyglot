...
public abstract class InnerClassAbstractRemover extends ContextVisitor {
	...
	List envAsActuals(List env, ClassType outer, Expr qualifier) {
		...
		for (Iterator i = env.iterator(); i.hasNext();) {
			...
			if (outer != null) {
				// XXX Search "outer" for a field whose name and type matches
				// "ct".
				...
				if (fi != null && fi.type().equals(ct)) {
					...
					Field field = nf.Field(Position.compilerGenerated(), this_,
			...
			TypeNode tn = nf
					.CanonicalTypeNode(Position.compilerGenerated(), ct);
			Special this_ = nf.Special(Position.compilerGenerated(),
					Special.THIS, tn);
			this_ = (Special) this_.type(ct);
			actuals.add(this_);
