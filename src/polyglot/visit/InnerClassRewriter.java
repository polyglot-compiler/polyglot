 ...
 *         field pointing to the enclosing instance.
 */
public class InnerClassRewriter extends InnerClassAbstractRemover {
	...
	FieldInstance localToField(ParsedClassType ct, ClassType outer) {
		FieldInstance fi = ts.fieldInstance(Position.compilerGenerated(), ct,
				Flags.FINAL.Protected(), outer, mangleClassName(outer));
		return fi;
	}
	...
	class ClassBodyTranslator extends NodeVisitor {
		...
		public Node leave(Node old, Node n, NodeVisitor v) {
			if (n instanceof Special) {
				Special s = (Special) n;
				if (s.qualifier() != null) {
					FieldInstance fi = (FieldInstance) fieldMap.get(s
							.qualifier().type());
					if (fi != null) {
	...
	ConstructorDecl translateConstructorDecl(ParsedClassType ct,
		...
		if (cc == null || cc.kind() == ConstructorCall.SUPER) {
			for (Iterator i = envAsFormals(env).iterator(); i.hasNext();) {
				...
				if (fi == null) {
					// Not a enclosing class of ct, so must be an enclosing
					// class
					// of a supertype. The supertype will initialize.
					continue;
				}

				Special this_ = nf.Special(Position.compilerGenerated(),
						Special.THIS);
				this_ = (Special) this_.type(ct);

				Field target = nf.Field(Position.compilerGenerated(), this_,
						fi.name());
				target = target.fieldInstance(fi);
				target = (Field) target.type(fi.type());

				Local source = nf
						.Local(Position.compilerGenerated(), li.name());
				source = source.localInstance(li);
				source = (Local) source.type(li.type());

				FieldAssign assign = nf.FieldAssign(
						Position.compilerGenerated(), target, Assign.ASSIGN,
						source);
	...
	protected Node leaveCall(Node old, Node n, NodeVisitor v)
		...
		if (n instanceof ClassDecl) {
			...
			if (!env.isEmpty()) {
				// Translate the class body if any supertype (including ct
				// itself)
				// is an inner class.
				Context innerContext = cd.del().enterChildScope(cd.body(),
						context);
