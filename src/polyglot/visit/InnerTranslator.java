 ...
 * @author xinqi
 * 
 *         This class translates inner classes to static nested classes with a
 *         field referring to the enclosing instance. It will also add "new"
 *         methods to the enclosing class corresponding to constructors of inner
 *         classes.
 * 
 */
...
public class InnerTranslator extends NodeVisitor {
	...
	protected class ClassInfo {
		ParsedClassType ct;
		Map localNameCount; // Count how many local/anonymous classes with a
							// particular name have appeared.
		List newMemberClasses; // New member class declarations converted from
		...
		public void addConsFormal(Formal f) {
			...
			FieldInstance fi = ts.fieldInstance(Position.compilerGenerated(),
					ct, Flags.PROTECTED, f.type().type(),
					newFieldName(f.name()));
			ct.addField(fi);
		}
	...
	protected class CodeInfo {
		...
		Stack blockFinals; // stack of lists of final variables defined in a
							// block.
		...
		/*
		 * Add the name of a local/anonymous class, where anonymous classes have
		 * name "".
		 */
		...
		/*
		 * Return whether the code is static. Local/anonymous classes inside
		 * static code should not have outer field.
		 */
	...
	protected Stack classContext; // The context stack of all the enclosing
									// classes.
	...
	protected HashMap innerClassInfoMap; // The map from full names to class
											// infos of inner classes.
	protected Stack insideCode; // Boolean stack that indicates whether it is
								// inside a piece of code now.
	protected Stack staticFieldDecl; // Boolean stack that indicates whether it
										// is inside
	...
	protected void enterClassDecl(ClassDecl cd) {
		...
		if (ct.isLocal()) {
			...
			ct.needSerialization(false); // local classes don't need
											// serialization.
			String className = classInfo.localClassName(cd.name(),
			...
			for (Iterator it = codeInfo.finalList().iterator(); it.hasNext();) {
				...
				Formal f = nf.Formal(
						...
						nf.CanonicalTypeNode(Position.compilerGenerated(),
								li.type()), name);
				f = f.localInstance(ts.localInstance(Position
						.compilerGenerated(), f.flags(), f.type().type(), f
						.name()));
	...
	protected void enterNew(New newExpr) {
		if (newExpr.body() != null) {
			...
			// Check whether the anonymous class is defined outside a code (as
			...
			if (inCode && !codeInfo.isStatic() || !inCode
					&& !((Boolean) staticFieldDecl.peek()).booleanValue()) {
				// If this local/anonymous class is inside a static method,
				// then it shouldn't have an outer field.
			...
			ct.kind(ClassType.MEMBER);
			ct.outer(classInfo.classType());
			ct.needSerialization(false); // anonymous classes don't need
											// serialization.
			String className = classInfo.localClassName("",
					classInfo.addLocalClassName(""));
			ct.name(className);

			if (inCode) {
				for (Iterator it = codeInfo.finalList().iterator(); it
						.hasNext();) {
					LocalInstance li = (LocalInstance) it.next();
					String name = li.name();
					Formal f = nf.Formal(Position.compilerGenerated(),
							Flags.NONE, nf.CanonicalTypeNode(
									Position.compilerGenerated(), li.type()),
							name);
					f = f.localInstance(ts.localInstance(Position
							.compilerGenerated(), f.flags(), f.type().type(), f
							.name()));
	...
	protected void enterCodeDecl(CodeDecl cd) {
		...
		// If it is a constructor or method, find all the final arguments and
		// add them to the finalVar map.
	...
	public Node leave(Node old, Node n, NodeVisitor v) {
		if (n instanceof ClassDecl) {
			...
			// Like Field accesses, we might also have method calls that have no
			// explicit "A.this."
	...
	protected Node leaveClassDecl(Node old, ClassDecl cd, NodeVisitor v) {
		...
		// Do nothing if it is already a static class, or it is a toplevel
		// class,
	...
	protected Node leaveNew(Node old, New newExpr, NodeVisitor v) {
		if (newExpr.body() != null) {
			...
			// Need to create a class declaration, and add it to the enclosing
			// class.
			ParsedClassType ct = newExpr.anonType();
			ClassDecl cd = nf.ClassDecl(
					Position.compilerGenerated(),
					ct.flags(),
					ct.name(),
					nf.CanonicalTypeNode(Position.compilerGenerated(),
							ct.superType()), Collections.EMPTY_LIST,
					newExpr.body());
			cd = cd.type(ct);
	...
	/**
	 * Translate final local variables that should become field accesses of
	 * local/anonymous classes.
	 * 
	 * @param old
	 * @param local
	...
	protected Node leaveLocal(Node old, Local local, NodeVisitor v) {
		if (local.flags().isFinal()) {
			...
			if (!codeInfo.existFinal(local.name())) {
				...
				f = f.fieldInstance(ts.fieldInstance(
						Position.compilerGenerated(), (ReferenceType) t.type(),
						Flags.PROTECTED, local.type(), f.name()));
				return f;
			}
		}
	...
	protected ClassDecl addAnonymousConstructor(ClassDecl cd,
		...
		ConstructorDecl cons = nf.ConstructorDecl(Position.compilerGenerated(),
				Flags.NONE, ct.name(), formals, Collections.EMPTY_LIST,
				nf.Block(Position.compilerGenerated(), cc));
		ConstructorInstance consInst = ts.constructorInstance(
				Position.compilerGenerated(), ct, Flags.NONE, ftypes,
	...
	protected ParsedClassType findMethod(MethodInstance mi, ClassType current) {
		for (int i = classContext.size() - 1; i >= 0; i--) {
			...
			ParsedClassType ct = cinfo.classType();
			try {
				MethodInstance temp = ts.findMethod(ct, mi.name(),
						mi.formalTypes(), current);
			} catch (SemanticException se) {
				continue;
			}
			return ct;
	...
	protected ConstructorCall updateConstructorCall(ConstructorCall cc,
		...
		if (cc.kind().equals(ConstructorCall.THIS)) {
			// If calling a constructor of the same class, just need to pass all
			// the new formals.
			ClassInfo cinfo = (ClassInfo) classContext.peek();
			...
			if (cinfo != null) {
				...
				if (cinfo.insideCode() == null) {
					// For member inner classes, only need to add one formal
					// that refers to the outer instance.
					ci = updateConstructorInst(ct, ci, cinfo);
					List args = new ArrayList(cc.arguments().size() + 1);
					Formal f = (Formal) selfInfo.newConsFormals().get(0);
					Local l = nf.Local(Position.compilerGenerated(),
							outerThisName(ct));
					l = l.localInstance(f.localInstance());
					l = (Local) l.type(f.type().type());
					args.addAll(cc.arguments());
					...
					// The super class is a local class, and they are within the
					// same code.
					ci = updateConstructorInst(ct, ci, cinfo);
					List formals = cinfo.newConsFormals();
					List args = new ArrayList(cc.arguments().size()
							+ formals.size());
					args.addAll(cc.arguments());
					for (Iterator it = formals.iterator(); it.hasNext();) {
						Formal f = (Formal) it.next();
						args.add(nf.Local(Position.compilerGenerated(),
								f.name()));
					}
					cc = (ConstructorCall) cc.arguments(args);
					cc = cc.constructorInstance(ci);
				} else {
					// The super class is a local class, and they are not within
					// the same code.
					// Need to first find an enclosing class that is inside the
					// same code as the super class,
					// and use its fields as arguments.
					Local outerLocal = nf.Local(Position.compilerGenerated(),
							outerThisName(ct));
					outerLocal = outerLocal.localInstance(((Formal) selfInfo
							.newConsFormals().get(0)).localInstance());
					Expr outer = outerLocal;
					ClassType outerCt = selfInfo.classType().outer();
					ClassType tThis = ct;
					ClassInfo outerCInfo = (ClassInfo) innerClassInfoMap
							.get(outerCt.fullName());
					while (outerCInfo.insideCode() != cinfo.insideCode()) {
						...
						outerCInfo = (ClassInfo) innerClassInfoMap.get(outerCt
								.fullName());
					}
					ci = updateConstructorInst(ct, ci, cinfo);
					List formals = cinfo.newConsFormals();
					...
					for (Iterator it = formals.iterator(); it.hasNext();) {
						Formal f = (Formal) it.next();
						args.add(nf.Field(Position.compilerGenerated(), outer,
								newFieldName(f.name())));
					}
					cc = (ConstructorCall) cc.arguments(args);
					cc = cc.constructorInstance(ci);
	...
	protected Expr updateNewExpr(New newExpr) {
		...
		if (cinfo != null) {
			...
			List formals = cinfo.newConsFormals();
			List args = new ArrayList(newExpr.arguments().size()
					+ formals.size());
			List ftypes = new ArrayList(newExpr.arguments().size()
					+ formals.size());
			...
			if (cinfo.hasOuterField()) {
				...
				it.next(); // Only if there is an outer field, the first
							// argument needs to be skipped.
			}
			for (; it.hasNext();) {
				Formal f = (Formal) it.next();
			...
			if (newExpr.anonType() != null) {
				...
				nExpr = nExpr.objectType(nf.CanonicalTypeNode(
			...
			// Maybe we have encountered the new expression of an inner class
			// before it is translated.
			// But we have to make sure that ct is a type in the source
			// language.
			ConstructorInstance ci = newExpr.constructorInstance();
			List args = new ArrayList(newExpr.arguments().size() + 1);
			List ftypes = new ArrayList(newExpr.arguments().size() + 1);
	...
	protected ConstructorDecl produceDefaultConstructor(ParsedClassType ct,
		...
		ConstructorCall cc = nf.ConstructorCall(Position.compilerGenerated(),
				ConstructorCall.SUPER, Collections.EMPTY_LIST);
		ConstructorInstance cci = ts.constructorInstance(
				Position.compilerGenerated(), (ClassType) ct.superType(),
		...
		ConstructorDecl cd = nf.ConstructorDecl(Position.compilerGenerated(),
				...
				Collections.EMPTY_LIST,
				nf.Block(Position.compilerGenerated(), cc));
		ConstructorInstance cdi = ts.constructorInstance(
				Position.compilerGenerated(), ct, Flags.PUBLIC,
				Collections.EMPTY_LIST, Collections.EMPTY_LIST);
	...
	protected ClassDecl updateClassDecl(ClassDecl cd, ParsedClassType ct,
		...
		if (ct.constructors().size() == 0) {
			...
			ConstructorDecl cons = updateConstructor(cd, ct,
					produceDefaultConstructor(ct, cinfo), cinfo);
			members.add(cons);
	...
	protected List produceFieldDecls(ClassType ct, ClassInfo cinfo) {
		...
		for (Iterator it = newFormals.iterator(); it.hasNext();) {
			...
			FieldDecl fd = nf
					.FieldDecl(Position.compilerGenerated(), Flags.PROTECTED,
							formal.type(), newFieldName(formal.name()));
			FieldInstance fi = ts.fieldInstance(Position.compilerGenerated(),
					ct, Flags.PROTECTED, formal.type().type(),
	...
	/*
	 * ct - ParsedClassType of the inner class that we are dealing with. oct -
	 * ParsedClassType of the outer class.
	 */
	protected Formal produceOuterFormal(ParsedClassType ct, ParsedClassType oct) {
		String fn = outerThisName(ct);
		Formal formal = nf.Formal(Position.compilerGenerated(), Flags.NONE,
				nf.CanonicalTypeNode(Position.compilerGenerated(), oct), fn);
		formal = formal.localInstance(ts.localInstance(Position
				.compilerGenerated(), formal.flags(), formal.type().type(),
				formal.name()));
		return formal;
	}

	protected Field produceOuterField(ClassType ct, Expr rec) {
		Field f = nf.Field(Position.compilerGenerated(), rec,
				newFieldName(outerThisName(ct)));
		f = f.fieldInstance(ts.fieldInstance(Position.compilerGenerated(), ct,
				Flags.PROTECTED, ct.container(), // FIXME: use the type of outer
													// formal stored in cinfo?
				f.name()));
		return f;
	}
	...
	protected ConstructorCall produceDefaultSuperConstructorCall(ClassType ct) {
		ConstructorCall superCc = nf.ConstructorCall(
				Position.compilerGenerated(), ConstructorCall.SUPER,
				Collections.EMPTY_LIST);
		ConstructorInstance superCi = ts.constructorInstance(
				Position.compilerGenerated(), (ClassType) ct.superType(),
	...
	protected ConstructorDecl updateConstructor(ClassDecl cd, ClassType ct,
		...
		if (it.hasNext()) {
			...
			if (s instanceof ConstructorCall) {
				stmts.add(s);
				// If it calls another constructor in the same class, we don't
				// need to initialize the field.
				if (((ConstructorCall) s).kind() != ConstructorCall.THIS) {
					stmts.addAll(produceFieldInits(cinfo));
				}
			} else {
				// If there is no explicit constructor call, we need to add the
				// default
				// constructor call, and update it, in case it is from another
				// inner class,
		...
		ConstructorDecl newCons = nf.ConstructorDecl(
				Position.compilerGenerated(), cons.flags(), ct.name(), formals,
				cons.throwTypes(), b);
		newCons = newCons.constructorInstance(updateConstructorInst(ct,
				cons.constructorInstance(), cinfo));
		return newCons;
	}

	// Generate a list that contains all the field assignments for initializing
	// newly added fields.
	protected List produceFieldInits(ClassInfo cinfo) {
		...
		for (Iterator it = newFormals.iterator(); it.hasNext();) {
			...
			Field field = nf.Field(Position.compilerGenerated(), thisExpr,
					newFieldName(formal.name()));
			field = field.fieldInstance(ts.fieldInstance(
					Position.compilerGenerated(), cinfo.classType(),
					Flags.PROTECTED, formal.type().type(), field.name()));
			FieldAssign fAssign = nf.FieldAssign(Position.compilerGenerated(),
					field, Assign.ASSIGN, local);
