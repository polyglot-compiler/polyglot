...
//					t = nf.Field(Position.compilerGenerated(), t, newFieldName(outerThisName(tThis)));
	...
	protected ClassDecl addAnonymousConstructor(ClassDecl cd, ParsedClassType ct, ClassInfo cinfo, New newExpr) {
		List<Formal> formals = new ArrayList<Formal>(newExpr.arguments().size() + 1);
		...
		List<ClassMember> newMembers = new ArrayList<ClassMember>(members.size() + 1);
		newMembers.add(cons);
	...
	protected ConstructorCall updateConstructorCall(ConstructorCall cc, ClassInfo selfInfo) {
		...
		if (cc.kind().equals(ConstructorCall.THIS)) {
			...
			List<Expr> args = new ArrayList<Expr>(cc.arguments().size() + formals.size());
			...
			if (cinfo != null) {
				...
				if (cinfo.insideCode() == null) {
					...
					ci = updateConstructorInst(ct, ci, cinfo);
					List<Expr> args = new ArrayList<Expr>(cc.arguments().size() + 1);
					...
					outerLocal = outerLocal.localInstance(selfInfo.newConsFormals().get(0).localInstance());
...
//		}
		if (cinfo != null) {
			...
			List<Formal> formals = cinfo.newConsFormals();
			List<Expr> args = new ArrayList<Expr>(newExpr.arguments().size() + formals.size());
			List<Type> ftypes = new ArrayList<Type>(newExpr.arguments().size() + formals.size());
...
//		}
	...
	protected ConstructorInstance updateConstructorInst(ClassType ct, ConstructorInstance ci, ClassInfo cinfo) {
		List<Formal> newFormals = cinfo.newConsFormals();
		List<Type> ftypes = new ArrayList<Type>(ci.formalTypes().size() + newFormals.size());
		ftypes.addAll(ci.formalTypes());
	...
	protected ConstructorDecl updateConstructor(ClassDecl cd, ClassType ct, ConstructorDecl cons, ClassInfo cinfo) {
		List<Formal> newFormals = cinfo.newConsFormals();
		List<Formal> formals = new ArrayList<Formal>(cons.formals().size() + newFormals.size());
		formals.addAll(cons.formals());
		formals.addAll(newFormals);
		
		List<Stmt> oldStmts = cons.body().statements();
		List<Stmt> stmts = new ArrayList<Stmt>(oldStmts.size() + newFormals.size());
