...
/**
 * Visitor which checks that all local variables must be defined before use, and
 * that final variables and fields are initialized correctly.
 * 
 * The checking of the rules is implemented in the methods leaveCall(Node) and
 * check(FlowGraph, Term, Item, Item).
 * 
 ...
 * local variables are correctly initialized. (The implementation of the method
 * will probably call checkLocalInstanceInit to see if the local used is
 * initialized).
 * 
 * If language extensions have new constructs that assign to local variables,
 * they can override the method <code>flowOther</code> to capture the way the
...
public class InitChecker extends DataFlow {
	public InitChecker(Job job, TypeSystem ts, NodeFactory nf) {
		super(job, ts, nf, true /* forward analysis */, false /*
															 * perform dataflow
															 * when leaving
															 * CodeDecls, not
															 * when entering
	...
	protected static class ClassBodyInfo {
		 ...
		 * to MinMaxInitCounts. This Map is used as the basis for the Maps
		 * returned in createInitialItem().
		 * */
		...
		/**
		 * Map from ConstructorInstances to ConstructorInstances detailing which
		 * constructors call which constructors. This is used in checking the
		 ...
		 * which final non-static fields each constructor initializes. This is
		 * used in checking the initialization of final fields.
		 */
		public Map fieldsConstructorInitializes = new HashMap();

		/**
		 * Set of LocalInstances from the outer class body that were used during
		 * the declaration of this class. We need to track this in order to
		 * correctly populate <code>localsUsedInClassBodies</code>
		 */
		 ...
		 * declaration, and S is the set of LocalInstances that are defined in
		 * the current code declaration, but are used in the declaration of the
		 * class C. We need this information in order to ensure that these local
		 * variables are definitely assigned before the class declaration of C.
		 */
		public Map localsUsedInClassBodies = new HashMap();

		/**
		 * Set of LocalInstances that we have seen declarations for in this
		 * class. This set allows us to determine which local instances are
		 * simply being used before they are declared (if they are used in their
		 * own initialization) or are locals declared in an enclosing class.
		 */
		public Set localDeclarations = new HashSet();
	}

	/**
	 * Class representing the initialization counts of variables. The different
	 * values of the counts that we are interested in are ZERO, ONE and MANY.
	 */
	...
	/**
	 * Class to record counts of the minimum and maximum number of times a
	 * variable or field has been initialized or assigned to.
	 */
	protected static class MinMaxInitCount {
		...
		static MinMaxInitCount join(MinMaxInitCount initCount1,
			...
			MinMaxInitCount t = new MinMaxInitCount(InitCount.min(
					initCount1.getMin(), initCount2.getMin()), InitCount.max(
					initCount1.getMax(), initCount2.getMax()));
			return t;
	...
	/**
	 * Dataflow items for this dataflow are maps of VarInstances to counts of
	 * the min and max number of times those variables/fields have been
	 * initialized. These min and max counts are then used to determine if
	 * variables have been initialized before use, and that final variables are
	 * not initialized too many times.
	 * 
	 ...
	 *         declaration; otherwise, an apropriately initialized FlowGraph.
	 */
		...
		if (n instanceof ClassBody) {
			...
			if (ct == null) {
				throw new InternalCompilerError(
						"ClassBody found but cannot find the class.",
						n.position());
			}
	 ...
	 * declaration is encountered, to ensure that all initializers are processed
	 * first.
	 * 
	...
	protected Node leaveCall(Node old, Node n, NodeVisitor v)
		...
		if (n instanceof ClassBody) {
			...
			try {
				for (Iterator iter = currCBI.allConstructors.iterator(); iter
						.hasNext();) {
			...
			if (cm instanceof FieldDecl) {
				...
				if (fd.flags().isFinal()) {
					...
					if (fd.init() != null) {
						...
						initCount = new MinMaxInitCount(InitCount.ZERO,
								InitCount.ZERO);
					}
	...
	protected void checkStaticFinalFieldsInit(ClassBody cb)
		...
		for (Iterator iter = currCBI.currClassFinalFieldInitCounts.entrySet()
	...
	protected void checkNonStaticFinalFieldsInit(ClassBody cb)
		...
		// constructors intialize it exactly once, taking into account
		// constructor calls.
		for (Iterator iter = currCBI.currClassFinalFieldInitCounts.keySet()
				.iterator(); iter.hasNext();) {
			...
			if (fi.flags().isFinal() && !fi.flags().isStatic()) {
				...
				MinMaxInitCount ic = (MinMaxInitCount) currCBI.currClassFinalFieldInitCounts
						.get(fi.orig());
				if (ic != null && !InitCount.ZERO.equals(ic.getMin())) {
					fieldInitializedBeforeConstructors = true;
				}

				for (Iterator iter2 = currCBI.allConstructors.iterator(); iter2
						.hasNext();) {
					...
					while (ci != null) {
						Set s = (Set) currCBI.fieldsConstructorInitializes
								.get(ci.orig());
						...
						ci = (ConstructorInstance) currCBI.constructorCalls
								.get(ci.orig());
					}
					if (!isInitialized) {
						throw new SemanticException("Final field \""
	 ...
	 * <code>post(FlowGraph, Term)</code> after <code>dataflow(FlowGraph)</code>
	 * has been called. There is no need to push a CFG onto the stack, as
	...
	private DataFlowItem createInitDFI() {
		return new DataFlowItem(new HashMap(
				currCBI.currClassFinalFieldInitCounts));
	}

	/**
	 * The confluence operator for <code>Initializer</code>s and
	 * <code>Constructor</code>s needs to be a little special, as we are only
	 * concerned with non-exceptional flows in these cases. This method ensures
	 * that a slightly different confluence is performed for these
	 * <code>Term</code>s, otherwise <code>confluence(List, Term)</code> is
	 * called instead.
	 */
	 ...
	 * However, if two or more of the initCount maps from the inItems each have
	 * a MinMaxInitCounts entry for the same VarInstance, the conflict must be
	 * resolved, by using the minimum of all mins and the maximum of all maxs.
	 */
	public Item confluence(List inItems, Term node, boolean entry,
			FlowGraph graph) {
	 ...
	 * 
	 * To summarize: - Formals: declaration of a Formal param, just insert a new
	 * MinMaxInitCount for the LocalInstance. - LocalDecl: a declaration of a
	 * local variable, just insert a new MinMaxInitCount for the LocalInstance
	 * as appropriate based on whether the declaration has an initializer or
	 * not. - Assign: if the LHS of the assign is a local var or a field that we
	 * are interested in, then increment the min and max counts for that local
	 * var or field.
	 */
	public Map flow(Item trueItem, Item falseItem, Item otherItem,
		...
		if (n instanceof Formal) {
			...
			ret = flowLocalAssign(inDFItem, graph, (LocalAssign) n,
					succEdgeKeys);
		} else if (n instanceof FieldAssign) {
			// assignment to a field
			ret = flowFieldAssign(inDFItem, graph, (FieldAssign) n,
					succEdgeKeys);
		} else if (n instanceof ConstructorCall) {
			// call to another constructor.
			ret = flowConstructorCall(inDFItem, graph, (ConstructorCall) n,
					succEdgeKeys);
	 ...
	 * parameter
	 */
	protected Map flowFormal(DataFlowItem inItem, FlowGraph graph, Formal f,
			Set succEdgeKeys) {
	 ...
	 * variable
	 */
	protected Map flowLocalDecl(DataFlowItem inItem, FlowGraph graph,
			LocalDecl ld, Set succEdgeKeys) {
		Map m = new HashMap(inItem.initStatus);
	 ...
	 * variable
	 */
	protected Map flowLocalAssign(DataFlowItem inItem, FlowGraph graph,
			LocalAssign a, Set succEdgeKeys) {
	 ...
	 */
	protected Map flowFieldAssign(DataFlowItem inItem, FlowGraph graph,
			FieldAssign a, Set succEdgeKeys) {
	 ...
	 */
	protected Map flowConstructorCall(DataFlowItem inItem, FlowGraph graph,
			ConstructorCall cc, Set succEdgeKeys) {
	 ...
	 */
	protected Map flowOther(DataFlowItem inItem, FlowGraph graph, Node n,
			Set succEdgeKeys) {
		return null;
	}

	/**
	 * Determine if we are interested in this field on the basis of the target
	 * of the field. To wit, if the field is static, then the target of the
	 * field must be the current class; if the field is not static then the
	 * target must be "this".
	 */
	 ...
	 * 
	 * To summarize the conditions: - Local variables must be initialized before
	 * use, (i.e. min count > 0) - Final local variables (including Formals)
	 * cannot be assigned to more than once (i.e. max count <= 1) - Final
	 * non-static fields whose target is this cannot be assigned to more than
	 * once - Final static fields whose target is the current class cannot be
	...
	public void check(FlowGraph graph, Term n, boolean entry, Item inItem,
		...
		if (n == graph.root() && !entry) {
			if (currCBI.currCodeDecl instanceof Initializer) {
				finishInitializer(graph, (Initializer) currCBI.currCodeDecl,
						dfIn, dfOut);
			}
			if (currCBI.currCodeDecl instanceof ConstructorDecl) {
				finishConstructorDecl(graph,
						(ConstructorDecl) currCBI.currCodeDecl, dfIn, dfOut);
			}
		}
	}
	 ...
	 */
	protected void finishInitializer(FlowGraph graph, Initializer initializer,
			DataFlowItem dfIn, DataFlowItem dfOut) {
	 ...
	 * .
	 */
	protected void finishConstructorDecl(FlowGraph graph, ConstructorDecl cd,
			DataFlowItem dfIn, DataFlowItem dfOut) {
		...
		while (iter.hasNext()) {
			...
			if (e.getKey() instanceof FieldInstance
				...
				FieldInstance fi = (FieldInstance) e.getKey();
				MinMaxInitCount initCount = (MinMaxInitCount) e.getValue();
				MinMaxInitCount origInitCount = (MinMaxInitCount) currCBI.currClassFinalFieldInitCounts
						.get(fi);
	 ...
	 */
	protected void checkLocal(FlowGraph graph, Local l, DataFlowItem dfIn,
		...
		if (!currCBI.localDeclarations.contains(l.localInstance().orig())) {
			...
			MinMaxInitCount initCount = (MinMaxInitCount) dfIn.initStatus.get(l
					.localInstance().orig());
	 ...
	 */
	protected void checkLocalAssign(FlowGraph graph, LocalAssign a,
	 ...
	 */
	protected void checkFieldAssign(FlowGraph graph, FieldAssign a,
			DataFlowItem dfIn, DataFlowItem dfOut) throws SemanticException {
		...
		if (fi.flags().isFinal()) {
			if ((currCBI.currCodeDecl instanceof ConstructorDecl || currCBI.currCodeDecl instanceof Initializer)
				...
				MinMaxInitCount initCount = (MinMaxInitCount) dfOut.initStatus
						.get(fi.orig());
				if (initCount == null) {
	...
	/**
	 * Check that the set of <code>LocalInstance</code>s <code>localsUsed</code>
	 * , which is the set of locals used in the inner class declared by
	 * <code>cb</code> are initialized before the class declaration.
	 * 
	 * @throws SemanticException
	 */
	protected void checkClassBody(FlowGraph graph, ClassBody cb,
			DataFlowItem dfIn, DataFlowItem dfOut) throws SemanticException {
	 ...
	 * <code>cb</code> are initialized before the class declaration.
	 */
	protected void checkLocalsUsedByInnerClass(FlowGraph graph, ClassBody cb,
			Set localsUsed, DataFlowItem dfIn, DataFlowItem dfOut)
			throws SemanticException {
	 ...
	 */
	protected void checkOther(FlowGraph graph, Node n, DataFlowItem dfIn,
			DataFlowItem dfOut) throws SemanticException {
	}
}
