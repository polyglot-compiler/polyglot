...
public class InnerClassRemover extends ContextVisitor {
	...
	/**
	 * Get a reference to the enclosing instance of the current class that is of
	 * type containerClass
	 */
	...
	protected Node leaveCall(Node old, Node n, NodeVisitor v)
		...
		if (n instanceof Special) {
			...
			if (s.qualifier().type().toClass().declaration() == context
					.currentClassScope())
				return s;
			Node ret = getContainer(pos,
					nf.This(pos).type(context.currentClass()),
					context.currentClass(), s.qualifier().type().toClass());
			return ret;
		}
		...
		if (n instanceof New) {
			...
			if (q != null) {
				...
				// Fix the ci if a copy; otherwise, let the ci be modified at
		...
		if (n instanceof ConstructorCall) {
			...
			// NOTE: we require that a constructor call to a non-static member
			// have a qualifier.
			// We can't check for this now, though, since the type information
			// may already have been
			// rewritten.

			// // Add a qualifier to non-static member class super() calls if
			// not present.
			...
			boolean fixCI = cc.arguments().size() + 1 != ci.formalTypes()
					.size();
			...
			// Fix the ci if a copy; otherwise, let the ci be modified at the
			// declaration node.
			if (ci != cidecl && fixCI) {
				List args = new ArrayList();
				args.add(ci.container());
		...
		if (n instanceof ClassDecl) {
			...
			if (cd.type().isMember() && !cd.type().flags().isStatic()) {
				...
				cd = addFieldsToClass(cd, Collections.singletonList(fi), ts,
						nf, true);
	...
	public static ClassDecl addFieldsToClass(ClassDecl cd, List newFields,
			TypeSystem ts, NodeFactory nf, boolean rewriteMembers) {
		...
		for (Iterator i = b.members().iterator(); i.hasNext();) {
			...
			if (m instanceof ConstructorDecl) {
				...
				for (Iterator j = newFields.iterator(); j.hasNext();) {
					...
					Position pos = fi.position();
					LocalInstance li = ts.localInstance(pos, Flags.FINAL,
							fi.type(), fi.name());
					li.setNotConstant();
					Formal formal = nf.Formal(pos, li.flags(),
				...
				for (int j = 0; j < newFields.size(); j++) {
					...
					LocalInstance li = ((Formal) formals.get(j))
							.localInstance();

					Position pos = fi.position();

					Field f = nf.Field(pos, nf.This(pos).type(fi.container()),
							nf.Id(pos, fi.name()));
					f = (Field) f.type(fi.type());
				...
				// Add the assignments to the constructor body after the super
				// call.
				// Or, add pass the locals to another constructor if a this
				// call.
				Block block = td.body();
				if (block.statements().size() > 0) {
					...
					if (s0 instanceof ConstructorCall) {
						...
						if (cc.kind() == ConstructorCall.THIS) {
							...
							for (Iterator j = statements.iterator(); j
									.hasNext();) {
								Stmt si = (Stmt) j.next();
								Eval e = (Eval) si;
							...
							// Modify the CI if it is a copy of the declaration
							// CI.
							// If not a copy, it will get modified at the
							// declaration.
							if (ci != ci.declaration()) {
								List newFormalTypes = new ArrayList();
								for (int j = 0; j < newFields.size(); j++) {
									FieldInstance fi = (FieldInstance) newFields
											.get(j);
							...
							// A super call. Don't rewrite it here; the visitor
					...
					statements.addAll(block.statements().subList(1,
	...
	private FieldInstance boxThis(ClassType currClass, ClassType outerClass) {
		...
		fi = ts.fieldInstance(pos, currClass, Flags.FINAL.Private(),
				outerClass, OUTER_FIELD_NAME);
		fi.setNotConstant();
