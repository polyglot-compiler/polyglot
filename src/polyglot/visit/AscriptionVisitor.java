 ...
 * modifications.
 * 
 * The major advantage of this visitor is the new <code>ascribe()</code> method,
 * which allows AST translations based on the expression and also the type that
 * is expected. For the base translation (standard Java), the type of the
 * expression and the type that is expceted are the same. Language extensions
 * however may not have this property, and can take advantage of the
 * <code>ascribe</code> method to transform the AST into a different form that
 * will pass Java type-checking.
 * 
 * @see #ascribe
 */
public class AscriptionVisitor extends ContextVisitor {
	 ...
	 * <code>ascribe()</code>. Other than that, plays the same role as the
	 * <code>enterCall</code> method in <code>ErrorHandlingVisitor</code>.
	 */
	...
	/**
	 * The <code>ascribe()</code> method is called for each expression and is
	 * passed the type the expression is <i>used at</i> rather than the type the
	 * type checker assigns to it.
	 * 
	 ...
	 * @return The new translated Expr node, or if nothing has changed, just e.
	 */
