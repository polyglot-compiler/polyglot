...
public class Import_c extends Node_c implements Import {
    ...
    public Node typeCheck(TypeChecker tc) throws SemanticException {
        if (kind == TYPE_IMPORT_ON_DEMAND && tc.typeSystem().packageExists(name)) {
        ...
        if (!tc.typeSystem().packageExists(pkgName)) {
            throw new SemanticException("Package \"" + pkgName + "\" not found.", position());
        }
        ...
        if (nt instanceof Type) {
            ...
            if (t.isClass()
                    && !tc.typeSystem()
                            .classAccessibleFromPackage(t.toClass(), tc.context().package_())) {
