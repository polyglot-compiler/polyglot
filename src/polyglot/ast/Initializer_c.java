...
/**
 * An <code>Initializer</code> is an immutable representation of an initializer
 * block in a Java class (which appears outside of any method). Such a block is
 * executed before the code for any of the constructors. Such a block can
 * optionally be static, in which case it is executed when the class is loaded.
 */
public class Initializer_c extends Term_c implements Initializer {
	...
	public NodeVisitor exceptionCheckEnter(ExceptionChecker ec)
		...
		if (!initializerInstance().container().toClass().isAnonymous()) {
			ec = ec.push(new ExceptionChecker.CodeTypeReporter(
			...
			for (Iterator iter = container.constructors().iterator(); iter
					.hasNext();) {
