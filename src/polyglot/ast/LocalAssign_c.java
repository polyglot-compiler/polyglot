...
/**
 * A <code>LocalAssign_c</code> represents a Java assignment expression to a
 * local variable. For instance, <code>x = e</code>.
 * 
...
public class LocalAssign_c extends Assign_c implements LocalAssign {
	...
	private void assertLeftType() {
		if (!(left() instanceof Local)) {
			throw new InternalCompilerError(
					"left expression of an LocalAssign must be a local");
		}
	}
	...
	protected void acceptCFGOpAssign(CFGBuilder v) {
		 ...
		 * Local l = (Local)left();
		 * 
		 * // l OP= e: visit l -> e -> (l OP= e) v.visitThrow(l); v.edge(l,
		 * right().entry()); v.visitCFG(right(), this);
		 */
