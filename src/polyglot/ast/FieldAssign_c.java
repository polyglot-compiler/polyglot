...
/**
 * A <code>FieldAssign_c</code> represents a Java assignment expression to a
 * field. For instance, <code>this.x = e</code>.
 * 
...
public class FieldAssign_c extends Assign_c implements FieldAssign {
	...
	private void assertLeftType() {
		if (!(left() instanceof Field)) {
			throw new InternalCompilerError(
					"left expression of an FieldAssign must be a field");
		}
	}
	...
	protected void acceptCFGOpAssign(CFGBuilder v) {
		/*
		 * Field f = (Field)left(); if (f.target() instanceof Expr) { Expr o =
		 * (Expr) f.target();
		 * 
		 * // o.f OP= e: visit o -> o.f -> e -> (o.f OP= e) v.visitCFG(o, f);
		 * v.visitThrow(f); v.edge(f, right().entry()); v.visitCFG(right(),
		 * this); } else { // T.f OP= e: visit T.f -> e -> (T.f OP= e)
		 * v.visitThrow(f); v.edge(f, right().entry()); v.visitCFG(right(),
		 * this); }
		 */
