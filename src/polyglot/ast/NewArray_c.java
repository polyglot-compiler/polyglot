...
public class NewArray_c extends Expr_c implements NewArray {
	...
	public NewArray_c(Position pos, TypeNode baseType, List dims, int addDims,
			ArrayInit init) {
		super(pos);
		assert (baseType != null && dims != null); // init may be null
		assert (addDims >= 0);
		assert (!dims.isEmpty() || init != null); // dims may be empty only if
													// there is an initializer
		assert (addDims > 0 || init == null); // init may be non-null only if
												// addDims > 0
	...
	protected NewArray_c reconstruct(TypeNode baseType, List dims,
			ArrayInit init) {
	...
	public List throwTypes(TypeSystem ts) {
		if (dims != null && !dims.isEmpty()) {
			...
			try {
				return CollectionUtil.list(ts
						.typeForName("java.lang.NegativeArraySizeException"));
	...
	public Node copy(NodeFactory nf) {
		return nf.NewArray(this.position, this.baseType, this.dims,
				this.addDims, this.init);
	}
