...
/**
 * A <code>Cast</code> is an immutable representation of a casting operation. It
 * consists of an <code>Expr</code> being cast and a <code>TypeNode</code> being
 * cast to.
 */
public class Cast_c extends Expr_c implements Cast {
	...
	public Object constantValue() {
		...
		if (v instanceof Boolean) {
			if (castType.type().isBoolean())
				return v;
		}

		if (v instanceof String) {
			TypeSystem ts = castType.type().typeSystem();
			if (castType.type().typeEquals(ts.String()))
				return v;
		}
		...
		if (v instanceof Double) {
			...
			if (castType.type().isDouble())
				return new Double((double) vv);
			if (castType.type().isFloat())
				return new Float((float) vv);
			if (castType.type().isLong())
				return new Long((long) vv);
			if (castType.type().isInt())
				return new Integer((int) vv);
			if (castType.type().isChar())
				return new Character((char) vv);
			if (castType.type().isShort())
				return new Short((short) vv);
			if (castType.type().isByte())
