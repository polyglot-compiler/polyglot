...
/**
 * A <code>Field</code> is an immutable representation of a Java field access.
 * It consists of field name and may also have either a <code>Type</code> or an
 * <code>Expr</code> containing the field being accessed.
 */
public class Field_c extends Expr_c implements Field {
	...
	public Node buildTypes(TypeBuilder tb) throws SemanticException {
		...
		FieldInstance fi = ts.fieldInstance(position(), tb.currentClass(),
				Flags.NONE, ts.unknownType(position()), name.id());
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
		...
		if (target.type().isReference()) {
			FieldInstance fi = ts.findField(target.type().toReference(),
					name.id(), c.currentClass());
		...
		throw new SemanticException("Cannot access field \"" + name.id()
				...
				+ "of non-reference type \"" + target.type() + "\".",
				target.position());
	}
	...
	protected void checkConsistency(Context c) {
		if (targetImplicit) {
			...
			throw new InternalCompilerError("Field " + this + " has an "
					+ "implicit target, but the name " + name.id()
					+ " resolves to " + vi + " instead of " + target,
					position());
		}
	}
