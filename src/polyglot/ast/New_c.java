 ...
 * <code>new</code> operator to create a new instance of a class. In addition to
 * the type of the class being created, a <code>New</code> has a list of
 * arguments to be passed to the constructor of the object and an optional
 * <code>ClassBody</code> used to support anonymous classes.
 */
public class New_c extends Expr_c implements New {
	...
	public New_c(Position pos, Expr qualifier, TypeNode tn, List arguments,
			ClassBody body) {
		super(pos);
		assert (tn != null && arguments != null); // qualifier and body may be
													// null
	...
	protected New_c reconstruct(Expr qualifier, TypeNode tn, List arguments,
			ClassBody body) {
	...
	public NodeVisitor buildTypesEnter(TypeBuilder tb) throws SemanticException {
		if (body != null) {
			 ...
			 * // get around to visiting it in the buildTypes method. // We do
			 * this because we need to visit the body of the anonymous // class
			 * after we've pushed an anon class onto the type builder, // but we
			 * need to check the arguments, and qualifier, etc. outside // of
			 * the scope of the anon class. return tb.bypass(body);
			 */
	...
	public Node buildTypes(TypeBuilder tb) throws SemanticException {
		...
		ConstructorInstance ci = ts.constructorInstance(position(),
				tb.currentClass(), Flags.NONE, l, Collections.EMPTY_LIST);
		...
		if (n.body() != null) {
			 ...
			 * 
			 * // push an anonymous class on the stack. bodyTB =
			 * bodyTB.pushAnonClass(position());
			 * 
	...
	public Node disambiguateOverride(Node parent, AmbiguityRemover ar)
		...
		if (nn.qualifier() == null) {
			nn = nn.objectType((TypeNode) nn.visitChild(nn.objectType(),
					childbd));
			...
			if (nn.objectType() instanceof AmbTypeNode
				...
				// We have to disambiguate the type node as if it were a member
				// of the
				...
				// extensions (e.g., PolyJ), the type node may be more complex
				// than
				...
				if (nn.qualifier().isDisambiguated()
					...
					if (!nn.qualifier().type().isClass()) {
						throw new SemanticException(
								"Cannot instantiate member class of non-class type.",
								nn.position());
					}

					ClassType outer = nn.qualifier().type().toClass();
					ClassType ct = ts.findMemberClass(outer, name,
							c.currentClass());
					TypeNode tn = nf.CanonicalTypeNode(nn.objectType()
							.position(), ct);
					nn = nn.objectType(tn);
				}
	...
	protected New findQualifier(AmbiguityRemover ar, ClassType ct)
		...
		if (outer == null) {
			throw new SemanticException(
					"Could not find non-static member class \"" + name + "\".",
					position());
		}
	...
	public Node typeCheckOverride(Node parent, TypeChecker tc)
		...
		if (nn.qualifier() != null) {
			...
			// Force the object type and class body, if any, to be
			// disambiguated.
	...
	public Node copy(NodeFactory nf) {
		return nf.New(this.position, this.qualifier, this.tn, this.arguments,
				this.body);
	}
