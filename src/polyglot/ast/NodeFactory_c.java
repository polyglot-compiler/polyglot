...
public class NodeFactory_c implements NodeFactory
{
	...
	return new AmbPrefix_c(new Ext_c(), pos, prefix, name);
    }
	...
	return new AmbReceiver_c(new Ext_c(), pos, prefix, name);
    }
	...
	return new AmbQualifierNode_c(new Ext_c(), pos, qualifier, name);
    }

    public AmbExpr AmbExpr(Position pos, String name) {
	return new AmbExpr_c(new Ext_c(), pos, name);
    }
	...
	return new AmbTypeNode_c(new Ext_c(), pos, qualifier, name);
    }

    public ArrayAccess ArrayAccess(Position pos, Expr base, Expr index) {
	return new ArrayAccess_c(new Ext_c(), pos, base, index);
    }

    public ArrayInit ArrayInit(Position pos) {
	return new ArrayInit_c(new Ext_c(), pos, Collections.EMPTY_LIST);
    }

    public ArrayInit ArrayInit(Position pos, List elements) {
	return new ArrayInit_c(new Ext_c(), pos, elements);
    }

    public Assign Assign(Position pos, Expr left, Assign.Operator op, Expr right) {
	return new Assign_c(new Ext_c(), pos, left, op, right);
    }

    public Binary Binary(Position pos, Expr left, Binary.Operator op, Expr right) {
	return new Binary_c(new Ext_c(), pos, left, op, right);
    }

    public Block Block(Position pos) {
	return new Block_c(new Ext_c(), pos, Collections.EMPTY_LIST);
    }
	...
	return new Block_c(new Ext_c(), pos, statements);
    }

    public SwitchBlock SwitchBlock(Position pos, List statements) {
	return new SwitchBlock_c(new Ext_c(), pos, statements);
    }

    public BooleanLit BooleanLit(Position pos, boolean value) {
	return new BooleanLit_c(new Ext_c(), pos, value);
    }
	...
	return new Branch_c(new Ext_c(), pos, kind, label);
    }
	...
	return new Call_c(new Ext_c(), pos, target, name, args);
    }
	...
	return new ConstructorCall_c(new Ext_c(), pos, kind, outer, args);
    }

    public ConstructorDecl ConstructorDecl(Position pos, Flags flags, String name, List formals, List exceptionTypes, Block body) {
	return new ConstructorDecl_c(new Ext_c(), pos, flags, name, formals, exceptionTypes, body);
    }
	...
	return new FieldDecl_c(new Ext_c(), pos, flags, type, name, init);
    }

    public Do Do(Position pos, Stmt body, Expr cond) {
	return new Do_c(new Ext_c(), pos, body, cond);
    }

    public Empty Empty(Position pos) {
	return new Empty_c(new Ext_c(), pos);
    }

    public Eval Eval(Position pos, Expr expr) {
	return new Eval_c(new Ext_c(), pos, expr);
    }
	...
	return new LocalDecl_c(new Ext_c(), pos, flags, type, name, init);
    }
	...
	return new MethodDecl_c(new Ext_c(), pos, flags, returnType, name, formals, exceptionTypes, body);
    }
    ...
    public New New(Position pos, Expr outer, TypeNode objectType, List args, ClassBody body) {
        return new New_c(new Ext_c(), pos, outer, objectType, args, body);
    }
	...
	return new NewArray_c(new Ext_c(), pos, base, dims, addDims, null);
    }

    public NewArray NewArray(Position pos, TypeNode base, int addDims, ArrayInit init) {
	return new NewArray_c(new Ext_c(), pos, base, Collections.EMPTY_LIST, addDims, init);
    }

    public NullLit NullLit(Position pos) {
	return new NullLit_c(new Ext_c(), pos);
    }
	...
	return new SourceFile_c(new Ext_c(), pos, packageName, imports, decls);
    }
	...
	return new Special_c(new Ext_c(), pos, kind, outer);
    }

    public StringLit StringLit(Position pos, String value) {
	return new StringLit_c(new Ext_c(), pos, value);
    }

    public Switch Switch(Position pos, Expr expr, List elements) {
	return new Switch_c(new Ext_c(), pos, expr, elements);
    }

    public Synchronized Synchronized(Position pos, Expr expr, Block body) {
	return new Synchronized_c(new Ext_c(), pos, expr, body);
    }

    public Throw Throw(Position pos, Expr expr) {
	return new Throw_c(new Ext_c(), pos, expr);
    }
	...
	return new Try_c(new Ext_c(), pos, tryBlock, catchBlocks, finallyBlock);
    }

    public ArrayTypeNode ArrayTypeNode(Position pos, TypeNode base) {
        return new ArrayTypeNode_c(new Ext_c(), pos, base);
    }
    ...
    public CanonicalTypeNode CanonicalTypeNode(Position pos, Type type) {
        ...
        return new CanonicalTypeNode_c(new Ext_c(), pos, type);
    }
    ...
    */
	...
	return new PackageNode_c(new Ext_c(), pos, p);
    }

    public Unary Unary(Position pos, Unary.Operator op, Expr expr) {
	return new Unary_c(new Ext_c(), pos, op, expr);
    }
	...
	return new While_c(new Ext_c(), pos, cond, body);
    }
}
