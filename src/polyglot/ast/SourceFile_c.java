...
/**
 * A <code>SourceFile</code> is an immutable representations of a Java langauge
 * source file. It consists of a package name, a list of <code>Import</code>s,
 * and a list of <code>GlobalDecl</code>s.
 */
public class SourceFile_c extends Node_c implements SourceFile {
	...
	public SourceFile_c(Position pos, PackageNode package_, List imports,
			List decls) {
		super(pos);
		assert (imports != null && decls != null && !decls.isEmpty()); // package_
																		// may
																		// be
																		// null,
																		// imports
	...
	protected SourceFile_c reconstruct(PackageNode package_, List imports,
			List decls) {
	...
	public Node disambiguateOverride(Node parent, AmbiguityRemover ar)
		 ...
		 * OuterScopeDisambiguator osd = new OuterScopeDisambiguator(ar); n =
		 * (SourceFile) osd.visitEdgeNoOverride(parent, n); if (osd.hasErrors())
		 * throw new SemanticException();
		 * 
		 * // Ensure supertyperts and signatures are disambiguated for all //
		 * classes visible from the outer scope. SupertypeDisambiguator sud =
		 * new SupertypeDisambiguator(ar); n = (SourceFile)
		 * sud.visitEdgeNoOverride(parent, n); if (sud.hasErrors()) throw new
		 * SemanticException();
		 * 
		 * SignatureDisambiguator sid = new SignatureDisambiguator(ar); n =
		 * (SourceFile) sid.visitEdgeNoOverride(parent, n); if (sid.hasErrors())
		 * throw new SemanticException();
		 * 
		 * // Now type check the children. n = (SourceFile)
		 * ar.visitEdgeNoOverride(parent, n); if (ar.hasErrors()) throw new
		 * SemanticException();
		 * 
	...
	public Node copy(NodeFactory nf) {
		return nf.SourceFile(this.position, this.package_, this.imports,
