...
public class Try_c extends Stmt_c implements Try {
	...
	public Try_c(Position pos, Block tryBlock, List catchBlocks,
			Block finallyBlock) {
		super(pos);
		assert (tryBlock != null && catchBlocks != null); // finallyBlock may be
															// null, catchBlocks
															// empty
		assert (!catchBlocks.isEmpty() || finallyBlock != null); // must be
																	// either
																	// try-catch
																	// or
																	// try(-catch)-finally
		this.tryBlock = tryBlock;
		this.catchBlocks = TypedList.copyAndCheck(catchBlocks, Catch.class,
				true);
	...
	protected Try_c reconstruct(Block tryBlock, List catchBlocks,
			Block finallyBlock) {
		if (tryBlock != this.tryBlock
			...
			Try_c n = (Try_c) copy();
			n.tryBlock = tryBlock;
			n.catchBlocks = TypedList.copyAndCheck(catchBlocks, Catch.class,
					true);
			n.finallyBlock = finallyBlock;
			return n;
		}
	...
	/**
	 * Bypass all children when peforming an exception check. exceptionCheck(),
	 * called from ExceptionChecker.leave(), will handle visiting children.
	 */
	...
	/**
	 * Performs exceptionChecking. This is a special method that is called via
	 * the exceptionChecker's override method (i.e, doesn't follow the standard
	 * model for visitation.
	 * 
	 * @param ec
	 *            The ExceptionChecker that was run against the child node. It
	 *            contains the exceptions that can be thrown by the try block.
	 */
	public Node exceptionCheck(ExceptionChecker ec) throws SemanticException {
		...
		for (ListIterator i = this.catchBlocks.listIterator(this.catchBlocks
				.size()); i.hasPrevious();) {
	...
	public Node copy(NodeFactory nf) {
		return nf.Try(this.position, this.tryBlock, this.catchBlocks,
				this.finallyBlock);
	}
