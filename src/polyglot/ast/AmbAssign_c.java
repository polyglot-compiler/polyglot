...
/**
 * A <code>AmbAssign</code> represents a Java assignment expression to an as yet
 * unknown expression.
 */
public class AmbAssign_c extends Assign_c implements AmbAssign {
	...
	public Node disambiguate(AmbiguityRemover ar) throws SemanticException {
		...
		if (n.left() instanceof Local) {
			return ar.nodeFactory().LocalAssign(n.position(), (Local) left(),
					operator(), right());
		} else if (n.left() instanceof Field) {
			return ar.nodeFactory().FieldAssign(n.position(), (Field) left(),
					operator(), right());
		} else if (n.left() instanceof ArrayAccess) {
			return ar.nodeFactory().ArrayAccessAssign(n.position(),
					(ArrayAccess) left(), operator(), right());
		}
