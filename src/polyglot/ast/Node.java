...
public interface Node extends JL, Copy, java.io.Serializable {
	...
	/**
	 * Get the position of the node in the source file. Returns null if the
	 * position is not set.
	 */
	Position position();
	...
	/**
	 * @return true if the all type information for the node (but not
	 *         necessarily for the node's children) is unambiguous.
	 */
	boolean isDisambiguated();

	/**
	 * @return true if all the type information for the node (but not
	 *         necessarily for the node's children) has been computed.
	 */
	...
	/**
	 * Visit the node, passing in the node's parent. This method is called by a
	 * <code>NodeVisitor</code> to traverse the AST starting at this node. This
	 * method should call the <code>override</code>, <code>enter</code>, and
	 * <code>leave<code> methods of the visitor.  The method may return a
	 * new version of the node.
	 * 
	 * @param parent
	 *            The parent of <code>this</code> in the AST.
	 * @param v
	 *            The visitor which will traverse/rewrite the AST.
	 * @return A new AST if a change was made, or <code>this</code>.
	 * 
	...
	/**
	 * Get the expected type of a child expression of <code>this</code>. The
	 * expected type is determined by the context in that the child occurs
	 * (e.g., for <code>x = e</code>, the expected type of <code>e</code> is the
	 * declared type of <code>x</code>.
	 * 
	 * The expected type should impose the least constraints on the child's type
	 * that are allowed by the parent node.
	 * 
	 * @param child
	 *            A child expression of this node.
	 * @param av
