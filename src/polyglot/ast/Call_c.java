...
public class Call_c extends Expr_c implements Call
{
	...
	public Call_c(Ext ext, Position pos, Receiver target, String name, List arguments) {
		super(ext, pos);
		this.target = target;
		this.name = name;
		...
		* in a static context. */
		...
		if (target instanceof TypeNode) {
			...
			throw new SemanticException(
				"Receiver of method invocation must be a reference type.",
				target.position());
		} else { // target == null
		...
		if (staticContext && !mi.flags().isStatic()) {
			...
			throw new SemanticException(
				"Cannot call non-static method " + name +
				" of " + containingClass + " in static context.");
		}
		...
		if (target == null) {
			...
			if (mi.flags().isStatic()) {
				...
				// brought the method into scope.  This is different
