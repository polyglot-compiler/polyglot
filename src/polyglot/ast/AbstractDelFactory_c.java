...
/**
 * This abstract implementation of <code>DelFactory</code> provides a way of
 * chaining together DelFactories, and default implementations of factory
 ...
 * The method <code>delN()</code> calls <code>delNImpl()</code> to create an
 * appropriate delegate object, and if other <code>DelFactory</code>s are
 * chained onto this one, it will also call <code>delN()</code> on the next
 * <code>DelFactory</code>. The method <code>delN()</code> will then call
 * <code>postDelN</code>, passing in the newly created extension object.
 * 
 * <p>
 * The default implementation of <code>delNImpl()</code> is to simply call
 * <code>delMImpl()</code>, where <code>M</code> is the immediate superclass of
...
public abstract class AbstractDelFactory_c implements DelFactory {
	...
	/**
	 * Compose two delegates together. Order is important: e1 gets added at the
	 * end of e2's chain.
	 * 
	 * @param e1
	 *            the <code>JL</code> object to add to the end of e2's chain of
	 *            delegates.
	 * @param e2
	 *            the second <code>JL</code> object that will have e1 added to
	...
	protected JL composeDels(JL e1, JL e2) {
		...
		throw new InternalCompilerError(
				"Composition of delegates unimplemented.");
