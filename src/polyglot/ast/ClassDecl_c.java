...
public class ClassDecl_c extends Term_c implements ClassDecl {
	...
	public ClassDecl_c(Position pos, Flags flags, Id name, TypeNode superClass,
		...
		super(pos);
		assert (flags != null && name != null && interfaces != null && body != null); // superClass
																						// may
																						// be
																						// null,
																						// interfaces
		...
		this.interfaces = TypedList.copyAndCheck(interfaces, TypeNode.class,
				true);
		this.body = body;
	}
	...
	protected ClassDecl_c disambiguateSupertypes(AmbiguityRemover ar)
		...
		if (!type.supertypesResolved()) {
			...
			for (Iterator i = interfaces.iterator(); supertypesResolved
					&& i.hasNext();) {
	...
	protected Node addDefaultConstructor(TypeSystem ts, NodeFactory nf)
		...
		if (ci == null) {
			throw new InternalCompilerError(
					"addDefaultConstructor called without defaultCI set");
		}
		...
		ConstructorDecl cd = nf.ConstructorDecl(body().position().startOf(),
				Flags.PUBLIC, name, Collections.EMPTY_LIST,
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
		if (this.type().isNested() && (this.type() instanceof Named)) {
			// The class cannot have the same simple name as any enclosing
			// class.
		...
		if (type.superType() != null) {
			...
			if (this.type.equals(tc.typeSystem().Object())
					|| this.type.fullName().equals(
							tc.typeSystem().Object().fullName())) {
	...
	public Node disambiguateOverride(Node parent, AmbiguityRemover ar)
			throws SemanticException {
		/*
		 * // Don't do anything special for member classes; the disambiguation
		 * passes // for the container have already been run and visited this
		 * class. if (type.isMember()) { return null; }
		 * 
		 * ClassDecl n = this; Node old = n;
		 * 
		 * // Ensure supertypes and signatures are disambiguated for all //
		 * classes visible from this class's scope.
		 * 
		 * // Disambiguate supertypes and signatures. SupertypeDisambiguator
		 * supDisamb = new SupertypeDisambiguator(ar); n = (ClassDecl)
		 * supDisamb.visitEdgeNoOverride(parent, n); if (supDisamb.hasErrors())
		 * throw new SemanticException();
		 * 
		 * // Hack to force n.disambiguate() to be called and supertypes set. //
		 * n = (ClassDecl) new SupertypeDisambiguator(tc).leave(parent, old, n,
		 * new SupertypeDisambiguator(tc)); SignatureDisambiguator sigDisamb =
		 * new SignatureDisambiguator(ar); n = (ClassDecl)
		 * sigDisamb.visitEdgeNoOverride(parent, n); if (sigDisamb.hasErrors())
		 * throw new SemanticException();
		 * 
		 * // Call enter and leave to manage the context. AmbiguityRemover
		 * childVisitor = (AmbiguityRemover) ar.enter(parent, n); if
		 * (ar.hasErrors()) throw new SemanticException();
		 * 
		 * n = (ClassDecl) n.visitChildren(childVisitor); if
		 * (childVisitor.hasErrors()) throw new SemanticException();
		 * 
	...
	public Node copy(NodeFactory nf) {
		return nf.ClassDecl(this.position, this.flags, this.name,
				this.superClass, this.interfaces, this.body);
	}
