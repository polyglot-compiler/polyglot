 ...
 * <code>switch</code> statement. Such a statement has an expression which is
 * evaluated to determine where to branch to, an a list of labels and block
 * statements which are conditionally evaluated. One of the labels, rather than
 * having a constant expression, may be lablled default.
 */
public class Switch_c extends Stmt_c implements Switch {
	...
	public Switch_c(Position pos, Expr expr, List elements) {
		...
		this.expr = expr;
		this.elements = TypedList.copyAndCheck(elements, SwitchElement.class,
				true);
	}
	...
	public Switch elements(List elements) {
		Switch_c n = (Switch_c) copy();
		n.elements = TypedList
				.copyAndCheck(elements, SwitchElement.class, true);
		return n;
	}
	...
	protected Switch_c reconstruct(Expr expr, List elements) {
		if (expr != this.expr
			...
			Switch_c n = (Switch_c) copy();
			n.expr = expr;
			n.elements = TypedList.copyAndCheck(elements, SwitchElement.class,
					true);
			return n;
		}
