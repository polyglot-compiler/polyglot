...
/**
 * A place holder type when serializing the Polylgot type information. When
 * serializing the type information for some class <code>C</code>, Placeholders
 * are used to prevent serializing the class type information for classes that
 * <code>C</code> depends on.
 */
public class PlaceHolder_c implements NamedPlaceHolder {
	...
	public boolean equals(Object o) {
		return o == this
				|| (o instanceof PlaceHolder_c && name
						.equals(((PlaceHolder_c) o).name));
	}

	/**
	 * Resolve the place holder into a TypeObject. This method should
	 * <strong>never</strong> throw a SchedulerException. Instead, it should
	 * return null if the object cannot be resolved until after another pass
	 * runs. The method is responsible for setting up the appropriate
	 * dependencies to reattempt the current goal.
	 */
	...
	public TypeObject resolveUnsafe(TypeSystem ts)
		...
		try {
			...
			scheduler
					.addDependencyAndEnqueue(scheduler.currentGoal(), g, false);
	...
	public TypeObject resolveSafe(TypeSystem ts)
		...
		scheduler.addDependencyAndEnqueue(scheduler.currentGoal(),
				scheduler.TypeExists(name), false);
