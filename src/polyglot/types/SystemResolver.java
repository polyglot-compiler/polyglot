...
/**
 * The <code>SystemResolver</code> is the main resolver for fully-qualified
 * names.
 */
public class SystemResolver extends CachingResolver implements TopLevelResolver {
		...
		for (Iterator i = cachedObjects().iterator(); i.hasNext();) {
			...
			if (o instanceof Importable) {
				...
				if (im.package_() != null
						...
						&& (im.package_().fullName().equals(name) || im
								.package_().fullName().startsWith(name + "."))) {
					return true;
				}
			}
		}
	...
	public Named find(String name) throws SemanticException {
		...
		if (previous == null) {
			...
			/*
			 * for (Iterator i = justAdded.iterator(); i.hasNext(); ) { Named n2
			 * = (Named) i.next(); if (n2 instanceof ParsedTypeObject) { if (!
			 * ((ParsedTypeObject) n2).initializer().isTypeObjectInitialized())
			 * { throw new InternalCompilerError(n +
			 * " is in the root system resolver, but not initialized"); } } }
			 */
