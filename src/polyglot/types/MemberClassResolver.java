...
/**
 * Loads member classes using a TopLevelResolver that can only handle top-level
 * classes.
 */
public class MemberClassResolver implements TopLevelResolver {
	 ...
	 *            The type system
	 * @param inner
	 *            The resolver for top-level classes
	 */
	...
	public Named find(String name) throws SemanticException {
		...
		try {
			...
			if (n instanceof ParsedTypeObject) {
				if (true
						|| ((ParsedTypeObject) n).initializer() != null
						&& ((ParsedTypeObject) n).initializer()
