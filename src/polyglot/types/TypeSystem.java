...
/**
 * The <code>TypeSystem</code> defines the types of the language and how they
 * are related.
 */
public interface TypeSystem {
	...
	/**
	 * Initialize the type system with the compiler. This method must be called
	 * before any other type system method is called.
	 * 
	 * @param resolver
	 *            The resolver to use for loading types from class files or
	 *            other source files.
	 * @param extInfo
	...
	/**
	 * Return the system resolver. This resolver contains types parsed from
	 * source files.
	 * 
	 * @deprecated
	 */
	...
	/**
	 * Create and install a duplicate of the system resolver and return the
	 * original.
	 */
	...
	/**
	 * Return the type system's loaded resolver. This resolver contains types
	 ...
	 * 
	 * @param sourceName
	 *            Name of the source file to import into. This is used mainly
	 *            for error messages and for debugging.
	 * @param pkg
	 *            The package of the source file in which to import.
	 */
	...
	/**
	 * Return a list of the packages names that will be imported by default. A
	 * list of Strings is returned, not a list of Packages.
	 */
	 ...
	 * @param container
	 *            Containing class of the initializer.
	 * @param flags
	 *            The initializer's flags.
	 */
	 ...
	 * @param container
	 *            Containing class of the constructor.
	 * @param flags
	 *            The constructor's flags.
	 * @param argTypes
	 *            The constructor's formal parameter types.
	 * @param excTypes
	...
	/**
	 * Returns true iff child descends from ancestor or child == ancestor. This
	 * is equivalent to:
	 * 
	 * <pre>
	...
	/**
	 * Returns true iff type1 and type2 are equivalent. This is usually the same
	 * as equals(type1, type2), but may differ in the presence of, say, type
	 * aliases.
	 */
	...
	/**
	 * Returns true iff <code>type</code> is a canonical (fully qualified) type.
	 */
	...
	/**
	 * Checks whether a class member can be accessed from the body of class
	...
	/**
	 * True if the class targetClass accessible from the body of class
	 * contextClass.
	 */
	...
	/**
	 * Returns a collection of the Throwable types that need not be declared in
	 ...
	 * 
	 * @exception SemanticException
	 *                if the type cannot be promoted.
	 */
	 ...
	 * 
	 * @exception SemanticException
	 *                if the types cannot be promoted.
	 */
	...
	/**
	 * Returns the field named 'name' defined on 'type'. We check if the field
	 * is accessible from the class currClass.
	 * 
	 * @exception SemanticException
	 *                if the field cannot be found or is inaccessible.
	 */
	...
	/**
	 * Find a member class. We check if the field is accessible from the class
	 * currClass.
	 * 
	 * @exception SemanticException
	 *                if the class cannot be found or is inaccessible.
	 */
	...
	/**
	 * Return a package by name with the given outer package. Fail if the
	...
	/**
	 * Return the set of objects that should be serialized into the type
	 * information for the given TypeObject. Usually only the object itself
	 * should get encoded, and references to other classes should just have
	 * their name written out. If it makes sense for additional types to be
	 * fully encoded, (i.e., they're necessary to correctly reconstruct the
	 * given object, and the usual class resolvers can't otherwise find them)
	 * they should be returned in the set in addition to object.
	 */
	Set getTypeEncoderRootSet(TypeObject o);

	/**
	 * Get the transformed class name of a class. This utility method returns
	 * the "mangled" name of the given class, whereby all periods ('.')
	 * following the toplevel class name are replaced with dollar signs ('$').
	 * If any of the containing classes is not a member class or a top level
	 ...
	 * 
	 * @param o
	 *            The object to get the place-holder for.
	 * @param roots
	 *            The root objects for the serialization. Place holders are not
	 *            created for these.
	 */
	...
	/**
	 * Return true if <code>pi</code> can be called with actual parameters of
	 * types <code>actualTypes</code>.
	 */
	boolean callValid(ProcedureInstance mi, List argTypes);

	/**
	 * Get the list of methods <code>mi</code> (potentially) overrides, in order
	 * from this class (that is, including <code>this</code>) to super classes.
	 */
	...
	/**
	 * Get the list of methods <code>mi</code> implements, in no specified
	 * order.
	 */
	...
	/**
	 * Assert that <code>ct</code> implements all abstract methods that it has
	 * to; that is, if it is a concrete class, then it must implement all
	 ...
	 * in the class <code>ct</code> or a supertype thereof. Since we are looking
	 * for implementations, <code>ct</code> cannot be an interface. The first
	 * potentially satisfying method is returned, that is, the method that is
	 * visible from <code>ct</code>, with the correct signature, in the most
	 * precise class in the class hierarchy starting from <code>ct</code>.
	 * 
	...
	/**
	 * Returns <code>t</code>, modified as necessary to make it a legal static
	 * target.
	 */
	public Type staticTarget(Type t);

	/**
	 * Given the JVM encoding of a set of flags, returns the Flags object for
	 * that encoding.
	 */
	 ...
	 * @param print_after
	 *            print the new flag after these flags
	 */
