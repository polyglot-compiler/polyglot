...
/**
 * A <code>Type</code> is the base type of all classes which represent types.
 */
public interface Type extends Qualifier {
	...
	/**
	 * Return true if this type is equivalent to t. Usually this is the same as
	 * equalsImpl(TypeObject), but that method should return true only if the
	 * types are <i>structurally equal</i>.
	 * 
	 * @param t
	 *            Type to compare to
	...
	/**
	 * Yields a string representing this type. The string should be consistent
	 * with equality. That is, if this.equals(anotherType), then it should be
	 * that this.toString().equals(anotherType.toString()).
	 * 
	 * The string is not necessarily a legal type expression in the language,
	 * such as a Java identifier. It is suggested, but not required, that it be
	 * an easily human-readable representation that can be used both in error
	 * messages and in generated output.
	 */
	String toString();

	/**
	 * Output a compilable representation of this type to w. For generated
	 * output, this method should be used rather than toString(). It is
	 * suggested, but not required, that it be an easily human-readable
	 * representation that can be used both in error messages and in generated
	 * output.
	 */
