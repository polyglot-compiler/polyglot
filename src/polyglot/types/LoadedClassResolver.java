...
public class LoadedClassResolver implements TopLevelResolver {
	 ...
	 * @param version
	 *            The version of classes to load.
	 * @param allowRawClasses
	 *            allow class files without encoded type information
	 */
	@Deprecated
	public LoadedClassResolver(TypeSystem ts, String classpath,
			ClassFileLoader loader, Version version, boolean allowRawClasses) {
		this(ts.extensionInfo(), allowRawClasses);
		this.loader = loader;
	}
	...
	public Named find(String name) throws SemanticException {
		...
		if (result != null) {
			...
			if (result instanceof ClassType
					&& name.equals(ts
							.getTransformedClassName((ClassType) result))) {
				return result;
			}
		}
	...
	protected ClassType getEncodedType(ClassFile clazz, String name)
		...
		if (comp == NOT_COMPATIBLE) {
			throw new SemanticException(
					"Unable to find a suitable definition of " + clazz.name()
		...
		try {
			...
			if (dt instanceof ClassType) {
				...
				if (Report.should_report("typedump", 1)) {
					new ObjectDumper(new SimpleCodeWriter(System.out, 72))
							.dump(dt);
				}
