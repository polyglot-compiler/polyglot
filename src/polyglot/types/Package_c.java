...
/**
 * An <code>PackageType</code> represents a package type. It may or may not be
 * fully qualified. Package types are never canonical and never primitive.
 */
public class Package_c extends TypeObject_c implements Package {
	...
	public Resolver resolver() {
		if (memberCache == null) {
			memberCache = new CachingResolver(
					ts.createPackageContextResolver(this));
		}
		return memberCache;
	}
	...
	public String fullName() {
		if (fullname == null) {
			fullname = prefix() == null ? name : prefix().fullName() + "."
					+ name;
		}
		return fullname;
	}
