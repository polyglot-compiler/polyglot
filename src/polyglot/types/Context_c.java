...
/**
 * This class maintains a context for looking up named variables, types, and
 * methods. It's implemented as a stack of Context objects. Each Context points
 * to an outer context. To enter a new scope, call one of the pushXXX methods.
 * To leave a scope, just follow the outer() pointer. NodeVisitors handle
 * leaving scope automatically. Each context object contains maps from names to
 * variable, type, and method objects declared in that scope.
 */
public class Context_c implements Context {
	...
	/**
	 * Return the code def that defines the local variable or type with the
	 * given name.
	 */
	...
	/**
	 * Returns whether the particular symbol is defined locally. If it isn't in
	 * this scope, we ask the parent scope, but don't traverse to enclosing
	 * classes.
	 */
	...
	public MethodInstance findMethod(String name, List argTypes)
		...
		if (this.currentClass() != null
			...
			return ts.findMethod(this.currentClass(), name, argTypes,
					this.currentClass());
		}
	 ...
	 * 
	 * @param classScope
	 *            The class whose scope is being entered. This is the object
	 *            associated with the class declaration and is returned by
	 *            currentClassScope. This is a mutable class type since for some
	 *            passes (e.g., addMembers), the object returned by
	 *            currentClassScope is modified.
	 * @param type
	 *            The type to be returned by currentClass(). For JL, this type
	 *            is the same as classScope. For other languages, it may differ
	 *            since currentClassScope might not represent a type.
	 * @return A new context with a new scope and which maps the short name of
	 *         type to type.
	 */
	public Context pushClass(ParsedClassType classScope, ClassType type) {
		if (Report.should_report(TOPICS, 4))
			Report.report(4,
					"push class " + classScope + " " + classScope.position());
		Context_c v = push();
		v.kind = CLASS;
		v.scope = classScope;
		v.type = type;
	...
	/**
	 * Returns whether the current context is a static context. A statement of
	 * expression occurs in a static context if and only if the inner-most
	 * method, constructor, instance initializer, static initializer, field
	 * initializer, or explicit constructor statement enclosing the statement or
	 * expressions is a static method, static initializer, the variable
	 * initializer of a static variable, or an explicity constructor invocation
	 * statment. (Java Language Spec, 2nd Edition, 8.1.2)
	 */
	...
	/**
	 * Adds a method to the current scoping level. Actually, this does nothing
	 * now.
	 * 
	 * @deprecated
	 */
