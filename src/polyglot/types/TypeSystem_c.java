 ...
 * TypeSystem_c
 * 
 * Overview: A TypeSystem_c is a universe of types, including all Java types.
 **/
public class TypeSystem_c implements TypeSystem {
	...
	protected void initTypes() throws SemanticException {
		...
		/*
		 * DISABLED CACHING OF COMMON CLASSES; CAUSES PROBLEMS IF COMPILING CORE
		 * CLASSES (e.g. java.lang package). TODO: Longer term fix. Maybe a flag
		 * to tell if we are compiling core classes? XXX Object(); Class();
		 * String(); Throwable();
		 * 
	...
	public void restoreSystemResolver(SystemResolver r) {
		if (r != this.systemResolver.previous()) {
			throw new InternalCompilerError(
					"Inconsistent systemResolver.previous");
		}
	...
	public Resolver packageContextResolver(Package p, ClassType accessor) {
		if (accessor == null) {
			...
			return new AccessControlWrapperResolver(
					createPackageContextResolver(p), accessor);
		}
	}
	...
	public Resolver classContextResolver(ClassType type, ClassType accessor) {
		...
		if (accessor == null) {
			...
			return new AccessControlWrapperResolver(
					createClassContextResolver(type), accessor);
		}
	}
	...
	public ConstructorInstance constructorInstance(Position pos,
		...
		assert_(argTypes);
		assert_(excTypes);
		return new ConstructorInstance_c(this, pos, container, flags, argTypes,
				excTypes);
	}
	...
	public MethodInstance methodInstance(Position pos, ReferenceType container,
		...
		assert_(argTypes);
		assert_(excTypes);
		return new MethodInstance_c(this, pos, container, flags, returnType,
				name, argTypes, excTypes);
	}

	/**
	 * Returns true iff child and ancestor are distinct reference types, and
	 * child descends from ancestor.
	 **/
	...
	/**
	 * Checks whether the member mi can be accessed from code that is declared
	 * in the class contextClass.
	 */
	...
	public boolean classAccessible(ClassType targetClass, Context context) {
		if (context.currentClass() == null) {
			return classAccessibleFromPackage(targetClass, context
					.importTable().package_());
		} else {
	...
	/**
	 * Returns a list of the Throwable types that need not be declared in method
	...
	/**
	 * Returns the FieldInstance for the field <code>name</code> defined in type
	 * <code>container</code> or a supertype, and visible from
	 * <code>currClass</code>. If no such field is found, a SemanticException is
	 * thrown. <code>currClass</code> may be null.
	 **/
	public FieldInstance findField(ReferenceType container, String name,
	...
	/**
	 * Returns the FieldInstance for the field <code>name</code> defined in type
	 * <code>container</code> or a supertype. If no such field is found, a
	...
	/**
	 * Returns a set of fields named <code>name</code> defined in type
	...
	protected Set findFields(ReferenceType container, String name) {
		...
		if (container.superType() != null
			...
			Set superFields = findFields(container.superType().toReference(),
					name);
	...
	/**
	 * Returns the list of methods with the given name defined or inherited into
	 * container, checking if the methods are accessible from the body of
	 * currClass
	 */
	 ...
	 * context. If no such field may be found, returns a fieldmatch with an
	 * error explaining why. Access flags are considered.
	 **/
	public MethodInstance findMethod(ReferenceType container, String name,
	...
	public ConstructorInstance findConstructor(ClassType container,
		...
		ConstructorInstance ci = (ConstructorInstance) maximal.iterator()
				.next();
		return ci;
	}

	protected ProcedureInstance findProcedure(List acceptable,
			ReferenceType container, List argTypes, ClassType currClass)
			throws SemanticException {
		Collection maximal = findMostSpecificProcedures(acceptable);
			...
			if (notAbstract.size() == 1) {
				...
				while (j.hasNext()) {
					...
					if (first instanceof Declaration) {
						firstDecl = (ProcedureInstance) ((Declaration) first)
								.declaration();
					}
					if (p instanceof Declaration) {
						pDecl = (ProcedureInstance) ((Declaration) p)
								.declaration();
					}
	...
	protected List findAcceptableMethods(ReferenceType container, String name,
		...
		while (!typeQueue.isEmpty()) {
			...
			for (Iterator i = type.toReference().methods().iterator(); i
					.hasNext();) {
	...
	public Type leastCommonAncestor(Type type1, Type type2)
		...
		if (type1.isArray() && type2.isArray()) {
			return arrayOf(leastCommonAncestor(type1.toArray().base(), type2
					.toArray().base()));
		}
		...
		if (type1.isReference() && type2.isReference()) {
			...
			if (typeEquals(type1, Object()))
				return type1;
			if (typeEquals(type2, Object()))
				return type2;
			...
			if (!StringUtil.isNameShort(name)) {
				...
				try {
					...
					if (container instanceof ClassType) {
						return classContextResolver((ClassType) container)
								.find(shortName);
					}
	 ...
	 * Does not require that <theClass> have a JavaClass registered in this
	 * typeSystem. Does not register the type in this TypeSystem. For use only
	 * by JavaClass implementations.
	 **/
	 ...
	 * given clazz, and the usual class resolvers can't otherwise find them)
	 * they should be returned in the set in addition to clazz.
	 */
	...
	public Flags legalTopLevelClassFlags() {
		return legalAccessFlags().clear(Private()).Abstract().Final()
				.StrictFP().Interface();
	}
	...
	public Flags legalMemberClassFlags() {
		return legalAccessFlags().Static().Abstract().Final().StrictFP()
				.Interface();
	}
	...
	public Flags flagsForBits(int bits) {
		...
		if ((bits & Modifier.PUBLIC) != 0)
			f = f.Public();
		if ((bits & Modifier.PRIVATE) != 0)
			f = f.Private();
		if ((bits & Modifier.PROTECTED) != 0)
			f = f.Protected();
		if ((bits & Modifier.STATIC) != 0)
			f = f.Static();
		if ((bits & Modifier.FINAL) != 0)
			f = f.Final();
		if ((bits & Modifier.SYNCHRONIZED) != 0)
			f = f.Synchronized();
		if ((bits & Modifier.TRANSIENT) != 0)
			f = f.Transient();
		if ((bits & Modifier.NATIVE) != 0)
			f = f.Native();
		if ((bits & Modifier.INTERFACE) != 0)
			f = f.Interface();
		if ((bits & Modifier.ABSTRACT) != 0)
			f = f.Abstract();
		if ((bits & Modifier.VOLATILE) != 0)
			f = f.Volatile();
		if ((bits & Modifier.STRICT) != 0)
			f = f.StrictFP();
