 ...
 * and variables. To push a new scope call one of the <code>push*</code> methods
 * to return a new context. The old context may still be used and may be
 * accessed directly through a call to <code>pop()</code>. While the stack of
 * scopes is treated functionally, each individual scope is updated in place.
 * Names added to the context are added in the current scope.
 */
public interface Context extends Resolver, Copy {
	 ...
	 */
	MethodInstance findMethod(String name, List formalTypes)
	...
	/**
	 * Finds the class which added a field to the scope. This is usually a
	 * subclass of <code>findField(name).container()</code>.
	 */
	ClassType findFieldScope(String name) throws SemanticException;

	/**
	 * Finds the class which added a method to the scope. This is usually a
	 * subclass of <code>findMethod(name).container()</code>.
	 */
	...
	/**
	 * Get the outer-most resolver for the source file currently in scope. This
	 * is usually just the system resolver.
	 * 
	 * @deprecated
	 */
	...
	/**
	 * Enter a static scope. In general, this is only used for explicit
	 * constructor calls; static methods, initializers of static fields and
	 * static initializers are generally handled by pushCode().
	 */
	Context pushStatic();
	...
	/**
	 * Return the code instance that defines the local symbol with the given
	 * name, or null.
	 */
