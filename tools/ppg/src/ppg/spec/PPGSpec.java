...
public class PPGSpec extends Spec {
	 ...
	 * PPG spec
	 * 
	 * public PPGSpec (String incFile, String pkg, Vector imp, Vector codeParts,
	 * Vector syms, Vector precedence, String startSym, Vector cmds) { super();
	 * include = incFile; packageName = pkg; imports = imp; code = codeParts;
	 * symbols = syms; prec = precedence; start = startSym; startSyms = null;
	 * commands = cmds; parent = null; }
	 */

	public PPGSpec(String incFile, String pkg, Vector imp, Vector codeParts,
			Vector syms, Vector precedence, Vector startList, Vector cmds) {
		super();
		include = incFile;
		packageName = pkg;
		imports = imp;
	 ...
	 * Token curr_sym;
	 * 
	 * // for each [start_sym, method, token] create code: method () { curr_sym
	 * = token; parse(); }
	 * 
	 * // ^^^^ parse code ^^^^
	 * 
	 * // in front of scanner, add code: // return the "fake" symbol only once
	 * after it's set if (curr_sym != null) Token result = curr_sym; curr_sym =
	 * null; return result; }
	 * 
	 * // ^^^^ scan code ^^^^
	 * 
	 * // code added to grammar: start with new_unique_start_symbol;
	 * 
	 * new_unique_start_symbol ::= token_1 start_sym_1:s {: RESULT = s; :} |
	...
	public void patchMultiStartSymbols(CUPSpec cupSpec) {
		...
		SymbolList tokenList = new SymbolList(SymbolList.TERMINAL, "Symbol",
				tokens);
	...
	public CUPSpec coalesce() throws PPGError {
		 ...
		 * 
		 * precedence list null: delete precedence list of parent precedence
		 * list of length 0: leave precedence of parent precedence list of
		 * length >0: override with current list
		 */
			...
			if (cmd instanceof DropCmd) {
				...
				if (drop.isProdDrop()) {
					...
					for (int j = 0; j < symbols.size(); j++) {
						...
						// remove all productions that have NT as lhs, if
						// possible
	...
	private void processTransferL(CUPSpec combined, CUPSpec newSpec) {
		...
		for (int i = 0; i < commands.size(); i++) {
			...
			if (cmd instanceof TransferCmd) {
				...
				// there must be at least one production by the grammar
				// definition
