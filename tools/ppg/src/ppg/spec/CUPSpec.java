...
public class CUPSpec extends Spec {
	 ...
	 * grammar, but is equal (minus semantic actions) to the given production
	 * set. Thus, we transfer the semantic actions without having to re-specify
	 * them.
	 */
	...
	public Object clone() {
		String newPkgName = (packageName == null) ? null : packageName
				.toString();
		/*******************/
		...
		if (actionCode != null)
			newCode.addElement(actionCode);
		if (initCode != null)
			newCode.addElement(initCode);
		if (parserCode != null)
			newCode.addElement(parserCode);
		if (scanCode != null)
			newCode.addElement(scanCode);
		/*
		 * for (int i=0; i < code.size(); i++) { newCode.addElement( ((Code)
		 * code.elementAt(i)).clone()); }
		 */
		/*******************/
		...
		/*
		 * return new CUPSpec(newPkgName, (Vector) imports.clone(), (Vector)
		 * code.clone(), (Vector) symbols.clone(), (Vector) prec.clone(),
		 * newStart, (Vector) productions.clone());
		 */
	}
	...
	public void dropSymbol(String gs) throws PPGError {
		...
		/*
		 * if (!dropped) throw new PPGError("file", -1,
		 * "symbol "+gs+" not found.");
		 */
	}
	...
	private int errorNotFound(int i, Nonterminal nt) {
		if (i == NT_NOT_FOUND) {
			...
			System.err
					.println(PPG.HEADER + "nonterminal " + nt + " not found.");
			System.exit(1);
		}
		return i;
	}
	...
	public void unparse(CodeWriter cw) {
		...
		/*
		 * try { export(System.out); } catch (Exception e) {
		 * System.err.println(HEADER+"Exception: "+e.getMessage()); return; }
		 */
	}
	...
	public void export(PrintStream out) throws Exception {
		...
		// code
		/*
		 * for (int i=0; i < code.size(); i++) out.println( ((Code)
