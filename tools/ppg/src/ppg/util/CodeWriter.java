...
/**
 * A <code>CodeWriter</code> is a pretty-printing engine. It formats structured
 * text onto an output stream <code>o</code> in the minimum number of lines,
 * while keeping the width of the output within <code>width</code> characters if
 * possible.
 */
public class CodeWriter {
	...
	/**
	 * Create a CodeWriter object with output <code>w</code> and width
	 * <code>width_</code>.
	 */
	 ...
	 * will break the block into lines: every <code>allowBreak</code> will cause
	 * a line change, the first line is printed at the current cursor position
	 ...
	 *            the current position) for all lines in the block.
	 */
	...
	/**
	 * Allow a newline. Indentation will be preserved. If no newline is
	 * inserted, a single space character is output instead.
	 * 
	 * @param n
	 *            the amount of increase in indentation if the newline is
	 *            inserted.
	 */
	 ...
	 * 
	 * @param n
	 *            the amount of increase in indentation after the newline.
	 */
...
abstract class Item {
	...
	/**
	 * Try to format this item and subsequent items. The current cursor position
	 * is <code>pos</code>, left and right margins are as specified. Returns the
	 * final position, which must be <code>&lt;
	 * fin</code>. If breaks may be broken, <code>can_break</code> is set.
	 * Return the new cursor position (which may overrun rmargin, fin, or both,
	 * and set any contained breaks accordingly. (It is important that formatN
	 * not necessarily convert overruns in its final position into exceptions.
	 * This allows the calling routine to distinguish between 'internal'
	 * overruns and ones that it can tack on a conservative estimate of how much
	 * formatting the rest of the list will make the overrun go up by. Also, it
	 * simplifies the coding of formatN.)
	 * 
	...
	/**
	 * Make the garbage collector's job easy: free references to any other
	 * items.
	 */
	void free() {
		if (next != null) {
	...
	/**
	 * Try to format a whole sequence of items in the manner of formatN. The
	 * initial position may be an overrun (this is the only way that overruns
	 * are checked!) <code>it</code> may be also null, signifying an empty list.
	 */
...
/**
 * A Block is a formatting unit containing a list of other items to be
 * formatted.
 */
class Block extends Item {
	...
	int formatN(int lmargin, int pos, int rmargin, int fin, boolean can_break,
		...
		while (true) {
			int next_pos;
			try {
				next_pos = format(first, pos + indent, pos, rmargin, this_fin,
						this_break, this_nofail && this_break);
			} catch (Overrun o) {
				if (!can_break)
					throw o;
				if (!this_break) {
			...
			try {
				return format(next, lmargin, next_pos, rmargin, fin, can_break,
						nofail);
			} catch (Overrun o) {
				if (!can_break)
					throw o; // no way to fix it
				if (next instanceof AllowBreak)
					throw o; // not our fault
				this_break = true;
...
class StringItem extends Item {
	...
	int formatN(int lmargin, int pos, int rmargin, int fin, boolean can_break,
			boolean nofail) throws Overrun {
		return format(next, lmargin, pos + s.length(), rmargin, fin, can_break,
				nofail);
	}
...
class Newline extends AllowBreak {
	...
	int formatN(int lmargin, int pos, int rmargin, int fin, boolean can_break,
		...
		if (!can_break)
			throw new Overrun(1);
		return format(next, lmargin, lmargin + indent, rmargin, fin, can_break,
				nofail);
	}
