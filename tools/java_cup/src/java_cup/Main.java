...
public class Main {
    ...
    public static void main(String argv[])
        ...
        if (print_progress) System.err.println("Parsing specification from standard input...");
        parse_grammar_spec();
    ...
    protected static void usage(String message) {
        ...
        System.err.println(
                version.title_str
                        ...
                        + " [options] [filename]\n"
                        + "  and expects a specification file on standard input if no filename is"
                        + " given.\n"
                        + "  Legal options include:\n"
                        + "    -package name  specify package generated classes go in [default"
                        + " none]\n"
                        + "    -destdir name  specify the destination directory, to store the"
                        + " generated files in\n"
                        + "    -parser name   specify parser class name [default \"parser\"]\n"
                        + "    -typearg args  specify type arguments for parser class\n"
                        + "    -symbols name  specify name for symbol constant class [default"
                        + " \"sym\"]\n"
                        + "    -interface     put symbols in an interface, rather than a class\n"
                        + "    -nonterms      put non terminals in symbol constant class\n"
                        + "    -expect #      number of conflicts expected/allowed [default 0]\n"
                        + "    -compact_red   compact tables by defaulting to most frequent"
                        + " reduce\n"
                        + "    -max_actions   maximum number of actions per method in generated"
                        + " code\n"
                        + "                   (useful if javac complains about code size >64K)\n"
                        + "                   [default 400]\n"
                        + "    -nowarn        don't warn about useless productions, etc.\n"
                        + "    -nosummary     don't print the usual summary of parse states, etc.\n"
                        + "    -nopositions   don't propagate the left and right token position"
                        + " values\n"
                        + "    -locations     generate handles xleft/xright for symbol positions in"
                        + " actions\n"
                        + "    -xmlactions    make the generated parser yield its parse tree as"
                        + " XML\n"
                        + "    -genericlabels automatically generate labels to all symbols in XML"
                        + " mode\n"
                        + "    -noscanner     don't refer to java_cup.runtime.Scanner\n"
                        + "    -progress      print messages to indicate progress of the system\n"
                        + "    -time          print time usage summary\n"
                        + "    -dump_grammar  produce a human readable dump of the symbols and"
                        + " grammar\n"
    ...
    protected static void build_parser() throws internal_error {
        ...
        if (opt_do_debug || print_progress) System.err.println("  Computing first sets...");
        non_terminal.compute_first_sets();

        first_end = System.currentTimeMillis();

        /* build the LR viable prefix recognition machine */
        if (opt_do_debug || print_progress) System.err.println("  Building state machine...");
    ...
    protected static void emit_summary(boolean output_produced) {
        ...
        System.err.print(
                non_terminal.number() + " non-terminal" + plural(non_terminal.number()) + ", and ");
        System.err.println(
                production.number() + " production" + plural(production.number()) + " declared, ");
    ...
    protected static void show_times() {
        ...
        if (check_end != 0)
            System.err.println(
                    "      Checking       " + timestr(check_end - parse_end, total_time));
        if (check_end != 0 && build_end != 0)
            System.err.println(
                    "      Parser Build   " + timestr(build_end - check_end, total_time));
        if (nullability_end != 0 && check_end != 0)
            System.err.println(
                    "        Nullability  " + timestr(nullability_end - check_end, total_time));
        if (first_end != 0 && nullability_end != 0)
            System.err.println(
                    "        First sets   " + timestr(first_end - nullability_end, total_time));
        if (machine_end != 0 && first_end != 0)
            System.err.println(
                    "        State build  " + timestr(machine_end - first_end, total_time));
        if (table_end != 0 && machine_end != 0)
            System.err.println(
                    "        Table build  " + timestr(table_end - machine_end, total_time));
        if (reduce_check_end != 0 && table_end != 0)
            System.err.println(
                    "        Checking     " + timestr(reduce_check_end - table_end, total_time));
        ...
        if (emit.action_code_time != 0)
            System.err.println(
                    "          Actions    " + timestr(emit.action_code_time, total_time));
        if (emit.production_table_time != 0)
            System.err.println(
                    "          Prod table " + timestr(emit.production_table_time, total_time));
        if (emit.action_table_time != 0)
            System.err.println(
                    "          Action tab " + timestr(emit.action_table_time, total_time));
    ...
    public static void dump_grammar() throws internal_error {
        ...
        for (int pidx = 0; pidx < production.number(); pidx++) {
            ...
            for (int i = 0; i < prod.rhs_length(); i++)
                if (prod.rhs(i).is_action()) System.err.print("{action} ");
                else System.err.print(((symbol_part) prod.rhs(i)).the_symbol().name() + " ");
            System.err.println();
        }
        System.err.println();
    }
    ...
    public static void dump_machine() {
        ...
        for (lalr_state st : lalr_state.all_states()) ordered[st.index()] = st;
