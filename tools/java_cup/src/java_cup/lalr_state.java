...
public class lalr_state {
			...
			if (itm != red_itm && !itm.dot_at_end()) {
				...
				if (!shift_sym.is_non_term()
					...
					if (Main.report_counterexamples) {
						ds.reset();
						System.err.print("    Example:    ");
						report_shortest_path(itm, System.err, new PrintStream(
								ds));
						System.err.print(" (*) ");
	...
	private class Path {
		...
		/**
		 * steps is a linked list of lalr_transition _or_ production. The latter
		 * are found in the path when a "push" occurs to work on a new
		 * production.
		 */
		LinkedList steps;
	...
	/**
	 * Report on example_s a textual version of the shortest path from the start
	 * state and start item to the current state and item itm. Report on
	 * derivation_str a more detailed textual description including derivation
	 * information. This output is useful for diagnosing conflicts in the
	 * grammar.
	 */
	...
	/**
	 * Find the shortest way to get from the start state and start item to the
	 * current state and the item "itm". The steps that change the item being
	 * transitioned on (corresponding to items formed through closure
	 * computation) are represented explicitly in this path.
	 */
	protected Path shortest_path(lalr_item itm) throws internal_error {
		...
		while (!active.isEmpty()) {
			...
			for (lalr_transition tr = s.transitions(); tr != null; tr = tr
					.next()) {
			...
			if (nt != null) {
				for (Enumeration pi = nt.productions(); pi.hasMoreElements();) {
					terminal_set new_lookaheads = i.calc_lookahead(i
							.lookahead());
					boolean need_prop = i.lookahead_visible();
					production prod = (production) pi.nextElement();
					lalr_item i2 = new lalr_item(prod, new terminal_set(
							new_lookaheads));
					LinkedList newt = new LinkedList(p1.steps);
					newt.add(prod);
