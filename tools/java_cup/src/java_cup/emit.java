...
public class emit {
	...
	/**
	 * Extends and implements declarations for the parser class (ACM extension)
	 */
	 ...
	 *            the start production of the grammar.
	 * @param max_actions
	 *            max number of actions per method in generated code
	 */
	protected static void emit_action_code(PrintWriter out,
		...
		if (num_prods > max_actions) {
			emit_dispatch_methods(out, 0, num_prods - 1, max_actions,
					start_prod);
		}
	...
	protected static void emit_production_block(PrintWriter out,
		...
		if (emit.lr_values()) {
			...
			out.println("              " + pre("result")
					...
					+ prod.lhs().the_symbol().name() + "*/" + ", " + leftstring
					+ ", " + rightstring + ", RESULT);");
		} else {
			out.println("              " + pre("result")
					+ " = new java_cup.runtime.Symbol("
